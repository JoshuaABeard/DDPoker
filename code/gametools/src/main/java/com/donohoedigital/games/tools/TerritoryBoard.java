...
public class TerritoryBoard extends Gameboard implements KeyListener, DrawingUtilInterface {
     ...
     */
    public TerritoryBoard(BaseFrame frame, GameboardTerritoryManager manager, GameboardConfig gameconfig) {
    ...
    protected void paintComponent(Graphics g1) {
        ...
        g.drawRect(scaleToCurrentSpace(bounds.x), scaleToCurrentSpace(bounds.y), scaleToCurrentSpace(bounds.width),
                scaleToCurrentSpace(bounds.height));
    }

    /**
     * draw our points - otherwise call superclass (called from
     * super.drawTerritories if territory needs to be repainted)
     */
    ...
    private void updateStatus(TerritoryPoint point) {
        if (point == null) {
            ...
            manager_.setStatus(t.getName() + sScale + " - " + point);
        }
    }
    ...
    public void processMouseClick(MouseEvent e) {
        ...
        if (t != null && !t.isDecoration()) {
            ...
            if (allPoints_.contains(point)) {
                logger.warn(
                        "New territory point already exists as a defined point.  Ignoring it! - " + point.shortDesc());
                return;
            }
