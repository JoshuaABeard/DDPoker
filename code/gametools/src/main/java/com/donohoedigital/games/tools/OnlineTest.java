...
/**
 * Tool which was used to develop online messaging architecture. I think this
 * was used mainly during development, so not sure that it is even useful
 * anymore. I was able to start a pokerserver and run the
 * bTestDDMessenger_=true. I'm not sure that the bTestP2P_=true was used for.
 */
...
public class OnlineTest implements DDMessageListener {
    ...
    // Options (switch from true to false to decide what to test) TODO: make
    // parameter driven
    ...
    private void init() {
        ...
        errors_ = new String[]{
                PropertyConfig.getMessage("msg.msgerror." + DDMessageListener.STATUS_CONNECT_FAILED + MOD),
    ...
    public void messageReceived(DDMessage message) {
        ...
        if (nStatus == DDMessageListener.STATUS_OK) {
            ...
            logger.debug("*** ERROR: " + errors_[nStatus] + " :: "
                    + Utils.joinWithDelimiter(" | ", msg, exception, ddException));
        }
    }
