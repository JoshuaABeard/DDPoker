...
public class BorderChooser extends InternalDialog implements ActionListener {
    ...
    private JComponent createTable(TerritoryTableModel model, String sLabel, int nIndex) {
        ...
        choice.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(4, 4, 4, swidth),
    ...
    /**
     * Private table which knows about its related label and the index of the
     * territory it is picking
     */
    ...
    private void enableButtons() {
        if (choices[0] != choices[1] && choices[0] != null && choices[1] != null && getNumber() != null) {
    ...
    public static Border getBorder(BorderChooser b, int x, int y) {
        // figure out the best location so not to cover x,y point with buffer of 10
        // pixels
