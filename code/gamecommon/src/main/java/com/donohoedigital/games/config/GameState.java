...
public class GameState extends MsgState implements SaveFile {
    ...
    /**
     * Creates a new instance of GameState from an existing file
     */
    ...
    /**
     * Creates a new instance of GameState from a byte array
     */
    ...
    /**
     * Create a new instance of GameState, given a name (stored in the file) and an
     * extension for the actual file. File saved as <sBegin>.NNNNNN.sExt where
     * NNNNNN is the next available file number. sDesc is a description of the game
     */
    ...
    /**
     * Create game with just name and description - for use for temporary game
     * states like in online play.
     */
    ...
    /**
     * Indicate whether the file associated with this game state is in the save
     * directory (might not be if loaded from email/web)
     */
    ...
    /**
     * Change file's location to save dir (used when file loaded from some other
     * location, like an email temp dir)
     */
    ...
    /**
     * Set the name of the file and extension - this method creates the appropriate
     * file object in the correct directory. Used in online play, which uses the
     * name of the online game instead of the normal save names
     */
    public void setOnlineGameID(String sID, String sExt) {
        file_ = new File(GameConfigUtils.getSaveDir(),
                ONLINE_GAME_BEGIN + SaveFile.DELIM + sID + SaveFile.DELIM + sExt);
    }

    /**
     * Does this save file represent an online game? Returns false if no file
     * defined
     */
    ...
    /**
     * read data in. If bFull is false, only the first line is read to get
    ...
    /**
     * Ask each game state entry to finish parsing. Called after read is done (and
     * all Object ids are known)
     */
    ...
    public void savePlayers(GamePlayerList players) {
        if (details_.getSavePlayers() == SaveDetails.SAVE_NONE)
            return;
    ...
    public void loadPlayers(GamePlayerList players) {
        ...
        if (details_.getSavePlayers() == SaveDetails.SAVE_ALL)
            players.clearPlayerList(nNum);
    ...
    public void saveObservers(GameObserverList observers) {
        if (details_.getSaveObservers() == SaveDetails.SAVE_NONE)
            return;
    ...
    public void loadObservers(GameObserverList observers) {
        ...
        if (details_.getSaveObservers() == SaveDetails.SAVE_ALL)
            observers.clearObserverList(nNum);
    ...
    public void saveTerritories(Territory[] territories) {
        if (details_.getSaveTerritories() == SaveDetails.SAVE_NONE || territories == null)
            return;
    ...
    public void loadTerritories() {
        if (details_.getSaveTerritories() == SaveDetails.SAVE_NONE)
            return;
    ...
    public void saveCustomData() {
        if (details_.getSaveCustom() == SaveDetails.SAVE_NONE)
            return;
        if (delegate_ != null) {
     ...
     * Prepopulate set ids
     */
    public void prepopulateIds(Object game, Territory[] territories, GamePlayerList players,
    ...
    /**
     * Get the phase to run (from the delegate) when restoring a game. This phase is
     * typically the one that creates the main screen for the game and subclasses
     * ChainPhase. The next-phase and params are passed in from the data stored in
     * the GameState.
     */
