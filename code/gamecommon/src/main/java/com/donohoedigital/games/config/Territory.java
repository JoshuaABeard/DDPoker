...
public class Territory implements GamePieceContainer, ObjectID, Comparable {
     ...
     */
    public static final String LABEL_LOCATION = PropertyConfig
            .getRequiredStringProperty("define.territoryPointType.label");
    ...
    /**
     * name of type representing a decorative territory - we don't allow these to be
     ...
     */
    public Territory(Integer id, String sName, String sArea, String sType) throws ApplicationError {
     ...
     */
    public Territory(Element territory, Namespace ns, String sAttrErrorDesc) throws ApplicationError {
        ...
        for (int j = 0; j < nNumTerriotryPoints; j++) {
            sAttrErrorDesc = "TerritoryPoint #" + (j + 1) + " of territory " + sName + " in "
                    + GameboardConfig.GAMEBOARD_CONFIG;
    ...
    /**
     * Return % to scale images as a double value. For example, 25% is returned as
     * .25
     */
    ...
    /**
     * Return whether this is the territory representing the EDGE of the game board
     */
    ...
    /**
     * Return whether this is a decorative territory (just there to look nice, but
    ...
    /**
     * Return true if this is an island (is land and the number of adjacent land
     * regions is 0)
     */
    ...
    /**
     * Return whether the given obj matches this territory. Names must match. Type
     * and Area don't factor in equality (all territory names must be unique)
     */
    ...
    public void printXML(XMLWriter writer, int nIndent) {
        ...
        if (nCat_ != null)
            writer.printAttribute("cat", nCat_);
        if (nScaleImages_ != null && !nScaleImages_.equals(FULL_SCALE))
            writer.printAttribute("scaleimages", nScaleImages_);
    ...
    /**
     * creates array of adjacent territories if not already created. Then calculates
     * number of water/land adjacent as this can change by manipulation of territory
     * types. If bClearBorders is true, the border data in this territory is removed
     * as it is no longer needed after the adjacent list is built
     */
    ...
    /**
     * Used in game mode - creates path from borders so this territory can be drawn
     */
    public void createPath() {
        ...
        for (int i = borders.size() - 1; i >= 0; i--) {
            if (borders.getBorder(i).isWrapAround())
                borders.remove(i);
        }
        Border border = null;
        BorderPoints points;
    ...
    /**
     * Return bounds of path (instead of computing it every time)
     */
    ...
    /**
     * Return scaled path which represents territory adjusted to current window size
     */
    ...
    /**
     * Set the scaled path (and store bounds, create the resuable path iter)
     */
    ...
    /**
     * Return area of path (instead of computing it every time)
     */
    ...
    /**
     * Copy contains() from GeneralPath, but with reusable path iter, to avoid so
     * many creations
     */
    ...
    /**
     * Return area representing a stroke around the area using the template as the
     * basis for a new stoke whos width is nNewStokeWidth
     */

    // NOTE: not currently used because not any faster than stoke method

    // public java.awt.geom.Area getScaledPathBorderArea(BasicStroke template, float
    // nNewStrokeWidth)
    // {
    // boolean bNew = false;
    ...
    // {
    // lastStroke_ = new BasicStroke(nNewStrokeWidth, template.getEndCap(),
    // template.getLineJoin());
    // bNew = true;
    // }
    //
    ...
    // borderArea_.intersect(new
    // java.awt.geom.Area(lastStroke_.createStrokedShape(area)));
    // }
    // return borderArea_;
    // }
    //
    ...
    public boolean isWaterAdjacent() {
        for (Territory anAdjacentTerritories_ : adjacentTerritories_) {
            if (anAdjacentTerritories_.isWater())
                return true;
        }
        return false;
    }
    ...
    /**
     * Is there a land territory owned by given player adjacent to this territory?
     */
    public boolean isLandAdjacent(GamePlayer player) {
        for (Territory anAdjacentTerritories_ : adjacentTerritories_) {
            if (anAdjacentTerritories_.isLand() && anAdjacentTerritories_.getGamePlayer() == player)
                return true;
        }
        return false;
    }
    ...
    public boolean isAdjacent(GamePlayer playerOwner, int nType) {
        for (Territory anAdjacentTerritories_ : adjacentTerritories_) {
            if (anAdjacentTerritories_.getGamePlayer() == playerOwner
                    && anAdjacentTerritories_.hasOwnerPiece(nType, playerOwner))
                return true;
        }
        return false;
    }
    ...
    /**
     * Is there a territory owned by territoryOwner adjacent to this territory which
     * contains a piece of the given type owned by pieceOwner?
     */
    public boolean isAdjacent(GamePlayer territoryOwner, GamePlayer pieceOwner, int nType) {
        for (Territory anAdjacentTerritories_ : adjacentTerritories_) {
            if (anAdjacentTerritories_.getGamePlayer() == territoryOwner
                    && anAdjacentTerritories_.hasOwnerPiece(nType, pieceOwner))
                return true;
        }
        return false;
    }
    ...
    /**
     * Get generic array for storing list of territory names associated with this
     * territory - used for application-specific functionality
     */
    ...
    /**
     * Set user data (allow applications to store arbitrary info on territory). This
     * information is not saved/marshalled
     */
