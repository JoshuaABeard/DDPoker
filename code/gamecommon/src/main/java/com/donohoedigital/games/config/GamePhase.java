...
public class GamePhase extends TypedHashMap {
     ...
     */
    public GamePhase(Element phase, Namespace ns, String sAttrErrorDesc) throws ApplicationError {
        ...
        if (sExtends_ != null) {
            ...
            if (extend == null) {
                ...
                logger.error(sMsg);
                throw new ApplicationError(ErrorCodes.ERROR_VALIDATION, sMsg, "Make sure order is correct in "
                        + GamedefConfig.GAMEDEF_CONFIG + "(" + sExtends_ + " must appear before " + sName_);
            } else {
                // copy class and params from phase we extend
                if (sClassname_ == null)
                    sClassname_ = extend.sClassname_;
                if (bCache_ == null)
                    bCache_ = extend.bCache_;
                if (bHistory_ == null)
                    bHistory_ = extend.bHistory_;
                if (bTransient_ == null)
                    bTransient_ = extend.bTransient_;
                if (sWindow_ == null)
                    sWindow_ = extend.sWindow_;
                putAll(extend);
            }
        ...
        if (bCache_ == null)
            bCache_ = Boolean.FALSE;
        if (bHistory_ == null)
            bHistory_ = Boolean.FALSE;
        if (bTransient_ == null)
            bTransient_ = Boolean.FALSE;
    ...
    /**
     * If a phase is transient, it isn't recorded as the current phase. Default is
     * false
     */
    ...
    /**
     * If a phase is a window, it is opened in a new EngineWindow. Default is false.
     */
    ...
    /**
     * Common gamephase param is to have param name in form of name.phase = phase
     * name. Return button name usable by DDButton if that exists, otherwise just
    ...
    public String toString() {
        return "Phase " + sName_ + " cached: " + isCached() + " history: " + isHistory() + " params: "
                + super.toString();
    }
}
