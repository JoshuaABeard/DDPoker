...
public class ActionItem extends DMTypedHashMap {
    ...
    /**
     * Add all players to the list, unless they are eliminated
     */
    public void addNonEliminatedPlayers(GameInfo game) {
        int nNumPlayers = game.getNumPlayers();
        for (int i = 0; i < nNumPlayers; i++) {
            if (!game.isEliminated(i))
                addPlayer(i);
        }
    }
    ...
    /**
     * Removes all players and sets the given id as the only player in affected by
     * this action
     */
    ...
    /**
     * Return true if this player needs to respond to this action.
     */
    ...
    /**
     * Return true if the player has acted on this item, false otherwise. If player
     * was not required to do anything (i.e., isPlayerActionRequired() returned
     * false, then this returns true)
     */
    public boolean hasPlayerActed(int id) {
        Long l = getLong(getString(id));
        if (l == null) {
            throw new ApplicationError(ErrorCodes.ERROR_CODE_ERROR,
                    "Asking if player acted, but player not part of this action: " + id + "(action " + getActionID()
                            + ")",
                    null);
        }
    ...
    /**
     * Set this player as having acted. Return false if invalid to do so (either
     * because player wasn't part of the action, or if player already acted)
     */
    public boolean setPlayerActed(int id) {
        ...
        if (l == null) {
            ...
            logger.warn("Setting player acted, but player not part of this action: " + id + "(action " + getActionID()
                    + ")");
            return false;
        }
        ...
        if (!l.equals(INIT_LONG)) {
            // BUG 199 - don't throw exception. Log error and return false.
            logger.warn("Setting player acted, but player already acted " + id + "(action " + getActionID() + ")");
            return false;
        }
