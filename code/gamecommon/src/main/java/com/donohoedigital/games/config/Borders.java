...
public class Borders extends TreeSet<Border> {
    ...
    public Borders(Territories territories, MapPoints points, Element root, Namespace ns, String module) {
        ...
        Integer nNum = XMLConfigFileLoader.getChildIntegerValue(root, NUMBORDERFILES, ns, false,
                GameboardConfig.GAMEBOARD_CONFIG, 0);
        XMLConfigFileLoader loader = new XMLConfigFileLoader();

        for (int j = 0; j < nNum; j++) {
            URL url = new MatchingResources("classpath*:config/" + module + "/" + BORDERDIR_PREFIX + j + ".xml")
    ...
    /**
     * Add a border to this. Returns t if a new border. If border exists, returns
     * existing border (and updates bEnclosed). When a border is added, the
     * territories that make up the border are notified of the new border.
     */
    ...
    /**
     * Remove given border from this When a border is removed, the territories that
     * make up the border are notified of the old border.
     */
    ...
    public void printXML(XMLWriter writer, File fConfigDir, int nIndent) {
        ...
        while (iter.hasNext()) {
            ...
            if (border.getBorderPoints().size() > 0) {
                // sName = border.getTerritory1().getName() + "." +
                // border.getTerritory2().getName() + ".xml";
