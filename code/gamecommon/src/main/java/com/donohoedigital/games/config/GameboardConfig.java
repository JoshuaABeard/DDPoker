...
public class GameboardConfig extends XMLConfigFileLoader {
    ...
    private void init(String module) throws ApplicationError {
        ...
        URL url = new MatchingResources("classpath*:config/" + module + "/" + GAMEBOARD_CONFIG)
    ...
    private void init(Document doc, String module) throws ApplicationError {
        ...
        // storage for all points in territories and borders (no duplicates
        // allowed/needed)
    ...
    /**
     * Create adjacent territories and other info - must call AFTER createPaths. If
     * bClearBorders is true, then all border information is cleared to free memory.
     */
    ...
    /**
     * Create territory paths. If bClearPoints true, then remove all points to free
     * memory (save is disabled after this is called). Should be called after
     * transformAllPoints is done
     */
    ...
    /**
     * translate all points so that the top-left most becomes (0,0). Adjust height
     * and width to refect distance between top-left most and bottom-right most
     * points. In other words, eliminate any gap between borders and the edge of the
     * original image. Returns rectangle that represents x,y,width,height of edge of
     * original image (in original image coordinate space)
     */
    public Rectangle transformAllPoints() {
        ...
        double dScale = dScale_;
        transformedSize_ = new Rectangle((int) (minx / dScale), (int) (miny / dScale), (int) (width / dScale),
                (int) (height / dScale));
    ...
    /**
     * Adjust paths, points and sizes so that the given Territory is at the upper
     * left (used when multiple map views used). Most be called after
     * transformAllPoints() and createPaths()
     */
    ...
    /**
     * Adjust paths by the given transform and the territory points by the given
     * adjx
     */
    ...
    /**
     * Return rectangle representing the area of the map on which territory borders
     * are drawn - returned in same coordinate space as map is defined. Note: does
     * not account for changes made by adjustPaths (used typically by tools like
     * TerritoryPointManager)
     */
