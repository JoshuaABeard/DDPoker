...
public class GamePieceContainerImpl implements GamePieceContainer {
    ...
    /**
     * Add a piece into the container. If the gamepiece already exists in the
     * container, its contents are moved using moveAllTokensTo(). Returns gp, or if
     * already exists, the GamePiece that exists (with modified counts).
     */
    public GamePiece addGamePiece(GamePiece gp) {
        synchronized (pieces_) {
            ApplicationError.assertTrue(gp.getContainer() == null,
                    "Adding GamePiece that already belongs to a container");
    ...
    /**
     * Get all game pieces - should synchronize on getMap() around call to this and
     * use of iterator to avoid concurrent modification exception
     */
    ...
    /**
     * Return gamepiece of given type. If owner is provided (not null), then the
     * GamePiece's owner must match the given owner to be returned. Returns null if
    ...
    /**
     * Return true if this container has a piece of the given type whose owner !=
     * the given owner
     */
    public boolean hasNonOwnerGamePiece(int nType, GamePlayer owner) {
        ...
        synchronized (pieces_) {
            ...
            while (iter.hasNext()) {
                piece = (GamePiece) iter.next();
                if (piece.getType().intValue() == nType && piece.getGamePlayer() != owner) {
                    return true;
                }
            }
        }
    ...
    /**
     * Return true if this territory has pieces that have moved (pieces with a Token
     * with history
     */
