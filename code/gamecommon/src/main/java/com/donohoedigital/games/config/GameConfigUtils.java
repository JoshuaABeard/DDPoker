...
public class GameConfigUtils {
    ...
    /**
     * Get the location for save files, creating the directory if not there. the
     * returned value is cached in the ConfigManager, so there is only one per
     * application (useful for locking)
     */
     ...
     * are in sorted order, with last one being highest number. BUG 32 - loops from
     * last till first to handle misnamed files
     */
    ...
    /**
     * Get file number from the given file, returns -1 if no valid number. File name
     * expected to be <leading>.<num>.<ext>
     */
