...
public class TerritoryPoint extends MapPoint {
     ...
     */
    public TerritoryPoint(Element point, Namespace ns, String sAttrErrorDesc) throws ApplicationError {
     ...
     * from the territory to determine what should become active.
     */
        ...
        switch (nType) {
            case POINT_NEXT :
                if (nIndex == (nSize - 1))
                    ...
                    return myTerritory_.getTerritoryPoint(nIndex + 1);

            case POINT_PREV :
                if (nIndex == 0)
                    return myTerritory_.getTerritoryPoint(nSize - 1);
                else
                    return myTerritory_.getTerritoryPoint(nIndex - 1);

            case POINT_DELETE :
                if (nIndex == 0)
                    return myTerritory_.getTerritoryPoint(1);
                else
                    return myTerritory_.getTerritoryPoint(nIndex - 1);
