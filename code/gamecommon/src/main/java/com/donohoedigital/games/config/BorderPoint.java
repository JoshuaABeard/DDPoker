...
public class BorderPoint extends MapPoint {
     ...
     */
    public BorderPoint(Element point, Namespace ns, String sAttrErrorDesc) throws ApplicationError {
    ...
    /**
     * adds a border that this point belongs to (called from Border)
     */
    ...
    /**
     * removes a border that this point belongs to (called from Border)
     */
    void removeBorder(Border border) {
        myBorders_.removeBorder(border);
        if (this.bCurrentBorder_ == border)
    ...
    /**
     * Shift this point in the border array up (+) / down(-). Wraps around if
     * reaches end/beginning
     */
    ...
    public void setCurrentBorder(Border border) {
        ...
        bCurrentBorder_ = null;
        logger.warn(
                "setCurrentBorder to border: " + border.shortDesc() + " - is not a border in point " + longDesc(null));
    }
    ...
    public Border nextBorder() {
        for (int i = 0; i < myBorders_.size(); i++) {
            if (myBorders_.getBorder(i) == bCurrentBorder_) {
                int nIndex = i + 1;
                if (nIndex >= myBorders_.size())
                    nIndex = 0;
                return myBorders_.getBorder(nIndex);
            }
        }
        logger.warn("nextBorder() not found for " + bCurrentBorder_.shortDesc() + " in point " + longDesc(null));
        return null;
    }

    /**
     * Return nearest point to this one. Used by GUIs about to remove this point
     * from the border to determine what should become active.
     */
        ...
        switch (nType) {
            case POINT_NEXT :
                if (nIndex == (nSize - 1))
                    ...
                    return border.getBorderPoint(nIndex + 1);

            case POINT_PREV :
                if (nIndex == 0)
                    return border.getBorderPoint(nSize - 1);
                else
                    return border.getBorderPoint(nIndex - 1);

            case POINT_DELETE :
                if (nIndex == 0)
                    return border.getBorderPoint(1);
                else
                    return border.getBorderPoint(nIndex - 1);
