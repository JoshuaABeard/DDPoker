...
public class Border {
     ...
     */
    public Border(String s1, String s2, boolean bEnclosed) throws ApplicationError {
    ...
    public Border(Element border, Namespace ns, String sAttrErrorDesc) {
        ...
        bEnclosed = XMLConfigFileLoader.getBooleanAttributeValue(border, "enclosed", false, sAttrErrorDesc,
                Boolean.FALSE);
        ...
        // points in border
        List<Element> points = XMLConfigFileLoader.getChildren(border, "point", ns, true,
                GameboardConfig.GAMEBOARD_CONFIG);
        Element point;
        int nNumBorderPoints = points.size();
        BorderPoint newPoint;
        for (int j = 0; j < nNumBorderPoints; j++) {
            sAttrErrorDesc = "BorderPoint #" + (j + 1) + " of border (" + sT1 + "," + sT2 + ") in "
                    + GameboardConfig.GAMEBOARD_CONFIG;
            point = points.get(j);
    ...
    /**
     * Return whether border is a wrap around border (left side of board touches
     * right)
     */
    ...
    /**
     * return true if the passed in object equals this object. Border equality is
     * determined if both contain the same two territories (order is not important)
     */
    public boolean equals(Object obj) {
        ...
        if (obj instanceof Border) {
            ...
            // and num is same
            if (b.getNumber() == getNumber() && b.t1_ == t1_ && b.t2_ == t2_)
                return true;
        }
    ...
    public void printXML(XMLWriter writer, int nIndent) {
        ...
        if (isEnclosed())
            writer.printAttribute("enclosed", isEnclosed() ? Boolean.TRUE : Boolean.FALSE);
        if (isWrapAround())
            writer.printAttribute("wrap", isWrapAround() ? Boolean.TRUE : Boolean.FALSE);
        // default territory num is 1, so don't write unless greater than 1
        if (getNumber() > 1)
            writer.printAttribute("num", nNum_);
    ...
    public String shortDesc() {
        ...
        if (getNumber() > 1)
            sNum = " (#" + getNumber() + ")";
        return "[" + getTerritory1().getName() + " - " + getTerritory2().getName() + sNum
                + (isWrapAround() ? " (wrap)" : "") + (isEnclosed() ? " (enclosed)" : "") + "]";
    }
