...
/**
 * Reusable computation of odds of winning (Effective Hand Strength) with a
 * given pocket hand, board, and cards to come.
 *
 * Computations are based on one-card lookahead (two cards is expensive and not
...
public class PocketOdds {
    ...
    /**
     * PocketScores is a wrapper on PocketMatrixShort, and stores a win/tie
     * probability for a pocket hand, with a given board. Instances are returned
     * only by PocketOdds.getInstance() so that they can be cached (constructor is
     * private). The cache is maintained as long as every call is for the same board
     * cards. This means that while a hand is in play, at most one instance is
     * cached for each combination of the 49 cards not in the flop, or 1176 1128
     * 1081instances.
     *
     ...
     * @param pocket
     *            The pocket cards to compute odds for.
    ...
    public static PocketOdds getInstance(Hand community, Hand pocket) {
        if (community == null) {
            throw new IllegalArgumentException("PocketOdds.getInstance() called with null community hand.");
        }

        if (community.size() < 3) {
            throw new IllegalArgumentException("PocketOdds.getInstance() called before the flop.");
        }

        if (community.size() == 5) {
            throw new IllegalArgumentException("PocketOdds.getInstance() called after the river.");
        }

        if (pocket == null) {
            throw new IllegalArgumentException("PocketOdds.getInstance() called with null pocket hand.");
        }

        if (pocket.size() < 2) {
            throw new IllegalArgumentException("PocketOdds.getInstance() called with empty pocket hand.");
        }
        ...
        if (odds == null) {
            ...
            // System.out.println("PocketOdds constructed in " + (after-before) + "
            // milliseconds.");
            cache_.put(key, odds);
        }
    ...
    private PocketOdds(Hand community, Hand pocket) {
        ...
        long divisor = (long) (50 - index) * (49 - index) / 2;
        ...
        for (int k = 0; k < 52; ++k) {
            ...
            for (int i = 1; i < 52; ++i) {
                if ((i == k) || community.containsCard(i) || pocket.containsCard(i))
                    continue;

                for (int j = 0; j < i; ++j) {
                    if ((j == k) || community.containsCard(j) || pocket.containsCard(j))
                        continue;
    ...
    /**
     * @return The percentage chance (0.0 - 1.0) of beating or tiying a random
     *         opposing hand after next card.
     */
     ...
     * @param hand
     *            Pocket cards.
     * @return The percentage chance (0.0 - 1.0) of beating or tiying the specified
     *         opposing hand after next card.
     */
