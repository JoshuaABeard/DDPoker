...
public class HandFutures {
    ...
    public boolean hasStraightDraw() {
        // these percentages are rough estimates (based on test runs in main())
        return (getOddsImproveTo(HandInfo.STRAIGHT)
                + getOddsImproveTo(HandInfo.STRAIGHT_FLUSH)) > (MORE == 2 ? 25 : 15);
    }
    ...
    /**
     * Return odds (0-100) of improving to given type of hand. We use figures from
     * hands which involve the hole cards (e.g., avoiding board-only trips,
    ...
    /**
     * Return odds (0-100) of improving to a better type of hand than we started
     * with.
     */
    ...
    /**
     * Calculate strength of hand against N opponents. Return is float from 0 to 1
     * indicating probability of this hand being the best hand
     */
    private void calcFutures(HandInfoFaster FAST, Hand hole, Hand community, int nMinHandType) {
        ...
        for (int next1 = 0; next1 < nEND; next1++) {
            ...
            for (int next2 = next1 + 1; MORE == 1 || next2 < nSize; next2++) {
                ...
                if (newscore > ourscore) {
                    nImprovements_++;
                    // logger.debug("IMPROVE: " + hole + " " + commcopy + ": " +
                    // HandInfo.getHandTypeDesc(nType) + " " + newscore + " - " +
                    // HandInfo.isOurHandInvolved(hole, newscore, suit, ourtype !=
                    // HandInfo.HIGH_CARD) + " ourscore: " + ourscore);
                }
                ...
                if (nType > ourtype && nType > nMinHandType) {
                    ...
                    // logger.debug("HAND: " + hole + " " + commcopy + ": " +
                    // HandInfo.getHandTypeDesc(nType) + " " + newscore + " - " +
                    // HandInfo.isOurHandInvolved(hole, newscore, suit, ourtype !=
                    // HandInfo.HIGH_CARD));
                }
        ...
        if (DEBUG) {
            ...
            for (int i = HandInfo.HIGH_CARD; i <= HandInfo.ROYAL_FLUSH; i++) {
                logger.debug("   " + fName.form(HandInfo.getHandTypeDesc(i)) + ": " + HandStat.fCnt.form(nTotals_[i])
                        + " (" + HandStat.fPerc.form(100.0d * ((float) nTotals_[i]) / nTotal_) + "%)" + " -- "
                        + HandStat.fCnt.form(nTotals2_[i]) + " (" + HandStat.fPerc.form(getOddsImproveTo(i)) + "%)");
                sum += nTotals_[i];
            }
            logger.debug("SUM: " + sum);
            logger.debug("Improve:       " + nImprovements_ + " ("
                    + HandStat.fPerc.form(100.0f * ((float) nImprovements_) / nTotal_) + "%)");
            logger.debug("Type Improve:  " + nTypeImprovements_ + " ("
                    + HandStat.fPerc.form(100.0f * ((float) nTypeImprovements_) / nTotal_) + "%)");
            logger.debug("Type Improve2: " + nTypeImprovements2_ + " ("
                    + HandStat.fPerc.form(100.0f * ((float) nTypeImprovements2_) / nTotal_) + "%)");
