...
public class DisplayTableMoves extends ChainPhase {
    ...
    public void process() {
        ...
        for (PokerPlayer player : moved) {
            sb.append(PropertyConfig.getMessage("msg.moved", Utils.encodeHTML(player.getName()), player.getChipCount(),
                    player.getSeat() + 1));
        }
        ...
        if (!TESTING(PokerConstants.TESTING_AUTOPILOT)) {
            String sMsg = PropertyConfig.getMessage("msg.dialog.moved",
                    moved.size() == 1
                            ? PropertyConfig.getMessage("msg.player.singular", nSIZE)
                            : PropertyConfig.getMessage("msg.player.plural", nSIZE),
                    sb.toString());
            EngineUtils.displayInformationDialog(context_, Utils.fixHtmlTextFor15(sMsg), "msg.windowtitle.moved",
                    "playermoved", "noplayermove");
        }
    }
