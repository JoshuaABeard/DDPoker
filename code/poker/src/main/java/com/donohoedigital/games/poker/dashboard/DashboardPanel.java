...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 16, 2005 Time: 12:06:34 PM
...
public class DashboardPanel extends DDPanel {
    ...
    public DashboardPanel(DashboardManager mgr) {
        ...
        sp_ = new DDScrollPane(dashitems_, "ChatInGame", null, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
    ...
    private void sync() {
        ...
        for (int i = 0; i < mgr_.getNumItems(); i++) {
            ...
            if (item.isInDashboard()) {
                ...
                ec = new ExplicitConstraints(header, MathEF.constant(0), y, width, ComponentEF.preferredHeight(header));
                ...
                ec = new ExplicitConstraints(body, bodyX, ComponentEF.bottom(header),
                        MathEF.max(width.subtract(bodyX), ComponentEF.preferredWidth(body)), bH, 0.0, 0.0, true, true);

                /*
                 * ec = new ExplicitConstraints(body, bodyX, ComponentEF.bottom(header),
                 * MathEF.max( width.subtract(bodyX), ComponentEF.preferredWidth(body)),
    ...
    private JComponent createTopPanel() {
        ...
        dashBG.add(edit, new ExplicitConstraints(edit,
                ContainerEF.right(dashBG).subtract(ComponentEF.preferredWidth(edit)), ContainerEF.top(dashBG).add(5)));
