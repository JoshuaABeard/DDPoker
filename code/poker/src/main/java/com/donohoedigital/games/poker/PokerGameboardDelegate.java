...
public interface PokerGameboardDelegate {
    /**
     * Notify delegate that the board is receiving a requestFocus event. Request
     * focus on a sub-widget if desired and return true if done so. Return false if
     * no focus requested so Gameboard can do so.
     */
