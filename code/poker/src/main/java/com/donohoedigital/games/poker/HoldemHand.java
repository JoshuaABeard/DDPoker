...
public class HoldemHand implements DataMarshal, GameHand {
    ...
    // set to -1 to force lazy load due to player seats not being set prior to
    // demarshal
    ...
    private void doBlinds() {
        ...
        if (nSmallBlind_ > 0)
            table_.getPlayerRequired(small).smallblind(nSmallBlind_);
        if (nBigBlind_ > 0)
            table_.getPlayerRequired(big).bigblind(nBigBlind_);
     ...
     * hand had taken place. It is used for all-computer tables where actually
     * playing a hand takes too much time. NOTE: This method is not thread safe. It
     * is not synchronized to avoid the performance hit
     */
    public static void simulateHand(PokerGame game, PokerTable table) {
        ...
        for (PokerPlayer pokerPlayer : sORDER) {
            ...
            p.newSimulatedHand();
            if (nAnte > 0)
                nTotalPot += p.addSimulatedBet(nAnte);
        }
        ...
        if (nSmallBlind > 0)
            nTotalPot += table.getPlayerRequired(small).addSimulatedBet(nSmallBlind);
        if (nBigBlind > 0)
            nTotalPot += table.getPlayerRequired(big).addSimulatedBet(nBigBlind);
        ...
        while (!bDone) {
            ...
            if (DebugConfig.isTestingOn()) {
                ...
                ApplicationError.assertTrue(nCheck == nTotalPot,
                        "Pot mismatch, actual=" + nCheck + ", running total=" + nTotalPot);
            }
        ...
        // that went all-in with $1 and won $5000, but this isn't meant to be fair, just
        // a way
    ...
    /**
     * Get player at in active player list. If NO_CURRENT_PLAYER is passed as the
    ...
    /**
     * Get community to display to user - takes into account whether we are in
     * all-in showdown, and if so, returns community as user is seeing it.
     */
    ...
    /**
     * return current pot (that chips go into - needed because of side pots
     */
    ...
    /**
     * If there is no more betting to be done because all but one of the players is
     * all-in, then we are in an all-in showdown situation.
     */
     ...
     * when players act. Could be set from outside for simulations, and backwards
     * compat for 1.0
     */
    public void setCurrentPlayerIndex(int n) {
        ...
        if (newPlayer != null)
            newPlayer.setCurrentGamePlayer(true);

        if (oldPlayer != newPlayer) {
            table_.firePokerTableEvent(new PokerTableEvent(PokerTableEvent.TYPE_CURRENT_PLAYER_CHANGED, table_, nOld,
                    nCurrentPlayerIndex_));
        }
    }
    ...
    public int getLastActionAI(PokerPlayer player, int nRound) {
        ...
        synchronized (history_) {
            for (int i = history_.size() - 1; i >= 0; i--) {
                hist = history_.get(i);
                if (hist.getPlayer() == player && hist.getRound() == nRound) {
    ...
    public int getLastActionThisRound(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (int i = history_.size() - 1; i >= 0; i--) {
                hist = history_.get(i);
                if (hist.getPlayer() == player && hist.getRound() == nRound_) {
                    return hist.getAction();
                }
            }
        }
    ...
    /**
     * Get last betting action by any player in any round (any action other than
     * OVERBET or WIN)
     */
    public HandAction getLastAction() {
        ...
        synchronized (history_) {
            for (int i = history_.size() - 1; i >= 0; i--) {
                hist = history_.get(i);
                if (hist.getAction() != HandAction.ACTION_OVERBET && hist.getAction() != HandAction.ACTION_WIN
    ...
    private void addToPot(PokerPlayer player, int nChips, int nCall, int nAction, String sDebug) {
        ...
        if (nChips % nMinChip != 0) {
            ...
            ApplicationError.fail(
                    "Adding uneven chips " + nChips + " debug: " + sDebug + " action: " + nAction + " pots: " + pots_);
        }
    ...
    public int getMinChip() {
        if (table_ == null)
            ...
            return table_.getMinChip();
    }

    /**
     * Figure out how many pots we have and allocate to side pots as necessary
     */
    private void calcPots() {
        ...
        if (TESTING(PokerConstants.TESTING_DEBUG_POT))
            logger.debug("PotInfo: {}", info);
        ...
        synchronized (pots_) {
            ...
            for (int i = 0; i < info.size(); i++) {
                ...
                if (potinfo.needSide()
                        && ((i == 0 && pot.hasBaseAllIn() && potinfo.nBet == 0) || potinfo.nBet > nLastSideBet)) {
    ...
    /**
     * Return the main pot for the current round, eliminating all side pots and
     * resetting main one.
     */
    ...
    /**
     * Return current bet on the table for current round (i.e., the highest bet by
     * any player)
     */
    public int getBet() {
    ...
    public int getBet(PokerPlayer player, int nRound) {
        ...
        synchronized (history_) {
            ...
            for (HandAction handAction : history_) {
                hist = handAction;
                nAction = hist.getAction();
                if (hist.getRound() == nRound && hist.getPlayer() == player
                        && (nAction == HandAction.ACTION_BET || nAction == HandAction.ACTION_RAISE
                                || nAction == HandAction.ACTION_CALL || nAction == HandAction.ACTION_BLIND_SM
    ...
    /**
     * Return biggest bet or raise this hand - smallest value to be returned is big
     * blind
     */
    public int getBiggestBetRaise() {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getAction() == HandAction.ACTION_BET || hist.getAction() == HandAction.ACTION_RAISE) {
                    if (hist.getAmount() > nBet)
                        nBet = hist.getAdjustedAmount();
                }
            }
        }
    ...
    public int getTotalBet(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if ((hist.getAction() == HandAction.ACTION_BET || hist.getAction() == HandAction.ACTION_RAISE
                        || hist.getAction() == HandAction.ACTION_CALL || hist.getAction() == HandAction.ACTION_BLIND_SM
                        || hist.getAction() == HandAction.ACTION_BLIND_BIG
                        || hist.getAction() == HandAction.ACTION_ANTE) && hist.getPlayer() == player) {
                    nBet += hist.getAmount();
                }
            }
        }
    ...
    public PokerPlayer getBettor() {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getAction() == HandAction.ACTION_BET && hist.getRound() == nRound_) {
    ...
    public PokerPlayer getRaiser() {
        ...
        synchronized (history_) {
            for (int i = history_.size() - 1; i >= 0; i--) {
                hist = history_.get(i);
                if ((hist.getAction() == HandAction.ACTION_RAISE) && hist.getRound() == nRound_) {
                    return hist.getPlayer();
                }
            }
        }
    ...
    /**
     * return player who initiated betting/raising. This is the first player to bet
    ...
    public int getNumCallers() {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if ((hist.getAction() == HandAction.ACTION_CALL) && hist.getRound() == nRound_) {
                    nCalls++;
                }
            }
        }
    ...
    public int getAnte(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getAction() == HandAction.ACTION_ANTE && hist.getPlayer() == player
    ...
    public int getAnteSmallBlind(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if ((hist.getAction() == HandAction.ACTION_ANTE || hist.getAction() == HandAction.ACTION_BLIND_SM)
    ...
    private int getMaxChipsOtherPlayers(PokerPlayer current, boolean bAddCurrentBet, boolean bSubtractCurrentCall) {
        ...
        for (int i = 0; i < nNum; i++) {
            ...
            if (bAddCurrentBet)
                nOtherLeft += getBet(other);
            if (bSubtractCurrentCall)
                nOtherLeft -= getCall(other);
    ...
    public int getCall(PokerPlayer player) {
        ...
        if (nCall < 0)
            ApplicationError.fail("Negative call " + nCall, player);
        return nCall;
    }
    ...
    public int getMinRaise() {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getRound() == nRound_
                        && (hist.getAction() == HandAction.ACTION_BET || hist.getAction() == HandAction.ACTION_RAISE)) {
    ...
    public boolean isDone() {
        ...
        if (nPlayersActed == 0 && getNumWithChips() <= 1)
            return true;

        // all players must have acted (or be all in)
        if (nPlayersToAct != (nPlayersActed + nAllIn))
            return false;
    ...
    /**
     * Is pot good? Meaning have all players have matched the current bet on the
    ...
    /**
     * Get number of players that have chips left to bet (and have cards in this
     * hand)
     */
    ...
    public boolean hasPlayerActed(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getPlayer() == player && hist.getRound() == nRound_
                        && hist.getAction() != HandAction.ACTION_ANTE && hist.getAction() != HandAction.ACTION_BLIND_SM
                        && hist.getAction() != HandAction.ACTION_BLIND_BIG && hist.getAction() != HandAction.ACTION_WIN
    ...
    public int getWin(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getAction() == HandAction.ACTION_WIN && hist.getPlayer() == player) {
                    nWin += hist.getAmount();
                }
            }
        }
    ...
    public int getOverbet(PokerPlayer player) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                hist = handAction;
                if (hist.getAction() == HandAction.ACTION_OVERBET && hist.getPlayer() == player) {
                    nWin += hist.getAmount();
                }
            }
        }
    ...
    public HandAction getPotResult(PokerPlayer player, int nPot) {
        ...
        synchronized (history_) {
            for (int i = history_.size() - 1; i >= 0; i--) {
                hist = history_.get(i);
                if ((hist.getAction() == HandAction.ACTION_OVERBET || hist.getAction() == HandAction.ACTION_WIN
                        || hist.getAction() == HandAction.ACTION_LOSE) && hist.getPlayer() == player
    ...
    private void addHistory(HandAction action) {
        ...
        // ai debugging
        if ((TESTING(EngineConstants.TESTING_AI_DEBUG) || TESTING(PokerConstants.TESTING_DEBUG_POT))
                && player.getTable().isCurrent()) {
            debugPrint(action);
        }
        ...
        switch (nAction) {
            ...
            case HandAction.ACTION_RAISE :
                calcPots();
                if (TESTING(PokerConstants.TESTING_DEBUG_POT))
                    logger.debug("POTS AT END: {}", pots_);
                verifyPot();
                break;
        }
    ...
    private void recordHandInfo(PokerPlayer player, int nLeft) {
        ...
        // if no record for showdown and player hasn't folded, then they lost, so record
        // that
        // we saw the river
        if (!isFolded(player)) {
    ...
    /**
     * Returns true if the player put chips in the pot voluntarily pre-flop (even if
     * subsequently raised out).
     * <p/>
    ...
    /**
     * Returns Boolean.TRUE if the player took the opportunity to open the pot in
     * the given round. Returns Boolean.FALSE if the player passed up the
    ...
    public Boolean betPot(PokerPlayer player, int round) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                ...
                if (hist.getPlayer() != player)
                    continue;
                if (hist.getAction() == HandAction.ACTION_CHECK)
    ...
    /**
     * Returns Boolean.TRUE if the player took the opportunity to raise the pot in
     * the given round. Returns Boolean.FALSE if the player passed up the
    ...
    public boolean getWasPotAction(int round) {
        ...
        synchronized (history_) {
            for (HandAction handAction : history_) {
                ...
                if ((hist.getRound() == round) && ((hist.getAction() == HandAction.ACTION_BET)
