...
public class OnlineManager implements ChatManager {
    ...
    public synchronized void connectionClosing(PokerConnection connection) {
        ...
        if (player != null) {
            ...
            if (game_.getOnlineMode() != PokerGame.MODE_CANCELLED) {
                logger.info("Connection closed for " + player.getName() + " [id #" + player.getID() + "] "
                        + (player.isHost() ? " (host)" : "") + (player.isObserver() ? " (observer)" : "") + ": "
                        + connection);
            }
    ...
    public synchronized DDMessageTransporter handleMessage(DDMessageTransporter msg, PokerConnection channel) {
        ...
        try {
            ...
            if (TESTING(EngineConstants.TESTING_P2P))
                logger.debug("** ERROR REPLY **: " + error.getReply());
     ...
     * <p/>
     * 1) An error occurs - we respond back with a direct message by throwing an
     * OnlineError
     * <p/>
     * 2) A direct response is desired, like for the CAT_TEST, in which case that is
     * returned directly
     * <p/>
     * 3) No resonse is required. In some cases, the message is just processed and
     * in other cases a seperate message is sent asynhcronously back to the sender
     */
    private DDMessageTransporter processMessage(OnlineMessage omsg) {
        ...
        switch (omsg.getCategory()) {
            ...
            case OnlineMessage.CAT_ALIVE :
                // do nothing specific for alive messages (above setMessageReceived() call is
                // all we need)
                break;
            ...
            default :
                throw new OnlineError(getAppErrorReply(p2p_, omsg,
                        PropertyConfig.getMessage("msg.p2p.unhandled", omsg.getCategory()), false));
        }
    ...
    private void validate(OnlineMessage omsg) {
        ...
        if (omsg.getCategory() == OnlineMessage.CAT_TEST || omsg.getCategory() == DDMessage.CAT_APPL_ERROR
                || omsg.getCategory() == DDMessage.CAT_ERROR)
            return;
        ...
        if (!gpass.equals(mpass) || !gid.equals(mid)) {
            String data = "Validation failed gid=" + gid + " gpass=" + gpass + " msg-gid=" + mid + " msg-gpass="
                    + mpass;
            logger.info(data + " omsg: " + omsg.toStringNoData());
            DDMessageTransporter reply = getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.p2p.validate"),
                    false);
    ...
    /**
     * Join game - host side processing. Checks for dups, makes sure game is
     * accepting registrations. If valid, creates player, sends game data to new
     * player and update to other players
     */
    private void processJoin(OnlineMessage omsg) {
        ...
        if (msgVersion.isBefore(PokerConstants.VERSION_LAST_COMPAT)) {
            throw new OnlineError(getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.nojoin.version",
                    msgVersion.toString(), PokerConstants.VERSION_LAST_COMPAT.toString()), false));
        }
        ...
        switch (nMode) {
            case PokerGame.MODE_INIT :
                throw new OnlineError(
                        getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.nojoin.init"), false));
            ...
            default :
                throw new OnlineError(
                        getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.nojoin.nomore"), false));
        }
        ...
        if (player != null) {
            ...
            if (player.isHost()) {
                throw new OnlineError(getAppErrorReply(p2p_, omsg,
                        PropertyConfig.getMessage("msg.nojoin.dup", player.getName()), false));
            }

            // reject dup player if current socket isn't null or player is the host (has no
            // socket)
            if (player.getConnection() != null) {
                boolean bSameConnect = player.getConnection().equals(conn);

                // if attempting to join in-game with same key, the player was probably
                // disconnected and
                // is rejoining before host has realized connection is dead. In this case,
                // instead of making
                // things harder for the rare case of someone joining in with a key, we kill the
                // old socket
                // and let this one takes it place.
                // Patch 8 - added isReconnect check which also does forced reconnect when that
                // flag is
                // set (from HostStatus)
                ...
                if (nMode == PokerGame.MODE_PLAY || omsg.isReconnect()
                    ...
                    //
                    // UDP: added equals check, which will allow existing UDP connection to continue
                    // to be re-used.
                    // TCP: use of equals check doesn't change functionality since sockets aren't
                    // re-used
                    if (!bSameConnect) {
                        logger.info(
                                "Player rejoining: " + player.getName() + ", closing existing, likely bad, connection");
            ...
            if (player.isObserver()) {
                ...
                if (game_.isGameOver() && player.getTable() != null) {
                    // player was at final table when tournament ended and therefore not turned into
                    // an
            ...
            //
            // otherwise if player is still in tournament and is attempting to rejoin as
            // observer,
        ...
        // new player coming in
            ...
            boolean bAllowObsSpecial = sPlayerName.equals("ddpoker") || sPlayerName.equals("DDPoker Support")
                    || sPlayerName.equals("Greg King") || sPlayerName.equals("Doug Donohoe")
                    || sPlayerName.equals("Arcade GameOver") || sPlayerName.equals("John14");

            // if player is demo, check if profile allows demo players
            if (omsg.isPlayerDemo() && !true) {
                throw new OnlineError(
                        getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.nojoin.demo"), false));
            }
            ...
            if ((banned_.containsPlayer(sPlayerName) || banned_.containsKey(omsg.getKey())) && !bAllowObsSpecial) {
                ...
                throw new OnlineError(
                        getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.nojoin.banned"), false));
            }

            // check invite only (players)
            if (!bObs && profile.isInviteOnly() && !profile.getInvitees().containsPlayer(sPlayerName)
                    && !bAllowObsSpecial) {
                ...
                throw new OnlineError(getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage(
                        profile.isInviteObserversPublic() ? "msg.nojoin.inviteonly.obs" : "msg.nojoin.inviteonly"),
                        false));
            }
            ...
            if (bObs && profile.isInviteOnly() && !profile.isInviteObserversPublic()
                ...
                sendRejectMessage(sPlayerName, "msg.chat.uninvitedrejected");
                throw new OnlineError(
                        getAppErrorReply(p2p_, omsg, PropertyConfig.getMessage("msg.nojoin.inviteonly"), false));
            }
            ...
            if (omsg.isOnlineActivated()) {
                ...
                for (int i = game_.getNumPlayers() - 1; i >= 0; i--) {
                    ...
                    if (exist.isOnlineActivated() && exist.getName().equals(sPlayerName)) {
                        throw new OnlineError(getAppErrorReply(p2p_, omsg,
                                PropertyConfig.getMessage("msg.nojoin.duponline", Utils.encodeHTML(exist.getName())),
                                false));
                    }
                }
                ...
                if (profile.isOnlineActivatedPlayersOnly()) {
                    ...
                    throw new OnlineError(getAppErrorReply(p2p_, omsg,
                            PropertyConfig.getMessage("msg.nojoin.onlineactivatedonly"), false));
                }
            }
            ...
            if (!bObs) {
                ...
                if (!isSpaceForPlayer()) {
                    throw new OnlineError(getAppErrorReply(p2p_, omsg,
                            PropertyConfig.getMessage("msg.nojoin.nomore", profile.getMaxOnlinePlayers()), false));
                }
            ...
            // observer
                ...
                if (!isSpaceForObserver() && !bAllowObsSpecial) {
                    throw new OnlineError(getAppErrorReply(p2p_, omsg,
                            PropertyConfig.getMessage("msg.nojoin.nomore.obs", profile.getMaxObservers()), false));
                }
        ...
        player.setProfilePath(omsg.getPlayerProfilePath()); // store in host copy so when we send back to player, it is
                                                            // correct
        ...
        if (nMode == PokerGame.MODE_PLAY) {
            // Need to specify table for observer since normal start logic (HostStart)
            // usually
            // assigns tables. This is a rejoin, so need to specify the table for observers.
            // Also do SAFETY check if player already has table and not host, reset them to
            // host table.
            ...
            // in game (td set) send game data, tables, hide cards and set current table to
            // this player
            if (td_ != null) {
            ...
            if (bObs) {
                ...
                sendDealerChat(PokerConstants.CHAT_1,
                        PropertyConfig.getMessage(rejoin ? "msg.chat.observerrejoined" : "msg.chat.observerjoined",
    ...
    /**
     * Only post a reject message per messageKey once about a particular player to
     * prevent griefing
     */
    ...
    public synchronized void switchPlayer(PokerPlayer player) {
        // Note: no need to change id since player/observer ids are in different
        // 'namespaces'
        if (player.isObserver()) {
        ...
        sendDirectorChat(
                PropertyConfig.getMessage(player.isObserver() ? "msg.chat.switchtoobserver" : "msg.chat.switchtoplayer",
                        Utils.encodeHTML(player.getName())),
                null);
     ...
     */
    private void sendClientJoinMessage(PokerPlayer player, String sPhase, OnlineMessage omsg,
    ...
    /**
     * Called after client does the PHASE_CLIENT_INIT and creates the
     * TournamentDirector
     */
    private void processReady(OnlineMessage omsg) {
        ...
        // during rejoin
        if (td_ != null)
            td_.notifyPlayerRejoinDone(getPlayer(omsg, true));
    }
        ...
        if (isHost()) {
            ...
            sendDealerChat(PokerConstants.CHAT_1,
                    PropertyConfig.getMessage(
                            omsg.isPlayerConnected() ? "msg.chat.playerrejoined" : "msg.chat.disconnected",
                            Utils.encodeHTML(player.getName())),
                    player.getTable());
        } else {
        ...
        if (isHost()) {
            if (game_.getOnlineMode() != PokerGame.MODE_PLAY) // same as !game_.isInProgress()
            {
                ...
                if (bObs) {
                    ...
                    if (table != null)
                        sendMessageTable(omsg, table, player);
    ...
    private void processCancel(OnlineMessage omsg) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                String sKey = bBan
                        ? "msg.cancel.client.ban"
                        : game_.isInProgress() ? "msg.cancel.client.play" : "msg.cancel.client.reg";

                PokerPlayer host = getHost();
                EngineUtils.displayInformationDialog(context_,
                        PropertyConfig.getMessage(sKey, Utils.encodeHTML(host.getName())));
                context_.restart();
            }
        });
    }
     ...
     * for calling setDirty(true/false) as appropriate. If bIncludeTables is set to
     * true, then the poker table data is included. If bIncludeProfile is true, then
     * the tournament profile is sent again
     */
    public void sendDirtyPlayerUpdateToAll(boolean bIncludeTables, boolean bIncludeProfile) {
        ...
        PokerSaveDetails pdetails = (PokerSaveDetails) details.getCustomInfo();
        if (bIncludeProfile)
            pdetails.setSaveProfileData(SaveDetails.SAVE_ALL);
        details.setSavePlayers(SaveDetails.SAVE_DIRTY);
        details.setSaveObservers(SaveDetails.SAVE_DIRTY);
     ...
     */
    public void sendTableUpdate(PokerTable table, DMArrayList<PokerTableEvent> events, boolean bOnlySendToWaitList,
        ...
        if (bOnlySendToWaitList) {
            ...
            for (int i = 0; i < nNum; i++) {
                ...
                if (player.isRejoining())
                    continue;
                sendTableUpdate(table, player, events, PokerSaveDetails.NO_OVERRIDE, bRunProcessTable, null, false,
                        null, null, null);
            }
        } else {
            for (int i = 0; i < PokerConstants.SEATS; i++) {
                player = table.getPlayer(i);
                if (player == null || player.isComputer() || player.isRejoining())
                    continue;
                sendTableUpdate(table, player, events, PokerSaveDetails.NO_OVERRIDE, bRunProcessTable, null, false,
                        null, null, null);
            }
    ...
    /**
     * Send poker table data to given player. If dirtyTables and/or dirtyPlayers are
     * non-null, then those specific tables and players are marked dirty and sent.
     * Otherwise the given table (and its players/observers) are marked dirty and
     * sent.
     */
    public void sendTableUpdate(PokerTable table, PokerPlayer playerTo, DMArrayList<PokerTableEvent> events,
            int nOverrideState, boolean bRunProcessTable, String sPhase, boolean bSetCurrentTableToLocal,
            List<PokerTable> dirtyTables, List<PokerPlayer> dirtyPlayers, List<PokerTable> removedTables) {
        if (playerTo.isHost())
            return;
    ...
    /**
     * Get int array of ids for tables in this list. Return null if null or empty.
     */
    ...
    /**
     * Send message to host to join game - assumes the 'parent' PokerGame has been
     * created with a temporary player setup with a connection URL to the host.
     * Return Boolean.TRUE if join successful. If successful, the next phase will be
     * run from here. If failed, return error EngineMessage with details of problem.
     */
    public Object joinGame(boolean bObserve, boolean bReconnect, boolean bHeadless) {
        ...
        if (bHeadless) {
            ...
            params.setString(PokerP2PDialog.PARAM_DISPLAY,
                    PropertyConfig.getMessage("msg.joingame", msg.getConnectURL()));
    ...
    public void alive(boolean bInGame) {
        if (bHost_) {
            ...
            for (int i = 0; i < nNum; i++) {
                ...
                if (p.isHost() || p.isComputer() || p.isObserver())
                    continue;
            ...
            if (host.getConnection() != null) {
                ...
                if (!host.getVersion()
                        .isBefore(bInGame
                                ? PokerConstants.VERSION_ALIVE_CHECK_ADDED
    ...
    /**
     * check to see whether all clients that should be connected are connected.
     */
    void checkAllConnected() {
        ...
        if (bPauseAllDisconnected) {
            ...
            for (int i = 0; i < nNum; i++) {
                ...
                if (player.isHost() || player.isComputer() || player.isEliminated())
                    continue;
                nClients++;
    ...
    /**
     * Set the TournamentDirector in use. If game updates have queued up, process
     * them immediately.
     */
    public void setTournamentDirector(TournamentDirector td) {
        synchronized (tdQueue_) {
            ...
            // if we are the host and now in play mode, adjust number of worker threads
            // based
            ...
            if (bHost_ && td != null) {
                ...
                // our estimate for the number of desired worker threads on host is
                // approximately 1 per every
                ...
                if (bMoreThreads) {
                    ...
                    if (GameServer.DEBUG_POOL)
                        logger.debug("Done adding threads");
                }
            }
    ...
    /**
     * process a game update message, or if TournamentDirector is null, queue it up
     */
    ...
    private void _processGameUpdate(OnlineMessage omsg) {
        ...
        if (events != null) {
            ...
            for (int i = 0; i < nNum; i++) {
                event = events.get(i);
                if (TournamentDirector.DEBUG_EVENT)
    ...
    public void processPlayerUpdate(OnlineMessage omsg) {
        ...
        if (td_ != null)
            td_.processPlayerUpdate(player, omsg.getPlayerSettings());
    ...
    /**
     * Marshal the game into a string and store it with this message. Only store the
     * game data and the components marked "dirty"
     */
    ...
    /**
     * Send chat (called from ChatPanel). If table is specified, message only sent
     * to players at that table.
     */
    ...
    public void sendDirectorChat(String sMessage, Boolean bPauseClock) {
        ...
        if (local.isHost()) {
            ...
            throw new ApplicationError(ErrorCodes.ERROR_UNSUPPORTED, "sendDirectorChat only allowed from host",
                    sMessage);
        }
    ...
    /**
     * Do sending of a chat. If a table is specified in the chat message, it should
     * only go to people at that table. If should display locally, return true.
     */
    ...
    private void deliverChat(OnlineMessage chat) {
        ...
        if (isHost()) {
            ...
            if (nTableNum != OnlineMessage.NO_TABLE) {
                ...
                if (table == null) {
                    logger.warn("No table found for table num (" + nTableNum + ") to deliver chat: " + chat.getChat()
                            + ", from-id: " + chat.getFromPlayerID() + ", chat-type: " + chat.getChatType());
                    return;
                }
                if (!table.isCurrent())
                    return;
    ...
    private PokerPlayer getPlayer(OnlineMessage omsg, boolean bThrowErrorIfMissing) {
        ...
        if (player == null) {
            if (bThrowErrorIfMissing) {
                throw new ApplicationError(ErrorCodes.ERROR_INVALID, "No player found for id in message",
                        omsg.toString(), null);
            } else {
     ...
     */
    public void onlineProcessPhase(String sPhaseName, DMTypedHashMap params, boolean bLocalNow, PokerPlayer to) {
        // if locally controlled
        if (to.isLocallyControlled()) {
            if (bLocalNow)
                context_.processPhaseNow(sPhaseName, params);
            else
                context_.processPhase(sPhaseName, params);
            return;
        }
    ...
    private void sendMessageTable(OnlineMessage omsg, PokerTable table, PokerPlayer exclude) {
        ...
        for (int i = 0; i < PokerConstants.SEATS; i++) {
            p = table.getPlayer(i);
            if (p == null || p.isComputer() || p.isHost() || p.isRejoining() || p == exclude)
                continue;
        ...
        for (int i = nNum - 1; i >= 0; i--) {
            ...
            if (p.isHost() || p.isRejoining() || p == exclude)
                continue;
    ...
    /**
     * Send message to all players and observers except specified player or host or
     * computer
     */
    private void sendMessageAllExcept(OnlineMessage omsg, PokerPlayer exclude, boolean bImmediate) {
        ...
        for (int i = 0; i < nNum; i++) {
            ...
            if (p == exclude || p.isHost() || p.isComputer() || p.isObserver())
                continue;
    ...
    /**
     * Send message to given player. If bImmediate, sends message now otherwise adds
    ...
    private void sendMessage(OnlineMessage omsg, PokerPlayer pTo, boolean bImmediate) {
        ...
        if (pc == null) {
            if (pTo.isHost() && pTo == getLocalPlayer()) {
                ...
                // logger.warn("Can't send message to: " + pTo.getName() + " (no socket): " +
                // omsg.toStringNoData());
            }
            return;
        }
     ...
     * get error reply
     */
    public static DDMessageTransporter getAppErrorReply(PokerConnectionServer p2p, OnlineMessage omsg, String sMsg,
            boolean bKeepAlive) {
    ...
    /**
     * Return message to be sent in response to test message. In a static so can be
     * used from PokerMain when no OnlineManager instance exists
     */
