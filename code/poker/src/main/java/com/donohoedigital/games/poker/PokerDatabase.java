...
public class PokerDatabase {
        ...
        try (Statement stmt = conn.createStatement()) {
            if (!conn.getMetaData().getTables(null, null, "TOURNAMENT", null).next()) {
                stmt.executeUpdate("CREATE TABLE TOURNAMENT (\n"
                        + "TRN_ID INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n"
            ...
            if (!conn.getMetaData().getTables(null, null, "TOURNAMENT_FINISH", null).next()) {
                stmt.executeUpdate("CREATE TABLE TOURNAMENT_FINISH (\n"
                        + "TRF_ID INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n"
            ...
            /*
             * if (!conn.getMetaData().getColumns(null, null, "TOURNAMENT_FINISH",
             * "TRF_PLAYERS_REMAINING").next()) { stmt.executeUpdate(
            ...
            if (!conn.getMetaData().getTables(null, null, "HAND", null).next()) {
                stmt.executeUpdate("CREATE TABLE HAND (\n"
                        + "HND_ID INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n" + "HND_TABLE VARCHAR(32),\n"
            ...
            if (!conn.getMetaData().getTables(null, null, "TOURNAMENT_PLAYER", null).next()) {
                stmt.executeUpdate("CREATE TABLE TOURNAMENT_PLAYER (\n"
                        + "TPL_ID INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n"
            ...
            if (!conn.getMetaData().getTables(null, null, "PLAYER_HAND", null).next()) {
                stmt.executeUpdate("CREATE TABLE PLAYER_HAND (\n" + "PLH_HAND_ID INTEGER NOT NULL,\n"
            ...
            if (!conn.getMetaData().getTables(null, null, "PLAYER_ACTION", null).next()) {
                stmt.executeUpdate("CREATE TABLE PLAYER_ACTION (\n" + "ACT_HAND_ID INTEGER NOT NULL,\n"
    ...
    private static int identity(PreparedStatement pstmt) throws SQLException {
    ...
    public static int storeHandHistory(ClientHoldemHand hhand) {
        ...
        try {
            ...
            if (game.isDeleteHandsAfterSaveDate()) {
                ...
                if (lastHand != 0) {
                    try (PreparedStatement pstmt = conn.prepareStatement("DELETE FROM PLAYER_ACTION\n"
                            + "WHERE ACT_HAND_ID > ?\n"
                    ...
                    try (PreparedStatement pstmt = conn.prepareStatement("DELETE FROM PLAYER_HAND\n"
                            + "WHERE PLH_HAND_ID > ?\n"
            ...
            try (PreparedStatement pstmt = conn.prepareStatement("INSERT INTO PLAYER_ACTION (\n" + "ACT_HAND_ID,\n"
                    + "ACT_PLAYER_ID,\n" + "ACT_SEQUENCE,\n" + "ACT_ROUND,\n" + "ACT_TYPE,\n" + "ACT_AMOUNT,\n"
                    + "ACT_SUB_AMOUNT,\n" + "ACT_ALL_IN,\n" + "ACT_INTENT\n" + ") VALUES (?,?,?,?,?,?,?,?,?)")) {
                for (int i = 0; i < history.size(); ++i) {
                    ...
                    if ((intent != null) && intent.length() > INTENT_LENGTH) {
                        logger.warn("Value of action.getDebug() is longer than " + INTENT_LENGTH
    ...
    private static int storeTournament(Connection conn, PokerGame game) throws SQLException {
        ...
        try (PreparedStatement pstmt = conn
                .prepareStatement("SELECT TRN_ID FROM TOURNAMENT\n" + "WHERE TRN_START_DATE=?")) {
        ...
        if (tournamentID == -1) {
            try (PreparedStatement pstmt = conn.prepareStatement("INSERT INTO TOURNAMENT (\n" + "TRN_TYPE,\n"
                    + "TRN_NAME,\n" + "TRN_TOTAL_PLAYERS,\n" + "TRN_START_DATE\n" + ") VALUES (?,?,?,?)",
                    Statement.RETURN_GENERATED_KEYS)) {
    ...
    private static int storeTournamentFinish(Connection conn, PokerGame game, ClientPlayer player) throws SQLException {
        ...
        try (PreparedStatement pstmt = conn
                .prepareStatement("DELETE FROM TOURNAMENT_FINISH\n" + "WHERE TRF_PROFILE_CREATE_DATE=? AND\n"
        ...
        try (PreparedStatement pstmt = conn.prepareStatement("INSERT INTO TOURNAMENT_FINISH (\n"
    ...
    private static void deleteTournament(Connection conn, TournamentHistory hist) throws SQLException {
        ...
        // delete hands
        try (PreparedStatement pstmt = conn.prepareStatement("DELETE FROM HAND\n" + "WHERE HND_TOURNAMENT_ID=?")) {
            pstmt.setLong(1, hist.getGameId());
            pstmt.executeUpdate();
        }
        ...
        // delete players
        try (PreparedStatement pstmt = conn
                .prepareStatement("DELETE FROM TOURNAMENT_PLAYER\n" + "WHERE TPL_TOURNAMENT_ID=?")) {
            pstmt.setLong(1, hist.getGameId());
            pstmt.executeUpdate();
        }
        ...
        // delete tournament
        try (PreparedStatement pstmt = conn.prepareStatement("DELETE FROM TOURNAMENT\n" + "WHERE TRN_ID=?")) {
            pstmt.setLong(1, hist.getGameId());
            ...
            try (PreparedStatement pstmt = conn
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        h.setTournamentName(PropertyConfig.getMessage(
                                count == 1 ? "msg.overalltourney.singular" : "msg.overalltourney.plural", count));
    ...
    public static void playerNameChanged(PokerGame game, ClientPlayer player) {
        ...
        try {
            ...
            if (playerID != -1) {
                try (PreparedStatement pstmt = conn
                        .prepareStatement("UPDATE TOURNAMENT_PLAYER\n" + "SET TPL_NAME=?\n" + "WHERE TPL_ID=?")) {
    ...
    public static int getHandCount(String where, BindArray bindArray) {
        ...
        try {
            try (PreparedStatement pstmt = conn.prepareStatement("SELECT COUNT(*) FROM HAND WHERE " + where)) {
    ...
    public static List<Integer> getHandIDs(String where, BindArray bindArray, int first, int page) {
        ...
        try {
            try (PreparedStatement pstmt = conn.prepareStatement("SELECT HND_ID FROM HAND WHERE " + where)) {
                int bindValueCount = (bindArray != null) ? bindArray.size() : 0;
    ...
    public static int getTournamentCount(String where, BindArray bindArray) {
        ...
        try {
            try (PreparedStatement pstmt = conn
                    .prepareStatement("SELECT COUNT(DISTINCT HND_TOURNAMENT_ID) FROM HAND WHERE " + where)) {
                int bindValueCount = (bindArray != null) ? bindArray.size() : 0;
    ...
    public static String[] getHandAsHTML(int handID, boolean bShowAll, boolean bShowReason) {
        ...
        try {
            ...
            Object[] oParams = new Object[]{sb2.toString(),
                    ...
                    PropertyConfig.getMessage("list.gameType." + gameType.toLowerCase()), (ante > 0) ? unit : "",
                    (ante > 0) ? ante : (Object) PropertyConfig.getMessage("msg.forcedbet.none"), unit, smBlind, unit,
                    bigBlind,
                    (startDate != null)
            ...
            try (PreparedStatement pstmt = conn.prepareStatement("SELECT DISTINCT\n" + "PLH_SEAT_NUMBER,\n"
                ...
                try (ResultSet rs = DatabaseManager.executeQuery(pstmt)) {
                    while (rs.next()) {
                        ...
                        if ((ts != null)
                                && (ts.getTime() == PlayerProfileOptions.getDefaultProfile().getCreateDate())) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement("SELECT\n" + "ACT_ROUND,\n" + "PLH_SEAT_NUMBER,\n"
                ...
                try (ResultSet rs = DatabaseManager.executeQuery(pstmt)) {
                    while (rs.next()) {
                        ...
                        HandAction action = new HandAction(ieHand.players[seat], round, actionType, amount, subamount,
                                intent);
                        action.setAllIn(bAllIn);
    ...
    private static String getActualDatabaseName(PlayerProfile profile) {
        ...
        return CLIENT_DATABASE_NAME + ((profile == null) ? "" : ("-" + profile.getFileNum()))
    ...
    public static String getHandListHTML(int handID) {
        ...
        try {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement("SELECT " + "HND_COMMUNITY_CARDS_DEALT,\n"
                    + "HND_COMMUNITY_CARD_1,\n" + "HND_COMMUNITY_CARD_2,\n" + "HND_COMMUNITY_CARD_3,\n"
                    + "HND_COMMUNITY_CARD_4,\n" + "HND_COMMUNITY_CARD_5\n" + "FROM HAND\n" + "WHERE HND_ID=?")) {
                pstmt.setInt(1, handID);
