...
public class DealCommunity extends ChainPhase implements PlayerActionListener {
    ...
    public void start() {
        ...
        if (hhand_.isAllInShowdown() && !table_.isZipMode()) {
            ...
            if (bPause && table_.getHandNum() != nLastHandAllin_) {
                EngineUtils.displayInformationDialog(context_, PropertyConfig.getMessage("msg.allin.info"),
                        "msg.allin.title", "allininfo");
            }
            ...
            for (int i = 0; i < PokerConstants.SEATS; i++) {
                ...
                if (player == null || player.isFolded() || player.getHand() == null)
                    continue;
    ...
    private void process(int nWaitTenths) {
        if (bDrawn_ && !table_.isZipMode() && nNumWithCards_ > 1) {
            ...
            if (nWaitTenths == 0)
                nWaitTenths = game_.isOnlineGame() ? 0 : PokerUtils.getIntOption(PokerConstants.OPTION_DELAY);
            if (nWaitTenths > 0) {
        ...
        switch (nRound_) {
            ...
            default :
                throw new ApplicationError(ErrorCodes.ERROR_CODE_ERROR,
                        "Invalid round: " + HoldemHand.getRoundName(nRound_), null);
        }

        // chat display
        if (bDrawn_ && !table_.isZipMode()) {
            String sRound = PropertyConfig.getMessage("msg.round." + nRound_);
            td_.sendDealerChatLocal(PokerConstants.CHAT_2,
                    PropertyConfig.getMessage("msg.chat.community", sRound, sHand));
        }
