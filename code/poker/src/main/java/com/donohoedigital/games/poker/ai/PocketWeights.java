...
public class PocketWeights {
    ...
    /**
     * PocketWeights is a singleton class (constructor is private) that reviews the
     * action so far in the hand to build a weight matrices reflecting the
     * likelihood of each remaining player having a given pocket hand. The single
     * instance is returned only by PocketWeights.getInstance() to enforce the
     * singleton model. When a call is made to PocketWeights.getInstance() with a
     * different instance of HoldemHand, or after an event that might change the
     * outcomes, the results are recomputed, otherwise results are reused.
     *
     * @param hhand
     *            The hand in play.
     * @return An instance of PocketWeights for the current state of the specified
     *         hand.
     */
    ...
    /**
     * Returns the weight table for a particular seat, for display or further
     * computation.
     *
     * @param seat
    ...
    public float getBiasedRawHandStrength(int seat, int opponentSeat) {
        ...
        if (hand.containsCard(Card.BLANK))
            return 0f;
        ...
        for (int i = 1; i < 52; ++i) {
            if (hand.containsCard(i) || community.containsCard(i))
                continue;

            for (int j = 0; j < i; ++j) {
                if (hand.containsCard(j) || community.containsCard(j))
                    continue;
            ...
            case HandAction.ACTION_CHECK :
                func = new PreFlopWeightFunction() {
                    public float adjustWeight(PokerPlayer player, int card1, int card2, float weight, float rhs) {
                        ...
                        /*
                         * if (callCount_ > 0 || raiseCount_ > 0) { return (float)(Math.pow(1.0f-rhs,
                         * callCount_))*weight; } else { return preflopActor.compute(player, card1,
                         * card2, rhs, tuple).checkFold * (1f-rhs); }
                         */
                    }
                };
                break;
            case HandAction.ACTION_CALL :
                ...
                func = new PreFlopWeightFunction() {
                    public float adjustWeight(PokerPlayer player, int card1, int card2, float weight, float rhs) {
                        ...
                        /*
                         * if (callCount_ > 0 || raiseCount_ > 0) { return (float)(Math.pow(rhs,
                         * raiseCount_ * 2 + callCount_ + 1)*weight); } else { return
                         * preflopActor.compute(player, card1, card2, rhs, tuple).call * rhs; }
                         */
                    }
                };
                break;
            ...
            case HandAction.ACTION_RAISE :
                ...
                func = new PreFlopWeightFunction() {
                    public float adjustWeight(PokerPlayer player, int card1, int card2, float weight, float rhs) {
                        ...
                        /*
                         * if (callCount_ > 0 || raiseCount_ > 0) { return (float)(Math.pow(rhs,
                         * raiseCount_ * 2 + callCount_ + 2)*weight); } else { return
                         * preflopActor.compute(player, card1, card2, rhs, tuple).betRaise * rhs; }
                         */
                    }
                };
                break;
        ...
        for (int i = 1; i < 52; ++i) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                weightTable.set(i, j, func.adjustWeight(player, i, j, weightTable.get(i, j),
                        scheme.getHandStrength(Card.getCard(i), Card.getCard(j))));
                // PreFlopBias.getOpenPotWeight(Card.getCard(i), Card.getCard(j),
                // Math.max(position-1, PokerAI.POSITION_EARLY), .5f))); }
            }
        }
    }
            ...
            case HandAction.ACTION_CHECK :
                func = new PostFlopWeightFunction() {
                    public float adjustWeight(float weight, float rhs, float ppot, float npot) {
                        ...
                        return (float) (0.75d + Math.sin(1.25d * Math.PI * (rhs - npot) + Math.PI / 2.0d) / 4.0d)
                                * weight;
                ...
                for (int j = 0; j < i; ++j) {
                    if (community.containsCard(j)) {
                        ...
                        if (Float.isNaN(rhs)) {
                            ...
                            float newWeight = func.adjustWeight(oldWeight, ranks.getRawHandStrength(i, j), 0.0f, 0.0f);
