 ...
 * logic from {@code TournamentProfileHtml} directly in the poker module,
...
public class TournamentProfileFormatter {
    ...
    public String toHTMLSummary(boolean bListMode, String sLocale) {
        if (htmlSummaryCache_ != null && htmlSummaryMode_ == bListMode)
        ...
        if (profile.isRebuys()) {
            ...
            switch (profile.getRebuyExpressionType()) {
                case PokerClientConstants.REBUY_LT :
                    sExpr = "&lt;";
                    break;
                case PokerClientConstants.REBUY_LTE :
                    sExpr = "&lt;=";
                    break;
            }
            params[5] = PropertyConfig.getMessage(sKey, profile.getMap().get(PARAM_REBUYCOST),
                    profile.getMap().get(PARAM_REBUYCHIPS), profile.getMap().get(PARAM_REBUY_UNTIL),
                    profile.getMap().get(PARAM_MAXREBUYS), sExpr, profile.getRebuyChipCount());
        } else {
        ...
        // addons
        if (profile.isAddons()) {
            params[6] = PropertyConfig.getMessage("msg.addoninfo", profile.getMap().get(PARAM_ADDONCOST),
                    profile.getMap().get(PARAM_ADDONCHIPS), profile.getMap().get(PARAM_ADDONLEVEL));
        } else {
        ...
        if (profile.hasLimitLevels()) {
            params[10] = PropertyConfig.getMessage("msg.limitraises", profile.getMaxRaises(10, false),
                    profile.isRaiseCapIgnoredHeadsUp() ? PropertyConfig.getMessage("msg.ignoreheads") : "");
        } else {
            params[10] = "";
        }

        if (profile.getFile() != null) {
            String sFile = GameConfigUtils.SAVE_DIR + File.separator + TOURNAMENT_DIR + File.separator
        ...
        if (bListMode) {
            ...
            if (!profile.isInviteOnly())
                sInfo = PropertyConfig.getYesNo(false);
            else
                sInfo = PropertyConfig.getMessage("msg.inviteonly." + profile.isInviteObserversPublic(),
    ...
    public String toHTML(String sLocale) {
        ...
        for (int i = 1; i <= nNum; i++) {
            ...
            if (profile.isBreak(i)) {
                ...
                sb.append(PropertyConfig.getMessage("msg.levelinfo", nLevel, getDollar(profile.getAnte(i)),
                        getDollar(profile.getSmallBlind(i)), getDollar(profile.getBigBlind(i)), sMinutes, sGame,
                        cssClass(odd)));
            }
        ...
        if (profile.isRebuys() && profile.isAddons())
            sKey = "msg.pool.both";
        else if (profile.isRebuys())
            sKey = "msg.pool.rebuys";
        else if (profile.isAddons())
            sKey = "msg.pool.addons";
        ...
        // limit info
        if (profile.hasLimitLevels()) {
            params[14] = PropertyConfig.getMessage("msg.limitraises", profile.getMaxRaises(10, false),
                    profile.isRaiseCapIgnoredHeadsUp() ? PropertyConfig.getMessage("msg.ignoreheads") : "");
        } else {
            params[14] = "";
        }
        ...
        // invite list
        if (!profile.isInviteOnly())
            params[17] = PropertyConfig.getYesNo(false);
        else
            params[17] = PropertyConfig.getMessage("msg.inviteonly." + profile.isInviteObserversPublic(),
                    profile.getInvitees().toCSV());
    ...
    public String toHTMLOnline() {
        ...
        if (!profile.isBootDisconnect())
            sBootDis = PropertyConfig.getYesNo(false);
        else
            sBootDis = PropertyConfig.getMessage("msg.boot", profile.getBootDisconnectCount());

        if (!profile.isBootSitout())
            sBootSit = PropertyConfig.getYesNo(false);
        else
            sBootSit = PropertyConfig.getMessage("msg.boot", profile.getBootSitoutCount());

        if (!profile.isInviteOnly())
            sInviteOnly = PropertyConfig.getYesNo(false);
        else
    ...
    static String getTableFormatDisplay(int seats) {
        return switch (seats) {
            case ProtocolConstants.SEATS_FULL_RING -> "Full Ring, 10 per table";
            case ProtocolConstants.SEATS_6MAX -> "6-Max, 6 per table";
            case ProtocolConstants.SEATS_HEADS_UP -> "Heads-Up";
            default -> seats + " per table";
        };
    }

    /**
     * Get blinds text for display.
     */
    public String getBlindsText(String prefix, int nLevel, boolean briefAmounts) {
        ...
        if (sGameType.length() > 0)
            sGameType = PropertyConfig.getMessage(prefix + "gametype", sGameType);
        ...
        return PropertyConfig.getMessage(nAnte == 0 ? prefix + "blinds" : prefix + "blinds.a",
                ...
                nAnte == 0
                        ? null
                        : (briefAmounts ? PropertyConfig.getAmount(nAnte, true, true) : Integer.toString(nAnte)),
                sGameType);
    }
}
