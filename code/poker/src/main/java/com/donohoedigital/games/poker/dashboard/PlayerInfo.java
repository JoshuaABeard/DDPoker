...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Jun 14, 2005 Time: 6:43:30 PM
...
public class PlayerInfo extends DashboardItem implements TerritorySelectionListener {
    ...
    private void updateInfo(Territory t) {
        ...
        if (t == null)
            ...
            p = PokerUtils.getPokerPlayer(context_, t);
    ...
    protected void updateInfo() {
        if (last_ != null && !last_.isObserver()) {
            ...
            if (numLeft == 0)
                numLeft = game_.getNumPlayers();
            labelInfo_.setText(PropertyConfig.getMessage("msg.dash.playerinfo", Utils.encodeHTML(last_.getName()),
                    last_.getHandsPlayedDisconnected(), last_.getHandsPlayedSitout(), sRebuy,
                    PropertyConfig.getPlace(game_.getRank(last_)), numLeft));
        } else {
            String sRebuySpace = "";
            if (game_.getProfile().isRebuys())
                sRebuySpace = PropertyConfig.getMessage("msg.dash.rebuy.space");
