...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 16, 2006 Time: 2:30:49 PM
...
public class ChatLobbyPanel extends ChatPanel {
     ...
     * new chat panel
     */
    public ChatLobbyPanel(GameContext context, ChatLobbyManager mgr, PlayerProfile profile, String sStyle,
            String sBevelStyle) {
    ...
    public void chatReceived(OnlineMessage omsg) {
        ...
        if (sMsg == null)
            return; // for chat-admin messages with no chat
    ...
    void displayMessage(String sFrom, String sFromKey, int nType, String sMsg, boolean bLocal) {
        ...
        if (nType == OnlineMessage.CAT_CHAT_ADMIN || sMsg.startsWith("tahoezorro")) {
            ...
            // muted specifically?
            if (!bLocal && (muted_.containsPlayer(sFrom) || muted_.containsKey(sFromKey)))
                return;
