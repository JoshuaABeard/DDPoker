...
public class HandProbabilityMatrix {
    ...
    public void adjustWeightsPreFlop(HandSelectionScheme scheme, PokerPlayer player, int action, Hand hole) {
    ...
    /*
     * public void adjustForFlop(HandContext handContext) { Hand hole =
     * handContext.getPocket(); Hand community = handContext.getCommunity();
     *
     * Hand hand = new Hand(Card.BLANK, Card.BLANK);
     *
     ...
     *
     * for (int i = 51; i >= 0; --i) { hand.setCard(0, Card.getCard(i % 4, i / 4 +
     * 2));
     *
     * for (int j = 51; j > i; --j) { hand.setCard(1, Card.getCard(j % 4, j / 4 +
     * 2));
     *
     * if (hole.containsAny(hand) || community.containsAny(hand)) { prob_[i][j] = 0;
     * otherHandScore_[i][j] = 0; } else { otherHandScore_[i][j] =
    ...
    /*
     * public float getOdds(Card card1, Card card2) { int i = (card1.getRank() - 2)
     * * 4 + card1.getSuit(); int j = (card2.getRank() - 2) * 4 + card2.getSuit();
     *
     * if (j > i) { return odds_[i][j]; } else { return odds_[j][i]; } }
     */
}
