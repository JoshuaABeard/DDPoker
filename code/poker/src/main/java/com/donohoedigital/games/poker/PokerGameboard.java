...
public class PokerGameboard extends Gameboard {
    ...
    private static final GeneralPath FELT = GuiUtils.drawSVGpath(
            "M180,0c90,1,760,1,850,0c90,1,180,184,180,443.5c0,260.5-90,442.5-180,442.5s-760,0-850,0S0,704,0,443.5C0,184,90,1,180,0z",
            false);
     ...
     */
    public PokerGameboard(GameEngine engine, GameContext context, GameboardConfig gameconfig, int nSmallWidth,
            int nSmallHeight, int nStartingWidth, int nStartingHeight, PokerGameboardDelegate delegate,
            ImageComponent tile) {
    ...
    public void repaintTerritory(Territory t, boolean bImmediate) {
        ...
        if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT))
            logger.debug("************** PokerGameboard REPAINT " + t.getName() + ' ' + bImmediate
                ...
                if (table.isColoringUpDisplay()) {
                    ...
                    if (nBet > 0 || player.isCurrentGamePlayer()) {
                        ...
                        if (player.isFolded()) {
                            ...
                            // something to call
                            if (nCall > 0) {
                                sText = PropertyConfig.getMessage(nBet == 0 ? "msg.bet.call" : "msg.bet.call2", NBET,
                                        PropertyConfig.getAmount(nCall));
                            }
                            // nothing to call, but already bet means option on big blind (or small if
                            // small==big)
    ...
    public void repaintAll() {
        ...
        if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT))
            logger.debug("************** REPAINT ALL END ****************************************");
    }

    @Override
    protected void paintChildren(Graphics g) {
        // if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT))
        // logger.debug("************** REPAINT CHILDREN
        // ++++++++++++++++++++++++++++++++++++++++");
        super.paintChildren(g);
        // if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT))
        // logger.debug("************** REPAINT CHILDREN END
        // +++++++++++++++++++++++++++++++++++++");
    }
    ...
    public static class FauxPokerGameboard extends ImageComponent {
        ...
        public void updateColors(Color top, Color bottom) {
            if (top_.equals(top) && bottom_.equals(bottom))
                return;
            top_ = top;
            bottom_ = bottom;
            refresh();
