...
public class OtherTables {
     ...
     *            the game
     * @param tables
     *            the list of tables to possibly break (players are moved to any
     *            table in the game)
     */
    public static void consolidateTables(PokerGame game, List<PokerTable> tables) {
        ...
        if (nOpenTotal > 0) {
            ...
            if (waiting != null) {
                ...
                for (int i = 0; i < waiting.size() && nOpenTotal > 0; i++) {
                    ...
                    if (DEBUG)
                        logger.debug("++++++++++ SEATING WAITING PLAYER: " + player.getName() + " currently observing "
                                + (player.getTable() == null ? "[nothing - ai]" : player.getTable().getName()));
        ...
        if (game.getNumTables() > 1 && nOpenTotal > 0) {
            for (int i = 0; i < nNum; i++) {
                ...
                if (nExtra > 0) {
                    if (DEBUG)
                        logger.debug("+++++++++ EXTRA AT " + table.getName() + " players to move: " + nExtra);
                    // TODO: move from big blind? as TDA rules state (also where to place in new
                    // table?)
            ...
            for (int i = 0; i < nNum; i++) {
                ...
                if (table.getNumOccupiedSeats() == 1) {
                    ...
                    if (DEBUG)
                        logger.debug("++++++++++ TABLE REMOVED (1 player): " + table.getName()
                                + " player on wait list: " + theone.getName());
    ...
    private static int getNumOpenSeats(PokerGame game) {
        ...
        for (int i = game.getNumTables() - 1; i >= 0; i--) {
            ...
            if (DEBUG && (nOpen > 0 || game.getNumTables() <= 20))
                logger.debug(table.getName() + " has open: " + nOpen);
        }
        if (DEBUG)
            logger.debug("TOTAL open spots: " + nOpenTotal);
        return nOpenTotal;
    }
    ...
    /**
     * Figure out the maximum number of players per table based on players left in
     * tournament
     */
    private static int getMaxPlayersPerTable(PokerGame game) {
        ...
        if (nNumPlayers % game.getSeats() > 0)
            nNumTables++;
        int nMax = nNumPlayers / nNumTables;
        if (nNumPlayers % nNumTables > 0)
            nMax++;
        return nMax;
    }
    ...
    /**
     * Break the table if it is possible. Return the number of open seats left after
     * breaking the table.
     */
    private static int breakTable(PokerGame game, PokerTable table, int nOpenTotal, int nMax) {
        ...
        PokerPlayer player;
        // note: this if is same as if (nOpenTotal >= 10) - a little counterintuitive,
        // but a table is never broke
        ...
        if ((nOpenTotal - table.getNumOpenSeats()) >= nAvail) {
            ...
            if (DEBUG)
                logger.debug("++++++++++ BREAKING TABLE: " + table.getName() + " spots to move: " + nAvail);
            // break up this table
            ...
            ApplicationError.assertTrue(table.getNumOccupiedSeats() == 0, "at end of breakTable, but still people left",
                    table);
    ...
    private static void placePlayer(PokerGame game, PokerTable from, PokerPlayer player, int nMax, int nOpenTotal) {
        ...
        // get sorted array of tables (do each time since as players added, things
        // change)
        ...
        for (int i = tables.size() - 1; i >= 0; i--) {
            ...
            if (table == from)
                continue;
            if (table.isRemoved())
                continue; // don't move to removed table

            if (table.getNumOpenSeats() > 0 && table.getNumOccupiedSeats() < nMax) {
                if (DEBUG)
                    logger.debug(player.getName() + " moved to " + table.getName() + " from "
        ...
        for (int i = tables.size() - 1; i >= 0; i--) {
            table = tables.get(i);
            logger.error(table.getName() + ":  numOpen=" + table.getNumOpenSeats() + ", numOcc="
                    + table.getNumOccupiedSeats() + ", removed=" + table.isRemoved());
        ...
        for (int i = 0; i < nNum; i++) {
            ...
            table = p.getTable();
            logger.debug("Player " + i + " (" + p.getName() + ") chips: " + p.getChipCount() + " elim: "
                    + p.isEliminated() + " table: " + (table == null ? "null" : table.getName()));
        }
    ...
    /**
     * Remove broke players and add to the removed array. If bRemovePlayers is
     * false, then the players are not actually taken off the table (used for
     * display purposes).
     */
    public static void cleanTable(PokerTable table, List<PokerPlayer> removed, boolean bRemovePlayers) {
        ...
        for (int i = 0; i < PokerConstants.SEATS; i++) {
            ...
            if (player != null && player.getChipCount() == 0) {
                if (DEBUG)
                    logger.debug("Removed " + player.getName() + " from " + table.getName());
                if (bRemovePlayers) {
    ...
    public static void recordPlayerPlacement(TournamentDirector td, PokerGame game, List<PokerPlayer> removed) {
        ...
        for (PokerPlayer p : removed) {
            ...
            if (game.isOnlineGame()) {
                ...
                td.sendDirectorChat(
                        PropertyConfig.getMessage(nPrize > 0 ? "msg.chat.finish.money" : "msg.chat.finish.none",
                                Utils.encodeHTML(p.getDisplayName(game.isOnlineGame())),
                                PropertyConfig.getPlace(p.getPlace()), nPrize),
                        null);
            }
