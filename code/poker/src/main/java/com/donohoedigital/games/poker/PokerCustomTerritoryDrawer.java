...
public class PokerCustomTerritoryDrawer implements CustomTerritoryDrawer {
    ...
    /**
     * Draw biggest chip the amount can handle and return amount less that chip
     */
    private int drawAmount(Gameboard board, int nAmount, Graphics2D g, Rectangle territoryBounds) {
        ...
        while (!bOkay) {
            ...
            x1 = x + (width * X) / RANGE;
            y1 = y + (height * Y) / RANGE;
            if (ellipse.contains(x1, y1))
                bOkay = true;
        }

        // draw chip
        chip_.setValue(nValue);
        chip_.paintCustom(g, x1, y1, (int) board.scaleToCurrentSpace(s * WIDTH),
