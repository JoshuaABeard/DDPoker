...
public class V1Player extends PokerAI {
    ...
    protected void checkInit() {
        if (game_ == null)
            game_ = ((PokerPlayer) gamePlayer_).getTable().getGame();
        if (profile_ == null)
    ...
    public static boolean WantsAddon(PokerPlayer player, int nAddonPropensity, TournamentProfile profile) {
        ...
        if (nAddonPropensity < 50) {
            if (player.getChipCount() < (3 * nBuyin))
                return true; // 25% if chips < 3x buyin
            return false;
        }
        if (nAddonPropensity < 75) {
            if (player.getChipCount() < (2 * nBuyin))
                return true; // 25% if chips < 2x buyin
            return false;
        }
        return false;
    }
    ...
    public PlayerAction getAction(boolean quick) {
        ...
        if (_nLast == HandAction.ACTION_NONE)
            bIntendCheckRaise_ = false;
        ...
        if (true && TESTING(EngineConstants.TESTING_AI_DEBUG) && player.getTable().isCurrent()) {
            ...
            if (isPreFlop())
                logger.debug(player.getName() + " - improve: " + _dImprove);
        }

        // testing
        if (TESTING(PokerConstants.TESTING_AI_ALWAYS_CALLS)) {
            if (_nToCall == 0)
                return _bet("debug always call");
    ...
    /**
     * Sklansky's system as described pp 128-133 in Tournament Poker for Advanced
     * Players
     */
    private PlayerAction getSystem() {
        // pg 131 - pot raised?
        if (isPotRaised()) {
            if (_hole.isEquivalent(HoldemExpert.AA) || _hole.isEquivalent(HoldemExpert.KK)
        ...
        if (theNumber >= 400) {
            ...
            if (_hole.isEquivalent(HoldemExpert.AA) || _hole.isEquivalent(HoldemExpert.KK)
                    || _hole.isEquivalent(HoldemExpert.QQ) || _hole.isEquivalent(HoldemExpert.AKs)
                    || _hole.isEquivalent(HoldemExpert.AKo))
                bAllIn = true;
        } else if (theNumber >= 100) {
            if (_hole.isEquivalent(HoldemExpert.AA) || _hole.isEquivalent(HoldemExpert.KK)
                    || _hole.isEquivalent(HoldemExpert.QQ) || _hole.isEquivalent(HoldemExpert.JJ)
                    || _hole.isEquivalent(HoldemExpert.TT) || _hole.isEquivalent(HoldemExpert.AKs)
                    || _hole.isEquivalent(HoldemExpert.AKo) || _hole.isEquivalent(HoldemExpert.AQs)
                    || _hole.isEquivalent(HoldemExpert.AQo) || _hole.isEquivalent(HoldemExpert.KQs)
                    || _hole.isEquivalent(HoldemExpert.KQo))
                bAllIn = true;
        } else if (theNumber >= 80) {
            if (_hole.isPair() || _hole.isEquivalent(HoldemExpert.AKs) || _hole.isEquivalent(HoldemExpert.AKo)
                    || _hole.isEquivalent(HoldemExpert.AQs) || _hole.isEquivalent(HoldemExpert.AQo)
                    || _hole.isEquivalent(HoldemExpert.KQs) || _hole.isEquivalent(HoldemExpert.KQo)
                    || (_hole.isSuited() && _hole.isInHand(Card.ACE)) || (_hole.isSuited() && _hole.hasConnector(0, 4)))
                bAllIn = true;
        } else if (theNumber >= 20) {
            // 20-40 is same as below plus any suited cards
            if (theNumber < 40 && _hole.isSuited())
                bAllIn = true;

            // 40-60 is same as 60-80 plus any king
            if (!bAllIn && theNumber < 60 && _hole.isInHand(Card.KING))
                bAllIn = true;

            if (!bAllIn && _hole.isPair() || _hole.isInHand(Card.ACE) || _hole.isEquivalent(HoldemExpert.KQo)
                    || (_hole.isSuited() && _hole.isInHand(Card.KING)) || (_hole.isSuited() && _hole.hasConnector(1)))
                bAllIn = true;
        } else if (theNumber < 20) {
            bAllIn = true;
        }
        ...
        if (player.isEarly()) {
            if (isPotRaised()) {
                ...
                if (_hhand.getNumCallers() == 0) {
                    if (_hole.isEquivalent(HoldemExpert.AA) || _hole.isEquivalent(HoldemExpert.KK)
                            || _hole.isEquivalent(HoldemExpert.QQ) || _hole.isEquivalent(HoldemExpert.AKo)
                    ...
                    // other high hands
                    else if (_hole.isEquivalent(HoldemExpert.AKs) || _hole.isEquivalent(HoldemExpert.KQs)
                            || _hole.isEquivalent(HoldemExpert.AJs) || _hole.isEquivalent(HoldemExpert.KQs)) {
                ...
                // (we'll play threshold%)
                        ...
                        && nRandom <= nLooseThreshold && (_hole.isSuited() || _hole.isPair())) {
        ...
        ////
            ...
            if (isPotRaised()) {
                ...
                // SK158 (11) - call a raise cold - need "very good" hand
                    ...
                    if (seatsOccupied_ <= 3) {
                        if ((_hole.isSuited() && _hole.getCard(0).getRank() > 7) || _hole.hasConnector(0, 9)
                                || _hole.isInHand(Card.ACE) || _hole.isPair()
                                || ((_hole.isInHand(Card.KING) || _hole.isInHand(Card.QUEEN)
                                        || _hole.isInHand(Card.JACK)) && _hole.hasConnector(2, 9))) {
                            if (nRandom < 25) {
                ...
                if (seatsOccupied_ <= 3) {
                    ...
                    if (nRandom > getTightFactor()) {
                        int nConn = 0;
                        if (getTightFactor() < 10)
                            nConn = 1;
                        nRandom = DiceRoller.rollDieInt(100);
                        if (_hole.isSuited() || _hole.hasConnector(nConn) || _hole.isInHand(Card.ACE)
                                || _hole.isPair()) {
    ...
    private PlayerAction getPostFlop() {
        ...
        if (_dImprove < MIN_IMPROVE_ODDS)
            _dImprove = MIN_IMPROVE_ODDS;

        if (TESTING(EngineConstants.TESTING_AI_DEBUG) && player.getTable().isCurrent())
            logger.debug(player.getName() + " - improve: " + _dImprove);

        if (false && TESTING(EngineConstants.TESTING_AI_DEBUG) && player.getTable().isCurrent()) {
            logger.debug("**** POST FLOP for " + player.getName() + ": [ #str/draw? " + nNumOppStraights + "/"
                    + bStraightDraw + "]  [2/3flush? " + bFlushDraw + "/" + bThreeFlush + "]  [pair on board:"
                    + bBoardPair + "] " + " " + HandStat.fPerc.form(dStrength) + "% [" + HandStat.fPerc.form(_potOdds)
                    + "% odds]" + " [" + HandStat.fPerc.form(_dImprove) + "% improve]");
        }
            ...
            case HandInfo.STRAIGHT :
                if (inHole) {
                    ...
                    if (bThreeFlush) {
                        ...
                        if (isRaised()) {
                            if (bBoardPair)
                                return _foldPotOdds("re-raised, paired board");
                        }
            ...
            case HandInfo.TRIPS :
                ...
                if (inHole) {
                    ...
                    // pair on board, with one matching in hand
                        ...
                        if (bThreeFlush || nNumOppStraights > 0) {
                            return doBiggerHandPossibleBets("trips(2 board), pos flush/str", 60, 25, 30,
                                    HandInfo.TRIPS);
                        } else {
                            if (isReRaised()) {
                                if (nRandom < 50)
            ...
            case HandInfo.TWO_PAIR :
                if (inHole) {
                    if (bThreeFlush || nNumOppStraights > 0 || bBoardPair) {
                        return doBiggerHandPossibleBets("2 pair, pos flush/str/board paired", 60, 25, 35,
                                HandInfo.TWO_PAIR);
                    } else {
        ...
        if (seatsOccupied_ > 4 && nRandom > getTightFactor()) {
            ...
            if (nRandom < nThresh) {
                ...
                return PlayerAction.call()
                        .reason("V1:loose potodds " + sDebug + " [nTight: " + getTightFactor() + " " + nRandom + "r]");
            }
        }
    ...
    private PlayerAction _foldLooseCheck(String sDebug) {
        ...
        if (isEasy()) {
            if (_hhand.getNumCallers() == 0
                    && (_hole.isSuited() || _hole.hasConnector(1, 6) || _hole.isPair() || _hole.isInHand(Card.ACE))) {
                int nRandom = DiceRoller.rollDieInt(100);
                if (player.isEarly() && nRandom <= 15 || player.isMiddle() && nRandom <= 35
                        || player.isLate() && nRandom <= 60 || player.isBlind() && nRandom <= 80) {
     ...
     */
    private PlayerAction doBiggerHandPossibleBets(String sDebug, int nReRaiseFoldPerc, int nRaiseFoldPerc,
            int nFoldPerc, int nType) {
        ...
        if (_nToCall == 0) {
            ...
            if (isFlop() && (_raiserPre == HandAction.ACTION_CALL || _raiserPre == HandAction.ACTION_CHECK)) {
                ...
                nReRaiseFoldPerc += 20;
                nRaiseFoldPerc += 20;
            } else if (isTurn() && (_raiserFlop == HandAction.ACTION_CALL || _raiserFlop == HandAction.ACTION_CHECK)) {
                sDebug += " [flop limp]";
                nReRaiseFoldPerc += 25;
                nRaiseFoldPerc += 25;
            } else if (isRiver() && (_raiserTurn == HandAction.ACTION_CALL || _raiserTurn == HandAction.ACTION_CHECK)) {
                sDebug += " [turn limp]";
                nReRaiseFoldPerc += 30;
                nRaiseFoldPerc += 30;
            }
            ...
            // flop, player raising limped into pot
            if (isFlop() && (_bettorPre == HandAction.ACTION_CALL || _bettorPre == HandAction.ACTION_CHECK)) {
                sDebug += " [pre limp]";
                nFoldPerc += 20;
            } else if (isTurn() && (_bettorFlop == HandAction.ACTION_CALL || _bettorFlop == HandAction.ACTION_CHECK)) {
                sDebug += " [flop limp]";
                nFoldPerc += 25;
            } else if (isRiver() && (_bettorTurn == HandAction.ACTION_CALL || _bettorTurn == HandAction.ACTION_CHECK)) {
                sDebug += " [turn limp]";
                nFoldPerc += 30;
            }
    ...
    private boolean isGoodKicker(int nType) {
        ...
        // if (TESTING(EngineConstants.TESTING_AI_DEBUG)) logger.debug("KICKER ***** " +
        // player_.getName() + " kicker is " + kick);

        if (kick == -1)
            return false;
        if (kick == Card.ACE)
            return true;
        if (kick == Card.KING && _comm.isInHand(Card.ACE))
            return true;
    ...
    /**
     * Were we raised (true if our last action was a call, bet or raise)
     */
    private boolean isRaised() {
        if (_nLast == HandAction.ACTION_CALL || _nLast == HandAction.ACTION_BET || _nLast == HandAction.ACTION_RAISE) {
            return true;
        }
