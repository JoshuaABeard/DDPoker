...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 17, 2005 Time: 9:01:42 PM
 * To change this template use File | Settings | File Templates.
 */
public class DashboardItem
        implements
            Comparable<DashboardItem>,
            DataMarshal,
            AncestorListener,
            PokerTableListener,
    ...
    public void setInDashboard(boolean b) {
        ...
        if (!b) {
            if (dashboard_ != null)
                dashboard_.itemRemoveRequested(DashboardItem.this);
    ...
    public void setOpen(boolean b) {
        bOpen_ = b;
        if (body_ != null && body_.isVisible() != b)
            body_.setVisible(b);
        if (header_ != null && header_.check_.isSelected() != b)
            header_.check_.setSelected(b);
        if (dashboard_ != null)
            dashboard_.itemOpenClose(DashboardItem.this);
    }
    ...
    /**
     * subclass should override this to update the UI components. When dynamic
     * titles in use, will be called even when body is hidden
     */
    protected void updateInfo() {
    }

    /**
     * updates title by calling setTitle(geTitle()). Called when dynamic titles in
     * use, after updateInfo
     */
    ...
    /**
     * called when displayed in dashboard, by default calls updateInfo()
     */
    ...
    /**
     * called when body hidden (not removed), by default calls updateInfo() if
     * dynamic titles in use
     */
    ...
    /**
     * called when header displayed, by default calls updateInfo() if dynamic titles
     * in use and body not open
     */
    protected void headerDisplayed() {
        if (!isOpen() && bDynamicTitle_)
            updateAll();
    }
    ...
    /**
     * called when poker table changed (if listening to table events turned on with
    ...
    /**
     * subclass should implement (if listening to table events turned on with
     * trackTableEvents()). By default, calls updateInfo()
     */
    ...
    public void ancestorAdded(AncestorEvent event) {
        if (event.getSource() == body_)
            bodyDisplayed();
        if (event.getSource() == header_)
            headerDisplayed();
    }
    ...
    /**
     * inner class for listener to poker table events. When received, invoke in
     * timer thread (to get out of any locks held by callers like
     * TournamentDirector).
     */
    ...
    /**
     * Task for particular poker table event. When invoked, runs immediately in
     * swing thread using invokeAndWait.
     */
    ...
    /**
     * subclass should call during constructor to indicate it is interested in
     * tracking table events
     */
    ...
    private void trackTable(PokerTable table) {
        ...
        // cleanup old
        if (table_ != null)
            table_.removePokerTableListener(listener, PokerTableEvent.TYPES_ALL);
    ...
    // DESIGN NOTE: Removed this when changed listener_ above to use invokeAndWait
    // so all listeners notified
    // immediately (3.0p1)
