...
/**
 * Represents a range of bet size, either in terms of the pot, a player's stack,
 * or the big blind amount.
 */
public class BetRange {
    ...
    /**
     * Constructor for stack-relative bets. Pass in player whose stack is the
     * driver.
     */
    ...
    private BetRange(int type, PokerPlayer player, float min, float max) {
        ...
        switch (type_) {
            case STACK_SIZE :
                if (player_ == null)
                    throw new ApplicationError("Stack relative BetRange but no player.");
                break;
            case POT_SIZE :
            case BIG_BLIND :
                if (min_ == max_ && min_ == 0.0d)
                    throw new ApplicationError("BetRange min/max both zero.");
                break;
            case ALL_IN :
                break;
            default :
    ...
    /**
     * Get the maximum bet this range will return. Pass in the player who is
     * betting.
     */
    ...
    /**
     * Choose a random bet amount within the range. Pass in the player who is
     * betting.
     */
