...
public class PokerTable implements ObjectID, GameTable {
    ...
    public static String getStringForState(int nTableState) {
        switch (nTableState) {
            ...
            case STATE_PENDING :
                return "pending";
            case STATE_DEAL_FOR_BUTTON :
                return "button";
            case STATE_BEGIN :
                return "begin";
            case STATE_BEGIN_WAIT :
                return "begin-wait";
            case STATE_CLEAN :
                return "clean";
            case STATE_CHECK_END_HAND :
                return "check-end-hand";
            case STATE_NEW_LEVEL_CHECK :
                return "new-level-check";
            case STATE_COLOR_UP :
                return "color-up";
            case STATE_START_HAND :
                return "start";
            case STATE_BETTING :
                return "betting";
            ...
            case STATE_GAME_OVER :
                return "game-over";
            case STATE_PENDING_LOAD :
                return "pending-load";
            case STATE_ON_HOLD :
                return "on-hold";
            case STATE_BREAK :
                return "break";
            case STATE_PRE_SHOWDOWN :
                return "pre-showdown";
            default :
                return "unknown - " + nTableState;
        }
    }
    ...
    /**
     * used in online to set a time to pause until next action. Pass in the millis
     * to wait - this method adds that to System.currentTimeMillis()
     */
    ...
    public void addPlayer(PokerPlayer p) {
        ...
        if (isRemoved()) {
            throw new ApplicationError(ErrorCodes.ERROR_CODE_ERROR,
                    toString() + " is removed but trying to add player " + p, null);
        }
    ...
    public PokerPlayer[] getPlayersSortedByLastMove() {
        ...
        if (nOcc > 1)
            Arrays.sort(players, SORTBYMOVED);
        return players;
    }
    ...
    /**
     * Get seat offset so that local human player is at seat 5 (index 4). Add this
     * number to seat (modulus PokerTable.SEATS) to get territory seat
     */
    ...
    /**
     * Determine if is all computer. We cache the value since this is called quite
     * frequently in TournamentDirector
     */
    private void setAllComputer() {
        ...
        if (TESTING(PokerConstants.TESTING_PROCESS_ALL_COMPUTER_TABLES)) {
            bAllComputer_ = false;
            return;
        }
    ...
    /**
     * Set table state. Synchronized so we can coordiate with tournament director.
     */
    public synchronized void setTableState(int n) {
        ...
        if (nOld != nTableState_) {
            ...
            firePokerTableEvent(new PokerTableEvent(PokerTableEvent.TYPE_STATE_CHANGED, this, nOld, nTableState_));
    ...
    /**
     * add millis to last state change timestamp and player think banks - used when
     * TD is paused.
     */
    ...
    public void addWait(PokerPlayer p) {
        ...
        if (waitList_.contains(p)) {
            ...
            logger.warn("Attempting to add player already on wait list: " + p + " from: "
    ...
    /**
     * Set the button, by dealing high card if a human is at the table, randomly if
     * an all computer table
     */
    public void setButton() {
        // if all computer (but not current), set button randomly
        if (isAllComputer() && !isCurrent())
            setButtonRandom();
        else
            setButtonHighCard();
    }
    ...
    /**
     * Get hand num - 0 means no hand yet dealt
     */
    ...
    public void doColorUpDetermination() {
        ...
        for (int i = 0; i < PokerConstants.SEATS; i++) {
            ...
            nLeft = nChips % nMin;
            nOdd = nLeft / nLastMin;
            // logger.debug(player.getName() + " " + nChips + " chips, remainder = " + nLeft
            // + " odd=" + nOdd);
    ...
    private class SortChipRace implements Comparator<PokerPlayer> {
        ...
        public int compare(PokerPlayer p1, PokerPlayer p2) {
            ...
            if (p1.getChipCount() == nOddTotal1)
                return -1;
            if (p2.getChipCount() == nOddTotal2)
                return 1;
     ...
     * playing a hand takes too much time.
     */
    ...
    /**
     * verify all ai are removed from all players at this table (used when a table
     * is all-computer)
     */
    ...
    /**
     * Set to skip next button move - used when manually moving the button at the
     * end of a hand
     */
    ...
    private void newHand(PokerGame game) {
        ...
        if (nHandNum_ > 0) {
            if (nButton_ == NO_SEAT || (!bSkipNextButtonMove_ && (game.isOnlineGame()
    ...
    public boolean isRebuyAllowed(PokerPlayer player, int nLevel) {
        ...
        // rebuy is okay
        return nLevel <= nLast && (player.getNumRebuys() + player.getNumRebuysPending()) < nMax;
    }
