...
public class ObserversDash extends DashboardItem {
        ...
        if (table.getNumObservers() == 0) {
            ...
            for (int i = 0; i < table.getNumObservers(); i++) {
                ...
                if (player.getName() == null)
                    continue; // could happen during load, b4 player object loaded
    ...
    private class DashObs extends DDLabel implements MouseListener {
        ...
        DashObs(int num, PokerPlayer p) {
            ...
            if (p != null) {
                setText(PropertyConfig.getMessage(p.isWaiting() ? "msg.observers.name.wait" : "msg.observers.name", num,
                        Utils.encodeHTML(p.getName())));
            } else {
        ...
        public void mouseReleased(MouseEvent e) {
            if (!GuiUtils.isPopupTrigger(e, false) || p == null)
                return;
