...
public class PokerMain extends GameEngine
        ...
        switch (event.getType()) {
            ...
            case RECEIVED :
                ...
                if (data.getType() == UDPData.Type.MESSAGE) {
                    ...
                    if (TESTING(EngineConstants.TESTING_UDP_APP))
                        logger.debug("POKER msg ({}) from {}: {}", data.getType(), link.toStringNameIP(),
                                data.toStringShort());
                }
                break;
        }
    ...
    protected void notifyTimeout() {
        if (chatHandler_ != null) {
            String sInfo = PropertyConfig.getMessage(!udp_.isBound()
                    ? "msg.chat.lobby.notbound"
                    : udp_.getChatLink().isEstablished() ? "msg.chat.lobby.lost" : "msg.chat.lobby.timeout");
