...
public class HandInfo implements Comparable<HandInfo> {
    ...
    private void categorize() {
        ...
        if (nNumRank_ == null)
            nNumRank_ = new int[Card.ACE + 1];
        else
            Arrays.fill(nNumRank_, 0);
        ...
        if (seqFlush_ == null)
            seqFlush_ = new ArrayList<>();
        else
            seqFlush_.clear();

        if (all_ == null)
            all_ = new HandSorted(hand_);
        else {
            all_.clear();
            all_.addAll(hand_);
        }
        ...
        for (int i = all_.size() - 1; i >= 0; i--) {
            ...
            for (Hand cards : seqFlush_) {
                ...
                if (seq.isEmpty())
                    continue;
                if (seq.getCard(0).getRank() == Card.TWO && seq.getCard(0).isSameSuit(c)) {
                    seq.insertCard(c);
                }
            }
        }
    ...
    /**
     * For determining straight flush runs, we need to get the last seq for the suit
     */
    ...
    private boolean hasFullHouse() {
        ...
        if ((nTrips_ > 0 && nPairs_ > 0) || (nTrips_ > 1)) {
            best_.clear();
            Card card;
            Hand dup = new Hand(all_);
            int topset = 0;
    ...
    /**
     * does this hand have two pair (returns true if num pairs is > 2)
     */
     ...
     * hand type (e.g., one of the cards in a pair, one in trips, etc.). Strict two
     * pair means both hole cards must be used. Otherwise, only one of the two pair
     * must match.
     */
    @SuppressWarnings("DuplicateBranchesInSwitch")
        ...
        switch (nType) {
            ...
            case HandInfo.STRAIGHT :
                return hole.isInHand(cards[0]) || hole.isInHand(cards[0] - 1) || hole.isInHand(cards[0] - 2)
                        || hole.isInHand(cards[0] - 3) || hole.isInHand(cards[0] > 5 ? cards[0] - 4 : Card.ACE); // BUG
                                                                                                                    // 316
                                                                                                                    // -
                                                                                                                    // ace
                                                                                                                    // low
    ...
    /**
     * assuming the given hand is a flush, return whether the hole cards represent a
     * nut flush (nCards == 1). If nCards > 1, then this return true if this hand
     * contains the 1st/2nd best flush (2), 1st/2nd/3rd best flush (3), etc.
     */
    ...
    public static void main(String[] args) {
        ...
        test("Royal Flush (clubs)        ", Card.CLUBS_A, Card.CLUBS_J, Card.CLUBS_K, Card.CLUBS_Q, Card.CLUBS_T,
                Card.SPADES_2, Card.HEARTS_K);
        test("Straight Flush (hearts)    ", Card.HEARTS_9, Card.HEARTS_J, Card.HEARTS_K, Card.HEARTS_Q, Card.CLUBS_Q,
                Card.HEARTS_T, Card.HEARTS_3);
        test("A straight, K str flush    ", Card.HEARTS_9, Card.HEARTS_J, Card.HEARTS_K, Card.HEARTS_Q, Card.CLUBS_Q,
                Card.HEARTS_T, Card.CLUBS_A);
        test("Flush (clubs)              ", Card.CLUBS_8, Card.CLUBS_J, Card.CLUBS_K, Card.CLUBS_Q, Card.CLUBS_T,
                Card.SPADES_2, Card.HEARTS_3);
        test("Full House (two trips)     ", Card.CLUBS_8, Card.HEARTS_8, Card.DIAMONDS_8, Card.CLUBS_Q, Card.SPADES_Q,
                Card.DIAMONDS_Q, Card.HEARTS_7);
        test("Full House                 ", Card.CLUBS_8, Card.HEARTS_8, Card.DIAMONDS_8, Card.CLUBS_Q, Card.SPADES_Q,
                Card.DIAMONDS_7, Card.HEARTS_7);
        test("Quads                      ", Card.CLUBS_8, Card.HEARTS_8, Card.DIAMONDS_8, Card.SPADES_8, Card.SPADES_7,
                Card.DIAMONDS_7, Card.HEARTS_7);
        test("Trips/FH                   ", Card.CLUBS_K, Card.HEARTS_8, Card.DIAMONDS_8, Card.SPADES_8, Card.SPADES_7,
                Card.DIAMONDS_7, Card.HEARTS_7);
        test("Trips                      ", Card.CLUBS_K, Card.HEARTS_8, Card.DIAMONDS_8, Card.SPADES_8, Card.SPADES_7,
                Card.DIAMONDS_J, Card.HEARTS_3);
        test("2 Pair/ST                  ", Card.CLUBS_8, Card.HEARTS_8, Card.DIAMONDS_6, Card.SPADES_5, Card.SPADES_7,
                Card.DIAMONDS_7, Card.HEARTS_4);
        test("Pair                       ", Card.CLUBS_8, Card.HEARTS_A, Card.DIAMONDS_6, Card.SPADES_5, Card.SPADES_K,
                Card.DIAMONDS_2, Card.HEARTS_6);
        test("High Card                  ", Card.CLUBS_8, Card.HEARTS_A, Card.DIAMONDS_6, Card.SPADES_5, Card.SPADES_K,
                Card.DIAMONDS_2, Card.HEARTS_Q);
        test("Straight Flush (low hearts)", Card.HEARTS_A, Card.HEARTS_2, Card.HEARTS_3, Card.HEARTS_4, Card.HEARTS_5,
                Card.CLUBS_A, Card.SPADES_A);
        test("Straight Flush (+1 hearts) ", Card.HEARTS_A, Card.HEARTS_2, Card.HEARTS_3, Card.HEARTS_4, Card.HEARTS_5,
                Card.HEARTS_6, Card.SPADES_A);
        test("Straight (5 high)          ", Card.CLUBS_2, Card.HEARTS_A, Card.HEARTS_3, Card.SPADES_4, Card.DIAMONDS_5,
                Card.CLUBS_A, Card.SPADES_A);
        test("Straight (6 high)          ", Card.CLUBS_2, Card.HEARTS_A, Card.HEARTS_3, Card.SPADES_4, Card.DIAMONDS_5,
                Card.CLUBS_6, Card.SPADES_A);
    }
    ...
    public String toStringDebug() {
        ...
        // suits
        sb.append("Spades: " + nSpades_ + "  Hearts: " + nHearts_ + "  Diamonds: " + nDiamonds_ + "  Clubs: " + nClubs_
                + "\n");
        ...
        for (int i = 2; i <= Card.ACE; i++) {
            if (i <= 10)
                ...
                sValue = "J";
            else if (i == Card.QUEEN)
                sValue = "Q";
            else if (i == Card.KING)
                sValue = "K";
            else
                /* if (i == Card.ACE) */ sValue = "A";
