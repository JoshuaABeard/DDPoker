...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Jul 6, 2005 Time: 8:58:58 PM To
...
public class ChipLeaderPanel extends DDTabPanel {
    ...
    public void createUI() {
        ...
        if (current.size() > 0) {
            ...
            DDScrollTable scrollCurrent = new DDScrollTable(GuiManager.DEFAULT, "OptionsDialog", "OptionsDialog",
                    PLAYING_NAMES, PLAYING_WIDTHS);
            ...
            summary.setText(PropertyConfig.getMessage("msg.tournstats", left, num, total, max, avg, min, details));
        }
        ...
        DDScrollTable scrollOut = new DDScrollTable(GuiManager.DEFAULT, "OptionsDialog", "OptionsDialog", RESULTS_NAMES,
                RESULTS_WIDTHS);
    ...
    private static final String[] PLAYING_NAMES = new String[]{COL_RANK, COL_NAME, COL_TABLE, COL_CHIPS};
    // client table info
    private static final int[] PLAYING_WIDTHS = new int[]{RW, NW, 75, CW};
    private static final String[] RESULTS_NAMES = new String[]{COL_FINISH, COL_NAME, COL_PRIZE, COL_BUYIN, COL_REBUY,
            COL_ADDON, COL_PROFIT};
    // client table info
    private static final int[] RESULTS_WIDTHS = new int[]{RW, NW, CW, CW, CW, CW, CW};
    ...
    static class PlayerModel extends DefaultTableModel {
        ...
        public Object getValueAt(int rowIndex, int colIndex) {
            ...
            if (names[colIndex].equals(COL_RANK) || names[colIndex].equals(COL_FINISH)) {
                sValue = PropertyConfig.getPlace(getRank(rowIndex));
            } else if (names[colIndex].equals(COL_SEAT)) {
                int seat = getRank(rowIndex);
                if (seat < 0)
                    sValue = PropertyConfig.getMessage("msg.table.observer");
                else
                    sValue = "" + getRank(rowIndex);
            } else if (names[colIndex].equals(COL_NAME) || names[colIndex].equals(COL_NAME_PLAYERTYPE)) {
                if (p == null)
                    return "";
                ...
                // BUG 404 - could be null if window left up after players go out
                    ...
                    sValue = p.getTable().getName();
            }

            if (p != null && p.isLocallyControlled() && p.isHuman()) {
                if (names[colIndex].equals(COL_NAME) || names[colIndex].equals(COL_NAME_PLAYERTYPE))
                    sValue = Utils.encodeHTML(sValue);
        ...
        private String getNumber(int nNum) {
            if (nNum < 0) {
                return "<HTML>" + PropertyConfig.getMessage("msg.chip.net.neg", -1 * nNum);
            } else {
                return "<HTML>" + PropertyConfig.getMessage("msg.chip.net.pos", nNum);
            }
        }
    }
}
