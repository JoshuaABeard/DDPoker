...
public class V2Player extends V1Player implements AIConstants {
    ...
    public void dealtFlop() {
        ...
        if (maPotRaised_ != maOld_) {
            // logger.info("Pre-flop raise metric changed from " + maOld_ + " to " +
            // maPotRaised_);
        }
    }
    ...
    public void computeOdds() {
        ...
        // if pocket is null, this player was not in this hand; probably moved from
        // another table
        if (hole == null) {
            return;
        }
        ...
        if ((fpPocket_ == hole.fingerprint()) && (fpCommunity_ == community.fingerprint())) {
            return;
        }
    ...
    private void _computeOdds(Hand hole, Hand community) {
        ...
        /*
         * int aa = 0; int at = 0; int ab = 0; int ta = 0; int tt = 0; int tb = 0; int
         * ba = 0; int bt = 0; int bb = 0;
         */
    ...
    public float getEffectiveHandStrength() {
        ...
        return (float) Math.pow(handStrength + (1 - handStrength) * getPositiveHandPotential(),
    ...
    private void computeBiasedPotential() {
        ...
        // System.out.println("BPP for " + getPokerPlayer().getName() + ":" +
        // biasedPositivePotential_);
        // System.out.println("BNP for " + getPokerPlayer().getName() + ":" +
        // biasedNegativePotential_);
    }

    public float getBiasedHandStrength() {
        computeOdds();

        if (biasedHandStrength_ >= 0.0)
        ...
        for (int i = 50; i >= 0; --i) {
            ...
            for (int j = 51; j > i; --j) {
                ...
                for (int p = hhand.getNumPlayers() - 1; p >= 0; --p) {
                    ...
                    float percentPaid = opponent.getOpponentModel().handsPaid.getPercentTrue(0.30f);
                     ...
                     *
                     * if (couldLimp[p]) { float percentLimped =
                     * handsLimped_[seat].getPercentTrue(percentPaid); float percentRaised =
                     * (float)Math.max(1.0f - percentLimped -
                     * handsFoldedUnraised_[seat].getPercentTrue(0.60f), 0.0f);
                     *
                     * int limpIndex = (int)((percentLimped + percentRaised) * 100.0f + 4) / 10; int
                     * raiseIndex = (int)(percentRaised * 100.0f + 4) / 10;
                     *
                     * if (limped[p]) { // allow for stronger hands if limped then called a raise if
                     * (wasRaisedPreFlop) { raiseIndex = Math.max(raiseIndex-1, 0); } weight =
                     * (simpleBias_[limpIndex][x][y] - simpleBias_[raiseIndex][x][y]) / 1000.0f; }
                     * else { // first raiser weight = simpleBias_[raiseIndex][x][y] / 1000.0f; } }
                     * else if (paid[p]) { int bhsIndex = (int)(percentPaid * 100.0f + 4) / 10;
                     *
                     * // allow for stronger hands if player called a raise if (wasRaisedPreFlop) {
                     * bhsIndex = Math.max(bhsIndex-2, 0); }
                     *
                     * weight = simpleBias_[bhsIndex][x][y] / 1000.0f; } else { // didn't pay, so
                     * could have any hand weight = 1.0f; }
                     */

                    // weight =
                    // getOpponentModel(seat).getHandProbabilityMatrix().getProbability(card1,
                    // card2);
         ...
         * float bhsoa = 1.0f;
         *
         * for (int seat = 0; seat < 10; ++seat) { if (seat == skip) continue;
         *
         * PokerPlayer opponent = player.getTable().getPlayer(seat);
         *
        ...
        for (int i = 51; i >= 0; --i) {
            ...
            for (int j = 51; j > i; --j) {
                ...
                for (int p = 0; p < numPlayers; ++p) {
                    ...
                    if (couldLimp[p]) {
                        float percentLimped = opponent.getOpponentModel().handsLimped.getPercentTrue(percentPaid);
                        float percentRaised = Math.max(1.0f - percentLimped
                                - opponent.getOpponentModel().handsFoldedUnraised.getPercentTrue(0.60f), 0.0f);
    ...
    public float getRemainingAverageHohM() {
        ...
        for (int i = 0; i < numPlayers; ++i) {
            ...
            if (opponent.isFolded() || opponent.isAllIn() || (opponent == player))
                continue;

            float m = (float) opponent.getChipCountAtStart() / (float) mDivisor;
    ...
    public float computeBadBeatScore(HoldemHand hhand, PokerPlayer player) {
         ...
         * Bad-Beat-o-Meter
         *
         * Just how bad was that beat? The Pokersavvy.com Bad-Beat-o-Meter has the
         * answers. Click on the cards to enter the favored starting hand, the eventual
         * winner, and the entire board. Also make sure to give the blinds and the total
         * pot size on each street. NOTE: do not enter the amount that went in on each
         * betting round, but the total pot size. For example, if the betting was
         * finished pre-flop because a player was all-in, the pot size would be the same
         * for each betting round.
         *
         * Launch the Bad-Beat-o-Meter
         *
         * How does it work? We've created an algorithm that determines how big a
         * favorite a hand was on each street and multiplies that by the amount of money
         * that went in on each street. So the more money you get in as a more
         * prohibitive favorite, the higher your score. The equation subtracts points
         * for money put into the pot as an underdog. The full equation is as follows:
         *
         * (pre flop percentage favorite-50 * pot size pre flop) + (on flop percentage
         * favorite-50 * money added to pot on flop) + (on turn percentage favorite-50 *
         * money added to pot on turn) - money added to pot on river
         ...
         *
         * MODIFICATIONS: - CONSIDER ANTES AS WELL - NO PENALTY FOR BETS WHEN TRAILING
         */
        ...
        float score = (float) ((Math.max(preflop.getWinOrTiePercent() - 50d, 0) * preflopBets
    ...
    private void loadReputation() {
        if (getPokerPlayer().getTable().getGame().isOnlineGame())
            return;
