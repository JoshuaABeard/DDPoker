...
public class OnlineManagerQueue implements Runnable {
    ...
    /**
     * Debug message about Q - print each player with messages and number of
     * good/bad messages
     */
    private void debugQ() {
        ...
        for (int i = 0; i < qsize; i++) {
            ...
            if (q.to.getConnection() != null && q.to.getConnection().equals(q.connection))
                q.to.nGoodMsgs_++;
            else
                q.to.nBadMsgs_++;
        }
        ...
        for (int i = 0; i < nNumP; i++) {
            ...
            if (p.nBadMsgs_ > 0 || p.nGoodMsgs_ > 0) {
                logger.info(
                        "Queued messages for " + p.getName() + ":  good=" + p.nGoodMsgs_ + ",  stale=" + p.nBadMsgs_);
            }
        }
        ...
        for (int i = 0; i < nNumO; i++) {
            p = game.getPokerObserverAt(i);
            if (p.nBadMsgs_ > 0 || p.nGoodMsgs_ > 0) {
                logger.info("Queued messages for " + p.getName() + " [obs]:  good=" + p.nGoodMsgs_ + ",  stale="
                        + p.nBadMsgs_);
            }
        }
    }
    ...
    private void sendMessages(ArrayList list) {
        ...
        if (nNum >= 50)
            logger.debug("OnlineMessageQueue: " + nNum + " messages to process");
    ...
    private boolean processMessage(Qentry q) {
        ...
        if (sc == null || !(sc.equals(q.connection))) {
            ...
            if (sc == null)
                logger.warn("Socket now null, skipping message to " + pTo.getName() + ": " + omsg.toStringCategory());
            else
                logger.warn("Socket changed, skipping message to " + pTo.getName() + ": " + omsg.toStringCategory());
            return true;
        }
        ...
        if (worker == null) {
            ...
            if (nElapsedNoWorkerTime_ >= LOG_UNAVAIL) {
                logger.warn("*** NO worker thread available for " + nElapsedNoWorkerTime_ + " millis " + "(sleep is "
                        + SLEEP_UNAVAIL + "), current player=" + q.to.getName());
    ...
    private void sendMessage(Qentry q) {
        ...
        // synchronize for case of immediate sends, so we only send one message at a
        // time to a player
        synchronized (pTo.getSendSync()) {
            ...
            if (connection == null || !(connection.equals(q.connection))) {
                if (connection == null)
                    logger.warn(
                            "Socket now null, skipping message to " + pTo.getName() + ": " + omsg.toStringCategory());
                else
                    logger.warn(
                            "Socket changed, skipping message to " + pTo.getName() + ": " + omsg.toStringCategory());
                return;
            }
            ...
            if (OnlineManager.DEBUG) {
                ...
                if (OnlineManager.DEBUG || omsg.getCategory() != OnlineMessage.CAT_CHAT) {
                    logger.debug(
                            OnlineManager.SCNT + " sending to " + pTo.getName() + ": " + omsg.toStringCategorySize());
                }
            }
            ...
            try {
                ...
                // logger.debug("Wrote " + nBytes + " bytes to " + pTo.getName() + ", data: " +
                // omsg.toStringNoData());

                if (DebugConfig.isTestingOn() && omsg.getCategory() == OnlineMessage.CAT_CHAT
                        && omsg.getChat().startsWith("disconnect test")) {
                    throw new TestException();
                }
            } catch (IOException ioe) {
                if (ioe instanceof TestException) {
                    logger.warn("Test exception caught sending message to: " + pTo.getName() + ": "
                            + omsg.toStringNoData());
                } else if (ioe instanceof SocketTimeoutException) {
                    logger.warn("Timeout sending message to: " + pTo.getName() + ": " + ioe.getMessage());
                    ...
                    if (sMsg.indexOf("forcibly") != -1) {
                        ...
                        logger.warn("Error sending message to: " + pTo.getName() + ": " + omsg.toStringNoData() + ";  "
                                + Utils.formatExceptionText(ioe) + "\nOriginated at: "
                                + Utils.formatExceptionText(q.source));
                    }
                }
