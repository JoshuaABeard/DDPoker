...
public class SimulatorDialog extends BasePhase implements ChangeListener {
    ...
    /**
     * get game - we get game from current context since it can change from
     * underneath us
     */
    ...
    private static void replaceCard(PokerTable table, Card replaceThis, Hand inThisHand, Card withThis,
        ...
        // repaint
        if (repaintThis != null)
            repaintThis.updateDisplay(true);
    }
    ...
    private void updateDisplay(boolean bCardsChanged) {
        ...
        if (simulations_.isSelectedTab())
            simulations_.updateStats(pocket, community);
        if (handstrength_.isSelectedTab())
            handstrength_.updateStats(pocket, community);
        if (flopstats_.isSelectedTab())
            flopstats_.updateStats(pocket, community);
        if (turnstats_.isSelectedTab())
     ...
     * actual popup menu
     */
    private static DDPopupMenu selectCard(Component parent, Point point, CardPiece cardPiece, ActionListener listener,
            SimulatorDialog sim) {
        ...
        if (sim.selector_ == null) {
            ...
            if (sim.last_ != null)
                sim.selector_.removeActionListener(sim.last_);
        ...
        if (game != null && !game.isClockMode()) {
            ...
            if (!game.isOnlineGame()) {
                ...
                if (sim.bShowAllAi_)
                    showallai.setIcon(checkedIcon_);
                menu.add(showallai);
            }
        }
        ...
        if (nType == MENU_CLEAR_ALL) {
            ...
            for (int i = 0; i < nNum; i++) {
                ...
                if (p != null && (p == local || !p.isFolded())) {
                    ...
                    if (bShowAllAi_ && !game.isOnlineGame())
                        bShowBlank = false;
                    if (p.isCardsExposed())
                        bShowBlank = false;
                ...
                if (nu != null) {
                    ...
                    if (i == 0 && nType == MENU_CHANGE_SHOWALL_AI)
                        continue;
            ...
            if (nType == MENU_LOAD_ALL)
                showdown_.setNumOpponents(nUpdateIndex - 1);
