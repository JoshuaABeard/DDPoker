...
public class HandStrength {
    ...
    /**
     * return number of straights made during most recent call to getStrength
     */
    ...
    /**
     * Calculate strength of hand against 1 opponents. Return is float from 0 to 1
    ...
    public float getStrength(Hand hole, Hand community) {
        ...
        if (DEBUG && TESTING(EngineConstants.TESTING_AI_DEBUG)) {
            logger.debug("STRENGTH for " + hole + "," + community + ": " + nAhead + " wins   " + nTied + " ties   "
                    + nBehind + " lose   " + HandStat.fPerc.form(nStrength));
        }
        return nStrength;
    }
    ...
    public String toHTML(Hand hole, Hand community, int nOpponents) {
        ...
        for (int i = 1; i <= nOpponents; i++) {
            ...
            sb.append(PropertyConfig.getMessage("msg.strength.row", i, HandStat.fPerc.form(oppStrength)));
        }
