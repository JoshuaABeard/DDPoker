...
public class HandLadder {
                ...
                if (opponentScore == handScore_) {
                    ...
                    switch (handType_) {
                        ...
                        case HandInfo.STRAIGHT :
                            ladderCategory = (opponentScore > handScore_)
                                    ? SAME_TYPE_HIGHER_RANK
                                    : SAME_TYPE_LOWER_RANK;
                            break;
                        case HandInfo.QUADS :
                        ...
                        case HandInfo.PAIR :
                            ladderCategory = ((handInfo_.getBigPairRank() == opponentInfo.getBigPairRank())
                                    && (handInfo_.getSmallPairRank() == opponentInfo.getSmallPairRank()))
                                            ? ((opponentScore > handScore_)
                                                    ? SAME_HAND_HIGHER_KICKER
                                                    : SAME_HAND_LOWER_KICKER)
                                            : ((opponentScore > handScore_)
                                                    ? SAME_TYPE_HIGHER_RANK
                                                    : SAME_TYPE_LOWER_RANK);
                            break;
    ...
    public int getHandRank() {
        return ladder_[HIGHER_TYPE].size() + ladder_[SAME_TYPE_HIGHER_RANK].size()
    ...
    public String toHTML() {
        ...
        if (getHandRank() > 1) {
            buf.append("<tr>");
            buf.append("<td><font color=\"white\"><b>Stronger Hands - " + PokerConstants.formatPercent(strongerPercent)
                    + "%</b></font></td>");
            ...
            if (ladder_[HIGHER_TYPE].size() > 0) {
                for (int i = HandInfo.ROYAL_FLUSH; i > handType_; --i) {
                    if (countByType_[i] > 0) {
                         ...
                         * buf.append(strongerHandsByType_[i].toHTML()); buf.append("</td></tr>");
                         */
                    }
                }
            }
        ...
        if (getHandRank() < totalCount_) {
            buf.append("<tr>");
            buf.append("<td><font color=\"white\"><b>Equal or Weaker Hands - "
                    + PokerConstants.formatPercent(lessEqualPercent) + "%</b></font></td>");
            buf.append("<td align=\"right\"><font color=\"white\"><b>Count</b></font></td>");
            buf.append("<td align=\"right\"><font color=\"white\"><b>Probability</b></font></td>");
            buf.append("</tr>");
            ...
            if (ladder_[SAME_HAND].size() > 0) {
                ...
                /*
                 * buf.append("<tr><td colspan=\"3\">");
                 * buf.append(ladder_[SAME_HAND].toHTML()); buf.append("</td></tr>");
                 */
            }
            ...
            if (ladder_[SAME_TYPE_LOWER_RANK].size() > 0) {
                ...
                /*
                 * buf.append("<tr><td colspan=\"3\">");
                 * buf.append(ladder_[SAME_TYPE_LOWER_RANK].toHTML()); buf.append("</td></tr>");
                 */
            }
            ...
            if (ladder_[LOWER_TYPE].size() > 0) {
                for (int i = handType_ - 1; i >= HandInfo.HIGH_CARD; --i) {
                    if (countByType_[i] > 0) {
                        ...
                        /*
                         * buf.append("<tr><td colspan=\"3\">");
                         * buf.append(weakerHandsByType_[i].toHTML()); buf.append("</td></tr>");
                         */
                    }
                }
            }
