...
public class DealDisplay extends ChainPhase implements Runnable {
    ...
    public void process() {
        ...
        if (game_.isOnlineGame() && bHighCard_) {
            ...
            if (!player.isHost() && !player.isObserver() && player.isSittingOut()) {
                ...
                GameButton bPlay = EngineUtils.displayConfirmationDialogCustom(context_, "OnlineGuestStart", null, null,
                        null, null);
        ...
        if (!TESTING(PokerConstants.TESTING_AUTOPILOT_INIT) && bHighCard_ && table_.getHandNum() == 0
                && !game_.isOnlineGame()) {
            String sMsg = PropertyConfig.getMessage("msg.highcardbutton");
            EngineUtils.displayInformationDialog(context_, Utils.fixHtmlTextFor15(sMsg), "msg.highcard.title",
                    "DealHighCard");
        }

        // show intro message on hand 5
        if (!TESTING(PokerConstants.TESTING_AUTOPILOT_INIT) && !bHighCard_ && !bColor_ && table_.getHandNum() == 5
                && !game_.isOnlineGame()) {
            EngineUtils.displayInformationDialog(context_, PropertyConfig.getMessage("msg.intro.info"),
                    "msg.intro.title", "introinfo");
        }

        // show option message on hand 10
        if (table_.getHandNum() == 10 && !TESTING(PokerConstants.TESTING_AUTOPILOT) && !game_.isOnlineGame()) {
            EngineUtils.displayInformationDialog(context_, PropertyConfig.getMessage("msg.hint.option.info"),
                    "msg.hint.option.title", "hintoption");
        }
        ...
        if (game_.isOnlineGame() && game_.getLocalPlayer().isObserver())
            nCardDelay_ = 0;
        ...
        if (td_ != null && !bHighCard_ && !bColor_) {
            // new deal
            td_.sendDealerChatLocal(PokerConstants.CHAT_1, PokerUtils.chatImportant(
                    PropertyConfig.getMessage("msg.chat.newdeal", table_.getHandNum(), table_.getNumber())));

            // blinds
            HoldemHand hhand = table_.getHoldemHand();
            List<HandAction> hist = hhand.getHistoryCopy();
            for (HandAction blind : hist) {
                if (blind.getAction() == HandAction.ACTION_ANTE || blind.getAction() == HandAction.ACTION_BLIND_SM
    ...
    private boolean showCard(int c) {
        int nSeat = 0;
        if (bHighCard_ || bColor_)
            nSeat = table_.getNextSeat(-1);
        else
            nSeat = table_.getNextSeatAfterButton();
