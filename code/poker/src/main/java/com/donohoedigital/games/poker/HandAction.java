...
public class HandAction implements DataMarshal {
    ...
    /**
     * Get sub amount (used for call portion of a raise, pot number in win/overbet,
     * and fold type )
     */
    ...
    public static int decodeActionType(String code) {
        ...
        if (code.equals("CALL"))
            return ACTION_CALL;
        if (code.equals("RAISE"))
            return ACTION_RAISE;
        if (code.equals("SMALL"))
            return ACTION_BLIND_SM;
        if (code.equals("BIG"))
            return ACTION_BLIND_BIG;
        if (code.equals("ANTE"))
            return ACTION_ANTE;
        if (code.equals("WIN"))
            return ACTION_WIN;
        if (code.equals("OVER"))
            return ACTION_OVERBET;
        if (code.equals("LOSE"))
            return ACTION_LOSE;
    ...
    public String toString(boolean bShort) {
        ...
        switch (nRound_) {
            case ClientBettingRound.ROUND_PRE_FLOP :
                sRound = " deal";
                break;
    ...
    /**
     * Get HTML snippet of action. This returns something that needs to go inside a
     * <TD></TD> block.
     */
    public String getHTMLSnippet(String sPropertyName, int nPrior, Object[] extraParams) {
        ...
        if (nAction == HandAction.ACTION_RAISE) {
            ...
            if (getSubAmount() == FOLD_FORCED)
                sExtraKey = ".forced";
            else if (getSubAmount() == FOLD_SITTING_OUT)
                sExtraKey = ".sitout";
        }
        ...
        return PropertyConfig
                .getMessage(isAllIn() ? sPropertyName + ".allin" : sPropertyName + "." + nAction + sExtraKey, params);
    }
