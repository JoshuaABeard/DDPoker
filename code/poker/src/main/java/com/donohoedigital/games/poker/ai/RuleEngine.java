...
public class RuleEngine implements AIConstants {
    ...
    private void execPostFlop() {
                ...
                if (nAmountToCall_ == 0) {
                    ...
                    if (probableStrength_ > 0.5f) {
                        ...
                        if (!isEligible(OUTCOME_CONTINUATION_BET) && !bOpponentPotCommitted
                                && (self_.getChipCount() > hhand_.getTotalPotChipCount() / 2)) {
        ...
        ai_.appendDebug("Bet amount: " + betAmount + " (" + PokerConstants.formatPercent(amountRatio * 100)
                + "% of the pot).<br>");
         ...
         *
         * for (int i = 0; i < score_.length; ++i) if (eligible_[i]) { if (count == 0) {
         * ai_.appendDebug("Recommend "); } else { ai_.appendDebug("or "); }
         *
         * ai_.appendDebug(getOutcomeLabel(i)); ai_.appendDebug(" "); ++count; }
         */
        ai_.appendDebug("<br>");
        ...
        // {
        // td.deliverChatLocal
        // (PokerConstants.CHAT_1, PokerUtils.chatInformation(ai_.getDebugText()),
        // self_.getID());
        // }
        // }
        // return;
        ...
        float xStraightDraw = ai.getStratFactor("draws.straight.nut", -1.0f, 1.0f) * pNutStraight
                + ai.getStratFactor("draws.straight.non_nut", -1.0f, 1.0f) * 0.5f * pNonNutStraight;

        float xFlushDraw = ai.getStratFactor("draws.flush.nut", -1.0f, 1.0f) * pNutFlush
         ...
         *
         * for (int i = hhand.getNumPlayers()-1; i >= 0; --i) { PokerPlayer opponent =
        ...
        if (potStatus == PokerConstants.NO_POT_ACTION) {
            ...
            for (int i = hhand_.getNumPlayers() - 1; i >= 0; --i) {
                ...
                if (p.getSeat() == ai.getSeatNumber() - 1)
                    continue;
                if (p.isFolded())
                    continue;
                if (hhand_.getLastActionThisRound(p) == HandAction.ACTION_CHECK)
                    continue;
            ...
            if (!NEWCODE)
                adjustOutcome(OUTCOME_BET, FACTOR_LEFT_TO_ACT, -0.08f * ai.getNumAfter() * xBasicsPosition);
            ...
            if ((getStrongestOutcome() == OUTCOME_BET) && (ai.getNumAfter() > 0)) {
                ...
                adjustOutcome(OUTCOME_CHECK_RAISE, FACTOR_LEFT_TO_ACT,
                        0.06f * ai.getNumAfter() * xBasicsPosition * (1.0f - allWeak));
                ...
                // TODO: don't make continuation bets when we have good draws and would rather
                // see a free card
                // TODO: don't make continuation bets into very scary flops
                // e.g. AhKhJd when holding 88
                /*
                 * community.hasPair(); community.containsRank(Card.ACE);
                 * community.containsRank(Card.KING); community.containsRank(Card.QUEEN);
                ...
                if (ai.wasLastRaiserPreFlop() && (numWithCards < 4) && (round_ == BettingRound.FLOP.toLegacy())) {
                    ...
                    adjustOutcome(OUTCOME_CONTINUATION_BET, FACTOR_RAW_HAND_STRENGTH,
                            (NEWCODE ? rhs : rhs * rhs + 0.05f));
                    // adjustOutcome(OUTCOME_CONTINUATION_BET, FACTOR_BIASED_HAND_STRENGTH, bhs -
                    // rhs);
                    adjustOutcome(OUTCOME_CONTINUATION_BET, FACTOR_HAND_POTENTIAL,
                            (NEWCODE ? drawPotential : drawPotential - 0.01f * draws));
                    if (NEWCODE)
        ...
        } else // pot action already
        {
            ...
            switch (round_) {
                case HoldemHand.ROUND_FLOP :
                    bettorActFrequency = bettor.getOpponentModel().actFlop.getWeightedPercentTrue(0.1f);
                    bettorCheckFoldFrequency = bettorModel.checkFoldFlop.getWeightedPercentTrue(0.5f)
                            * bettorActFrequency;
                    bettorOpenFrequency = bettorModel.openFlop.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    bettorRaiseFrequency = bettorModel.raiseFlop.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    break;
                case HoldemHand.ROUND_TURN :
                    bettorActFrequency = bettorModel.actTurn.getWeightedPercentTrue(0.1f);
                    bettorCheckFoldFrequency = bettorModel.checkFoldTurn.getWeightedPercentTrue(0.5f)
                            * bettorActFrequency;
                    bettorOpenFrequency = bettorModel.openTurn.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    bettorRaiseFrequency = bettorModel.raiseTurn.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    break;
                case HoldemHand.ROUND_RIVER :
                    bettorActFrequency = bettorModel.actRiver.getWeightedPercentTrue(0.1f);
                    bettorCheckFoldFrequency = bettorModel.checkFoldRiver.getWeightedPercentTrue(0.5f)
                            * bettorActFrequency;
                    bettorOpenFrequency = bettorModel.openRiver.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    bettorRaiseFrequency = bettorModel.raiseRiver.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    break;
            }
            ...
            // base hand strength scores
            adjustOutcome(OUTCOME_CALL, FACTOR_RAW_HAND_STRENGTH,
                    (NEWCODE ? 0.85f - Math.abs(rhs - 0.85f) : xrhs + 0.10f));
            ...
            if (round_ < BettingRound.RIVER.toLegacy()) {
                if (NEWCODE) {
                    ...
                    adjustOutcome(OUTCOME_CALL, FACTOR_POT_ODDS,
                ...
                if (NEWCODE) {
                    adjustOutcome(OUTCOME_CALL, FACTOR_POT_ODDS,
                            (1.0f - bhs) * bhs / breakEvenPercentage * xBasicsPotOdds);
                } else {
        ...
        /*
         * if (matrix) { float xxx = ehsA;
         *
         * //System.out.println(xxx); betRange_ = new BetRange(BetRange.ALL_IN); if (xxx
         * < 0.60) strongestOutcome_ = OUTCOME_FOLD; else if (xxx < 0.70)
         * strongestOutcome_ = OUTCOME_CHECK_RAISE; else if (xxx < 0.80)
         * strongestOutcome_ = OUTCOME_LIMP; else if (xxx < 0.85) strongestOutcome_ =
         * OUTCOME_CALL; else if (xxx < 0.95) strongestOutcome_ = OUTCOME_STEAL; else
         * strongestOutcome_ = OUTCOME_BET;
         *
         * return; }
         *
         * float xTournamentStackSize = ai.getStratFactor("tournament.stack_size", 0.0f,
         * 2.0f); float xTournamentOpponentStackSize =
         * ai.getStratFactor("tournament.opponent_stack_size", 0.0f, 2.0f);
         *
         * int position = ai.getPostFlopPositionCategory(); float adjPosition = 0.0f;
         *
         * int nAfter = ai.getPlayersLeftToAct(); int nBefore = ai.getPlayersBefore();
         *
         * adjPosition = nBefore * 0.05d - nAfter * 0.1d;
         *
         * switch (position) { case PokerAI.POSITION_EARLY: adjPosition = -0.1d *
         * xBasicsPosition; break; case PokerAI.POSITION_MIDDLE: adjPosition = 0.0f *
         * xBasicsPosition; break; case PokerAI.POSITION_LATE: adjPosition = 0.1d *
         * xBasicsPosition; break; case PokerAI.POSITION_LAST: adjPosition = 0.2d *
         * xBasicsPosition; break; }
         *
         ...
         *
         * if (ai.isPotReraised()) { adjustOutcome(OUTCOME_RAISE, FACTOR_POT_RERAISED,
         * (bhs-1.0f)); } else if (ai.isPotRaised()) { adjustOutcome(OUTCOME_RAISE,
         * FACTOR_POT_RAISED, -0.4d * amountToCall / ai.getTotalPotAmount());
         * //adjustOutcome(OUTCOME_CALL, FACTOR_POT_ODDS, xBasicsPotOdds *
         * Math.max(potOdds - 1.0f, 0.0f) * 0.1); }
         */
    }
    ...
    private void determineEligibleOutcomes(V2Player player) {
        ...
        if (player.isPreFlop()) {
            ...
            // setEligible(OUTCOME_CHECK_RAISE, (amountToCall == 0) && (playerChips >
            // minBet));
        }
    }
    ...
    private void executePreFlop(V2Player player) {
        ...
        // float xBasicsTightness = player.getStratFactor("basics.tightness", -0.8f,
        // +0.8f);
        ...
        /*
         * float zoneLoosenUp = 0.0f; float zoneAggression = 0.0f; float
         * zoneFirstInMultiplier = 1.0f; float zoneConserveStack = 0.0f;
         *
         * // basic zone stuff switch (hohZone) { case V2Player.HOH_GREEN: break; case
         * V2Player.HOH_YELLOW: zoneLoosenUp = 0.1d; zoneAggression = 0.1d;
         * zoneFirstInMultiplier = 1.5f; zoneConserveStack = 0.1d; break; case
         * V2Player.HOH_ORANGE: zoneLoosenUp = 0.2d; zoneAggression = 0.2d;
         * zoneFirstInMultiplier = 2.0f; zoneConserveStack = 0.2d; break; case
         * V2Player.HOH_RED: zoneLoosenUp = 0.3d; zoneAggression = 0.3d;
         * zoneFirstInMultiplier = 2.5f; zoneConserveStack = 0.3d; break; case
         * V2Player.HOH_DEAD: zoneLoosenUp = 0.4d; zoneAggression = 0.4d;
         * zoneFirstInMultiplier = 3.0f; zoneConserveStack = 0.4d; break; }
         */
        ...
        for (int i = 0; i < PokerConstants.SEATS; ++i) {
            ...
            if ((p != null) && (p.getSeat() != player.getPokerPlayer().getSeat()) && !p.isFolded()
                    && (p.getHand() != null)) {
        ...
        if (player.debugEnabled()) {
             ...
             * player.appendDebug("Zone First In Multiplier: " + zoneFirstInMultiplier +
             * "<br>"); player.appendDebug("Zone Conserve Stack Offset: " +
             * zoneConserveStack + "<br>");
             */
        ...
        if ((hohZone == AIConstants.HOH_RED) || (hohZone == AIConstants.HOH_DEAD)) {
            ...
            if ((potStatus == PokerConstants.CALLED_POT) && (player.getNumLimpers() > 1)) {
                ...
                // adjustOutcome(OUTCOME_RAISE, FACTOR_PLAYERS_DEALT, 0.1d * (PokerTable.SEATS -
                // numPlayers) * adjustedHandStrength);
            ...
            if (potStatus == PokerConstants.RAISED_POT) {
                ...
                /*
                 * if (adjustedHandStrength < 0.6f) { adjustOutcome(OUTCOME_FOLD,
                 * FACTOR_HAND_SELECTION, 1.0f); return; }
                 */
                ...
                float raiserFrequency = (float) Math
                        .max(raiser.getOpponentModel().handsRaisedPreFlop.getWeightedPercentTrue(0.1f) - 0.1, 0.0);
            ...
            if ((getStrongestOutcome() != OUTCOME_OPEN_POT) && (potStatus == PokerConstants.NO_POT_ACTION)
                ...
                switch (startingPosition) {
                    ...
                    case PokerAI.POSITION_LATE :
                        adjustOutcome(OUTCOME_STEAL, FACTOR_POSITION,
                                (player.isButton() ? 0.2f : 0.1f) * xBasicsPosition);
                        break;
                    case PokerAI.POSITION_SMALL :
                ...
                float xDeceptionBluffStealBlinds = player.getStratFactor("deception.bluff.steal_blinds", 0.0f, 2.0f);
         ...
         *
         * if (stealSuspicion_ > 0.0f) { adjustOutcome(OUTCOME_CALL,
         * FACTOR_STEAL_SUSPECTED, stealSuspicion_); adjustOutcome(OUTCOME_RAISE,
         * FACTOR_STEAL_SUSPECTED, stealSuspicion_); }
         *
         * if (player.isPotRaised()) { adjustOutcome(OUTCOME_CALL, FACTOR_POT_RAISED,
         * xBasicsBoldness * 0.5f); adjustOutcome(OUTCOME_STEAL, FACTOR_POT_RAISED,
         * xBasicsBoldness); adjustOutcome(OUTCOME_RAISE, FACTOR_POT_RAISED,
         * xBasicsBoldness * 0.75d); }
         */

        // adjustOutcome(OUTCOME_RAISE, FACTOR_LAST_TO_ACT, nLeftToAct == 0 ? 0.05d :
        // 0.0f);
        // adjustOutcome(OUTCOME_STEAL, FACTOR_LAST_TO_ACT, nLeftToAct == 0 ? 0.1d :
        // 0.0f);

        /*
         * float xDeceptionBluffBully = player.getStratFactor("deception.bluff.bully",
         * -0.5f, 1.0f); float xDeceptionBluffBuyPot =
         * player.getStratFactor("deception.bluff.buypot", -1.0f, 1.0f);
         *
         * // less inclined to raise with large raise to call // more inclined to limp
         * with higher pot odds (more callers) // more inclined to call raise with
         * higher pot odds // less inclined to call large raise // more inclined to limp
         * when fewer players left to act // more inclined to steal when blinds are
         * small compared to stack // more inclined to steal blinds with stronger hands
         * // more inclined to steal when fewer players left to act // more inclined to
         * limp when blinds are small compared to stack // more inclined to steal when
         * big blind is short stacked // adjust for blind stealing tendency // more
         * inclined to protect blinds with stronger hands // less inclined to protect
         * blinds with large raise to call // more inclined to buy the pot with stronger
         * hands // more inclined to buy the pot when fewer players left to act // more
         * inclined to buy the pot when blinds are small compared to stack // more
         * inclined to buy the pot when big blind is short stacked // more inclined to
         * buy the pot with stronger hands // more inclined to buy the pot when fewer
         * players left to act
         */
    }
    ...
    private void executeFlopTurn(V2Player player) {
        /*
         * System.out.println("Board quality   0% for " + player.getCommunity() + " : "
         * + player.getBoardQuality(0.0)); System.out.println("Board quality  10% for "
         * + player.getCommunity() + " : " + player.getBoardQuality(0.1));
         * System.out.println("Board quality  20% for " + player.getCommunity() + " : "
         * + player.getBoardQuality(0.2)); System.out.println("Board quality  30% for "
         * + player.getCommunity() + " : " + player.getBoardQuality(0.3));
         * System.out.println("Board quality  40% for " + player.getCommunity() + " : "
         * + player.getBoardQuality(0.4)); System.out.println("Board quality  50% for "
         * + player.getCommunity() + " : " + player.getBoardQuality(0.5));
         * System.out.println("Board quality  60% for " + player.getCommunity() + " : "
         * + player.getBoardQuality(0.6)); System.out.println("Board quality  70% for "
         * + player.getCommunity() + " : " + player.getBoardQuality(0.7));
         * System.out.println("Board quality  80% for " + player.getCommunity() + " : "
         * + player.getBoardQuality(0.8)); System.out.println("Board quality  90% for "
         * + player.getCommunity() + " : " + player.getBoardQuality(0.9));
         * System.out.println("Board quality 100% for " + player.getCommunity() + " : "
         * + player.getBoardQuality(1.0));
         */
        ...
        float xStraightDraw = player.getStratFactor("draws.straight.nut", -1.0f, 1.0f) * pNutStraight
                + player.getStratFactor("draws.straight.non_nut", -1.0f, 1.0f) * 0.5f * pNonNutStraight;

        float xFlushDraw = player.getStratFactor("draws.flush.nut", -1.0f, 1.0f) * pNutFlush
        ...
        if (potStatus == PokerConstants.NO_POT_ACTION) {
            ...
            for (int i = hhand.getNumPlayers() - 1; i >= 0; --i) {
                ...
                if (p.getSeat() == player.getSeatNumber() - 1)
                    continue;
                if (p.isFolded())
                    continue;
                if (hhand.getLastActionThisRound(p) == HandAction.ACTION_CHECK)
                    continue;
            ...
            if ((getStrongestOutcome() == OUTCOME_BET) && (player.getNumAfter() > 0)) {
                ...
                // boost for players left to act
                adjustOutcome(OUTCOME_CHECK_RAISE, FACTOR_LEFT_TO_ACT,
                        0.06f * player.getNumAfter() * xBasicsPosition * (1.0f - allWeak));
        ...
        } else // pot action already
        {
            ...
            switch (round) {
                case HoldemHand.ROUND_FLOP :
                    bettorActFrequency = bettorModel.actFlop.getWeightedPercentTrue(0.1f);
                    bettorCheckFoldFrequency = bettorModel.checkFoldFlop.getWeightedPercentTrue(0.5f)
                            * bettorActFrequency;
                    bettorOpenFrequency = bettorModel.openFlop.getWeightedPercentTrue(0.5f) * bettorActFrequency;
                    bettorRaiseFrequency = bettorModel.raiseFlop.getWeightedPercentTrue(0.5f) * bettorActFrequency;
         ...
         * return; }
         *
         * float xTournamentStackSize = player.getStratFactor("tournament.stack_size",
         * 0.0f, 2.0f); float xTournamentOpponentStackSize =
         * player.getStratFactor("tournament.opponent_stack_size", 0.0f, 2.0f);
         *
         * int position = player.getPostFlopPositionCategory(); float adjPosition =
         * 0.0f;
         *
         * int nAfter = player.getPlayersLeftToAct(); int nBefore =
         * player.getPlayersBefore();
         *
         * adjPosition = nBefore * 0.05d - nAfter * 0.1d;
         *
         * switch (position) { case PokerAI.POSITION_EARLY: adjPosition = -0.1d *
         ...
         * //adjustOutcome(OUTCOME_FOLD, FACTOR_BIASED_HAND_STRENGTH, rhs-bhs);
         *
         * if (player.isPotReraised()) { adjustOutcome(OUTCOME_RAISE,
         * FACTOR_POT_RERAISED, (bhs-1.0f)); } else if (player.isPotRaised()) {
         * adjustOutcome(OUTCOME_RAISE, FACTOR_POT_RAISED, -0.4d * amountToCall /
         * player.getTotalPotAmount()); //adjustOutcome(OUTCOME_CALL, FACTOR_POT_ODDS,
         * xBasicsPotOdds * Math.max(potOdds - 1.0f, 0.0f) * 0.1); }
         */
    }
    ...
    private void adjustOutcome(int outcome, int factor, int curve, boolean invert, float weight, float min, float max,
        ...
        if (eligible_[outcome]) {
            ...
            /*
             * System.out.println("Factor " + factorNames_.get(factor) +
             * " applied to outcome " + outcomeNames_.get(outcome) + " (" + min + " - " +
             * value + " - " + max + ") x=" + adjustment.x + " curve " +
             * curveNames_.get(curve) + (invert ? " (inverted) " : "") + " f(x)=" +
             * adjustment.fx + " weight " + weight);
             */
        }
    }
    ...
    private class OutcomeAdjustment {
        ...
        public String getDisplay() {
            ...
            if (display_ == null) {
                ...
                display_ = PropertyConfig.getMessage("msg.aireason." + factorNames_.get(factor), oParams);
            }
    ...
    public String getResultsTable() {
        ...
        for (int outcome = 0; outcome < outcomeNames_.size(); ++outcome) {
            ...
            String color = (outcome == strongestOutcome_) ? "#44CC44" : (score < 0) ? "#CC4444" : "#4444CC";
    ...
    public String toHTML(PokerPlayer player, boolean bVerbose, boolean bBreak) {
        if (bVerbose) {
            ...
            if (outcome_ == null)
                return PropertyConfig.getMessage("msg.advisor.verbose", Utils.encodeHTML(playerType.getName()),
                        (bBreak ? "<br>" : " "),
                ...
                return PropertyConfig.getMessage("msg.advisor.verbose2", Utils.encodeHTML(playerType.getName()),
                        (bBreak ? "<br>" : " "), outcome_.toHTML(2));
        } else {
            if (outcome_ == null)
    ...
    public String getReasoningHTML() {
        ...
        for (int i = 0; i < outcomeNames_.size(); ++i) {
            ...
            for (index = 0; index < outcomes.size(); ++index) {
                if (score_[(Integer) outcomes.get(index)] < score_[i])
                    break;
            }
         ...
         *
         * for (int i = 0; i < outcomeNames_.size(); ++i) { if (i == strongestOutcome_)
         * continue;
         *
         ...
         *
         * buf.append("<br><br>"); buf.append(getReasoningHTML(i)); }
         */
    ...
    public String getReasoningHTML(int outcome) {
        ...
        for (int i = 0; i < adjustments.size(); i += 2) {
            ...
            String display = PropertyConfig.getMessage("msg.aireason." + factorNames_.get(factor));
