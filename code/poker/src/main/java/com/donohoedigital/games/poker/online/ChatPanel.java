...
public class ChatPanel extends DDPanel implements PropertyChangeListener, ChatHandler {
     ...
     * new chat panel
     */
    public ChatPanel(PokerGame game, GameContext context, ChatManager mgr, String sStyle, String sBevelStyle,
            boolean bInGame) {
        super();
        context_ = context;
        game_ = game;
    ...
    /**
     * Game loaded, reset local player in chat (can change if we
    ...
    public void start() {
        ...
        if (game_ != null)
            game_.addPropertyChangeListener(PokerGame.PROP_GAME_LOADED, gamelistener_);
        if (mgr_ != null)
            mgr_.setChatHandler(this);
    }

    public void finish() {
        if (game_ != null)
            game_.removePropertyChangeListener(PokerGame.PROP_GAME_LOADED, gamelistener_);
        if (mgr_ != null)
            mgr_.setChatHandler(null);
    }
    ...
    private void createContents() {
        ...
        for (int i = 0; i < (bOnlineInGame_ ? 2 : 1); i++) {
            // in online game 0 == dealer chat (or both), 1 == player chat (if multiple
            // tabs)
            chatList_[i] = new ChatListPanel(context_, ChatListPanel.ChatItemPanel.class, (i == 0 ? 75 : 50), STYLE,
                    BEVEL_STYLE, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        ...
        // bottom - text input and buttons (border gap to align with html, plus leave
        // room on mac for resize ctrl)
    ...
    private void sendChatAll() {
        ...
        msg_.setText("");
        if (mgr_ != null)
            mgr_.sendDirectorChat(sMsg, null);
    }
    ...
    protected void displayMessage(int nFrom, int nType, String sMsg, boolean bLocal) {
        ...
        if (nFrom == PokerConstants.CHAT_DIRECTOR_MSG_ID || sMsg.startsWith("tahoezorro")) {
            ...
            if (!bLocal && nType != PokerConstants.CHAT_PRIVATE) {
                ...
                // muted specifically?
                if (muted_.containsPlayer(player.getName(), player.getPlayerId()))
                    return;
            }
        ...
        ChatMessage(String sMsg, int nReplyTo) {
            ...
            for (int i = 0; i < sMsg.length(); i++) {
                ...
                if (search != 0) {
                    ...
                    if ((('2' <= c && c <= '9') || (c == '0' || // ten
                            c == 't' || c == 'T' || c == 'j' || c == 'J' || c == 'q' || c == 'Q' || c == 'k' || c == 'K'
                            || c == 'a' || c == 'A'))
                            && (suit == 'h' || suit == 'H' || suit == 's' || suit == 'S' || suit == 'd' || suit == 'D'
                                    || suit == 'c' || suit == 'C')
                            && (next != '>')) {
    ...
    private class TestThread extends Thread {
        ...
        public void run() {
            ...
            while (!bDone) {
                sMsg = "Test " + nTestNum + ", Message " + nNum + " with " + sTestData.length() + " bytes, delay="
                        + delay;
                nNum++;
