...
public class DashboardClock extends DashboardItem implements GameClockListener {
    ...
    protected JComponent createBody() {
        ...
        centering.add(base,
                new ExplicitConstraints(base, WIDTH.subtract(ComponentEF.preferredWidth(base)).multiply(.5d),
                        MathEF.constant(0), ComponentEF.preferredWidth(base), ComponentEF.preferredHeight(base)));
    ...
    private void updateLevel() {
        ...
        // ante and blinds
            ...
            if (!sGameType.isEmpty())
                sGameType = PropertyConfig.getMessage("msg.dash.gametype", sGameType);

            int nAnte = profile.getAnte(nLevel);
            int nBig = profile.getBigBlind(nLevel);
            int nSmall = profile.getSmallBlind(nLevel);
            labelBlinds_.setText(PropertyConfig.getMessage(nAnte == 0 ? "msg.dash.blinds" : "msg.dash.blinds.a", nSmall,
                    nBig, nAnte == 0 ? null : nAnte, sGameType));
        }
