...
public class ShowTournamentTable extends ShowPokerTable
        implements
            PokerTableInput,
            PokerTableListener,
...

{
    ...
    public void subclassInit(GameEngine engine, GamePhase gamephase) {
        ...
        // game buttons
        buttonSidePot_ = new PokerButton(getGameButton("sidepots"), 50 / 0.6f, 10 / 0.6f, 8.40f / 42.0f,
                582.0f / 900.0f, 5.5f / 42.0f, true);
        ...
        buttonContinueMiddle_ = new PokerButton(getGameButton("continue"), 70 / .92f, 20 / .92f, 17.60f / 42.0f,
                440.0f / 900.0f, 7f / 42.0f, true);
        ...
        buttonContinueLower_ = new PokerButton(getGameButton("continue"), 70 / .90f, 20 / .90f, 17.55f / 42.0f,
                555.0f / 900.0f, 6f / 42.0f, true);
        ...
        // debugging focus...
        // buttonContinueMiddle_.addFocusListener(new
        // GuiUtils.FocusDebugger("continuemiddle"));
        // buttonContinueLower_.addFocusListener(new
        // GuiUtils.FocusDebugger("continuelower"));
        ...
        for (int i = 0; i < PokerConstants.SEATS; i++) {
            ...
            bet = new PokerBetArea(prefw, prefh, (tp.x_ / nTABLEWIDTH), (tp.y_ / nTABLEHEIGHT), width * 1.2, i);
            info.bet = bet;

            tp = t.getTerritoryPoint("icon");
            icon = new PokerIcon(prefh, prefh, (tp.x_ / nTABLEWIDTH) - (icond / 2),
                    (tp.y_ / nTABLEHEIGHT) - (icond / 2), icond);
            info.icon = icon;
    ...
    public void focusGained(FocusEvent e) {
        ...
        // opposite component, ignore the next event if it is a focus gained on the
        // board.
        ...
        // focus gained events in succession on text then board, which invokes the logic
        // to
    ...
    public void focusLost(FocusEvent e) {
        ...
        if (e.getOppositeComponent() == amountText_ || e.getOppositeComponent() == board_) {
            return;
        }
    ...
    /**
     * spinner with special flag to indicate will be disabled, for focus handling
     */
    ...
    private class AmountPanel extends DDPanel implements ChangeListener {
        ...
        public AmountPanel() {
            ...
            GuiUtils.addKeyAction(amountText_, JComponent.WHEN_FOCUSED, "betraise", new BetRaise(), KeyEvent.VK_ENTER,
                    0);
    ...
    protected void addDashboardItems(DashboardManager manager) {
        ...
        if (!game_.isOnlineGame() || bDashAdvisor) {
            ...
            if (!game_.isOnlineGame())
                manager.addItem(new CheatDash(context_), false);
        }
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    EngineUtils.displayInformationDialog(context_,
                            "Autopilot is enabled, but currently paused (F5 or F9).");
    ...
    public void finish() {
        ...
        amount_.getParent().removeAll(); // remove so memory can be cleaned earlier (can retain due to focus in this
                                            // widget)
        super.finish();
    }
        ...
        if (table_ != table) {
            ...
            if (table_ != null) {
                table_.addPokerTableListener(this,
                        PokerTableEvent.TYPE_CURRENT_PLAYER_CHANGED | PokerTableEvent.TYPE_PLAYER_ACTION
                                | PokerTableEvent.TYPE_DEALER_ACTION | PokerTableEvent.TYPE_LEVEL_CHANGED
                                | PokerTableEvent.TYPE_BUTTON_MOVED | PokerTableEvent.TYPE_PREFS_CHANGED
                                | PokerTableEvent.TYPE_PLAYER_REBUY | PokerTableEvent.TYPE_PLAYER_ADDON
                                | PokerTableEvent.TYPE_CLEANING_DONE | PokerTableEvent.TYPE_NEW_PLAYERS_LOADED);
            }
            ...
            if (bRepaint) {
                ...
                if (table_ != null && !TESTING(PokerConstants.TESTING_AUTOPILOT)) {
                    ...
                    final String sMsg = PropertyConfig.getMessage(
                            ...
                            table_.getName(), game_.getHumanPlayer().getSeat() + 1, game_.getSeats());
                    ...
                    Runnable rmsg = new Runnable() {
                        ...
                        public void run() {
                            ...
                            (_human.isObserver() ? "change.observer" : "change.player"),
                                    _human.isObserver() ? "change.observer" : "change.player", !game_.isOnlineGame());
                        }
                    };

                    if (game_.isOnlineGame())
                        GuiUtils.invoke(rmsg);
                    else
                        GuiUtils.invokeAndWait(rmsg);
                }
    ...
    public String getDebugDisplay(Territory t) {
        ...
        if (hs >= 0)
            sb.append("HS ").append(HandStat.fPerc.form(hs)).append("% ");
        if (hp >= 0) {
    ...
    private void setBetRaiseButton(HoldemHand hhand, PokerPlayer player, boolean bRaise) {
        ...
        if (hhand != null) {
            if (bRaise)
                nMax = hhand.getMaxRaise(player);
            else
                nMax = hhand.getMaxBet(player);
        }
    ...
    private void updatePotDisplay(boolean bRepaint) {
        ...
        if (bRepaint && !table_.isZipMode())
            PokerUtils.repaintPot();
    }
    ...
    // {
    // if (e.getButton() != MouseEvent.BUTTON1 && (!(e.getSource() instanceof
    // JButton)))
    // {
    // deal();
    // }
    ...
    protected boolean filterKeystroke(KeyEvent k) {
        ...
        return !(src == board_ || src == amountText_ || src == buttonContinueLower_ || src == buttonContinueMiddle_);
        ...
        if (event.getModifiersEx() == 0 || event.isShiftDown()) {
            switch (key) {
                ...
                case KeyEvent.VK_P :
                    ...
                    break;
                case KeyEvent.VK_B :
                    if (buttonBetRaise_.isEnabled() && getInputMode() == PokerTableInput.MODE_CHECK_BET
                            && !bDisableShortcuts) {
                        buttonBetRaise_.doClick(10);
                        buttonBetRaise_.setEnabled(false);
                        return true;
                ...
                case KeyEvent.VK_C :
                    if (buttonContinueMiddle_.isEnabled() && buttonContinueMiddle_.isVisible() && !bDisableShortcuts) {
                        ...
                        return true;
                    } else if (buttonContinueLower_.isEnabled() && buttonContinueLower_.isVisible()
                            && !bDisableShortcuts) {
    ...
    private class PokerIcon extends ImageComponent {
        public PokerIcon(int nPrefW, int nPrefH, double x, double y, double scale) {
    ...
    private class PokerResult extends DDLabel {
        public PokerResult(int nPrefW, int nPrefH, double x, double y, double scale, int nSeat) {
    ...
    private class PokerBetArea extends DDFastLabel {
        public PokerBetArea(int nPrefW, int nPrefH, double x, double y, double scale, int nSeat) {
    ...
    private class PokerGamePieceListener implements GamePieceSelectionListener {
        ...
        public void mouseExited(Gameboard g, EngineGamePiece gp) {
            ...
            // follow this.
            if (g.getGamePieceUnderMouse() != null)
                return;
        ...
        private void checkCard(Gameboard g, EngineGamePiece gp, boolean bVisible) {
            if (gp instanceof CardPiece) {
                ...
                if (t == null)
                    return; // could happen when cards are removed
    ...
    public void territorySelected(Territory t, MouseEvent e) {
        ...
        if (System.currentTimeMillis() - lastDismissed_ > 200 || (t == lastTerritory_ && cardPiece != null)) {
            // initialize
            ...
            if (PokerUtils.isPot(t) || PokerUtils.isFlop(t) || p == null) {
                ...
                title.setText(PropertyConfig.getMessage("menuitem.player.title",
                        Utils.encodeHTML(title.player.getName()), title.player.getSeat() + 1));
                title.setDisplayMode(DDMenuItem.MODE_TITLE);
                ...
                if (game_.isOnlineGame() && p.isHuman() && !p.isLocallyControlled()) {
                    ...
                    menu.add(new MutePlayer(sStyle, PokerUtils.getPokerPlayer(context_, t),
                ...
                if (!game_.isOnlineGame() || bDashAdvisor) {
                    ...
                    if (!p.isHumanControlled()) {
                        if (bShowCheatItems)
                            menu.add(new SelectPlayerType(sStyle, t, point));
                    }
                    // locally controlled players
                    else if (p.isLocallyControlled())
                        menu.add(new SelectAdvisorType(sStyle, t, point));
                }
    ...
    private class TerritoryMenuItem extends DDMenuItem {
        ...
        TerritoryMenuItem(String sStyle, Territory t) {
            ...
            this.t = t;
            if (t != null)
                player = PokerUtils.getPokerPlayer(context_, t);
    ...
    private void selectPlayerType(Point point, PokerPlayer player, String sTitle, boolean bAdvisor) {
        ...
        DDMenuItem title = new DDMenuItem(GuiManager.DEFAULT, sStyle);
        title.setText(PropertyConfig.getMessage("menuitem.player.title2", sTitle, Utils.encodeHTML(player.getName()),
                player.getSeat() + 1));
        title.setDisplayMode(DDMenuItem.MODE_TITLE);
        menu.add(title);
    ...
    private class ChangeCard extends TerritoryMenuItem implements ActionListener {
        ...
        ChangeCard(String sStyle, Territory t, Point point, CardPiece cardPiece) {
            super(sStyle, t);
            point_ = point;
            text_ = PropertyConfig.getMessage("menuitem.changecard", cardPiece.getCard().getDisplay());
    ...
    private void selectCard(Point point, final PokerPlayer player, final CardPiece cardPiece, String sTitle) {
        ...
        title.setText(PropertyConfig.getMessage("menuitem.changecard.title", sTitle,
                (player == null)
                        ? PropertyConfig.getMessage("menuitem.community")
        ...
        cardSelector.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                table_.firePokerTableEvent(new PokerTableEvent(PokerTableEvent.TYPE_CARD_CHANGED, table_, player,
    ...
    public static class MutePlayer extends DDMenuItem implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            PokerPrefsPlayerList muted = PokerPrefsPlayerList.getSharedList(PokerPrefsPlayerList.LIST_MUTE);
            if (bMuted)
                muted.remove(player.getName(), true);
            else
    ...
    public static class BanPlayer extends DDMenuItem implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            // confirm
            if (!bBanNow || EngineUtils.displayConfirmationDialog(context,
                    PropertyConfig.getMessage("msg.confirm.ban", Utils.encodeHTML(player.getName())))) {
                PokerPrefsPlayerList banned = PokerPrefsPlayerList.getSharedList(PokerPrefsPlayerList.LIST_BANNED);
                if (bBanned)
                    banned.remove(player.getName(), true);
                else
    ...
    /**
     * mouse motion/click translation - need for mouse motion/click handling by
     * board for widgets that obscure it
     */
    private class MouseTranslator extends MouseInputAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            board_.mousePressed(new MouseEvent(board_, e.getID(), e.getWhen(), e.getModifiersEx(), p.x, p.y,
        ...
        public void mouseReleased(MouseEvent e) {
            Point p = SwingUtilities.convertPoint((Component) e.getSource(), e.getX(), e.getY(), board_);
            board_.mouseReleased(new MouseEvent(board_, e.getID(), e.getWhen(), e.getModifiersEx(), p.x, p.y,
                    e.getClickCount(), e.isPopupTrigger()));
        }
        ...
        public void mouseDragged(MouseEvent e) {
            Point p = SwingUtilities.convertPoint((Component) e.getSource(), e.getX(), e.getY(), board_);
            board_.mouseDragged(new MouseEvent(board_, e.getID(), e.getWhen(), e.getModifiersEx(), p.x, p.y,
                    e.getClickCount(), e.isPopupTrigger()));
        }
        ...
        public void mouseMoved(MouseEvent e) {
            Point p = SwingUtilities.convertPoint((Component) e.getSource(), e.getX(), e.getY(), board_);
            board_.mouseMoved(new MouseEvent(board_, e.getID(), e.getWhen(), e.getModifiersEx(), p.x, p.y,
                    e.getClickCount(), e.isPopupTrigger()));
        }
    }
}
