...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 19, 2006 Time: 11:13:50 AM
...
public class PokerContext extends GameContext {
     ...
     * Constructor
     */
    public PokerContext(PokerContext context, String sName, int nDesiredMinWidth, int nDesiredMinHeight) {
     ...
     * Constructor
     */
    public PokerContext(GameEngine engine, PokerGame game, String sName, int nDesiredMinWidth, int nDesiredMinHeight,
    ...
    private class PokerWindow extends EngineWindow {
        public PokerWindow(GameEngine engine, GameContext context, String sName, int nDesiredMinWidth,
                int nDesiredMinHeight) {
        ...
        public void init(GamePhase gamephase, boolean bMain, Dimension size, boolean bFull, String sTitle,
            ...
            GuiUtils.addKeyAction((JComponent) getContentPane(), JComponent.WHEN_IN_FOCUSED_WINDOW, "printscreen",
                    new PrintAction(), KeyEvent.VK_P, Utils.ISMAC ? KeyEvent.META_DOWN_MASK : KeyEvent.CTRL_DOWN_MASK);

            // GuiUtils.addKeyAction((JComponent) getContentPane(),
    ...
    public static void LogGameInfo(PokerGame game) {
        ...
        for (int i = 0; i < game.getNumTables(); i++) {
            ...
            logger.debug("**** Holdem hand on " + t.getName() + ((current == t) ? " (current)" : "")
                    + " when error occurred: ");
