...
public class HandPotential {
    ...
    public String toHTML(int round) {
        ...
        for (int i = 0; i < aStatKeys_.size(); ++i) {
            ...
            if (((handCountA > 0) || (handCountB > 0)) && ((stage != 2) || i < nFirstDrawKey_)) {
                ...
                // until we implement omaha, if ever, since in hold'em they're mutually
                // exclusive
                ...
                if (handCountA > 0) {
                    ...
                    buf.append(bold(PokerConstants.formatPercent(handCountA / ((float) totalHandCount_[a] / 100)) + "%",
                            bold));
                    buf.append("</td><td align=\"right\">");
                    buf.append(bold(
                            PokerConstants.formatPercent(((float) totalHandCount_[a] - handCountA) / handCountA) + ":1",
                            bold));
                    buf.append("</td>");
                } else if (twoColumns) {
                ...
                if (twoColumns) {
                    if ((handCountB > 0) && (i < nFirstDrawKey_)) {
                        ...
                        buf.append("</td><td align=\"right\">");
                        buf.append(bold(
                                PokerConstants.formatPercent(handCountB / ((float) totalHandCount_[1] / 100)) + "%",
                                bold));
                        buf.append("</td><td align=\"right\">");
                        buf.append(bold(
                                PokerConstants.formatPercent(((float) totalHandCount_[1] - handCountB) / handCountB)
                                        + ":1",
                                bold));
                        buf.append("</td>");
                    } else {
    ...
    public static float getPotential(Hand hole, Hand community) {
        ...
        int MORE = 5 - nComm;
        if (MORE == 2)
            MORE = 1; // TESTING force to 1 card more
            ...
            for (int j = i + 1; j < nSize; j++) {
                ...
                for (int next1 = 0; next1 < nEND; next1++) {
                    ...
                    for (int next2 = next1 + 1; next2 < nSize; next2++) {
                        ...
                        // logger.debug(index+ " check: " + hole + " (" + ourbest +") , " + holecopy + "
                        // ("+oppbest+"), " + commcopy);
        ...
        float npot = (hp[AHEAD][BEHIND] + hp[TIED][BEHIND] / 2 + hp[AHEAD][TIED] / 2)
        ...
        if ((TESTING(EngineConstants.TESTING_AI_DEBUG))) {
            logger.debug("POTENTIAL for " + hole + "," + community + ": " + HandStat.fPerc.form(ppot) + " ppot   "
