...
public class LanClientInfo {
    ...
    /**
     * comapre this to given LanClientInfo and return true if they are equivalent
     * for purposes of data display
     */
    public boolean isEquivalent(LanClientInfo info, LanControllerInterface controller) {
        if (info == null)
            return false;
        if (!getPlayerName().equals(info.getPlayerName()))
            return false;
        if (!getHostName().equals(info.getHostName()))
            return false;
        if (!getIP().equals(info.getIP()))
            return false;
        return controller.isEquivalentOnlineGame(this.getGameData(), info.getGameData());
    }
    /**
     * Creates a new instance of LanClientInfo with the DDMessage source data
     */
    ...
    /**
     * Create a new instance of LanClientInfo that is empty
     */
