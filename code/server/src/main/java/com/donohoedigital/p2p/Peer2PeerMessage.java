...
public class Peer2PeerMessage implements DDMessageTransporter {
    ...
    // timeout
    public static final int READ_TIMEOUT_MILLIS = PropertyConfig
            .getRequiredIntegerProperty("settings.server.readtimeout.millis");
    public static final int READ_WAIT_MILLIS = PropertyConfig
            .getRequiredIntegerProperty("settings.server.readwait.millis");
    public static final int WRITE_TIMEOUT_MILLIS = PropertyConfig
            .getRequiredIntegerProperty("settings.server.writetimeout.millis");
    public static final int WRITE_WAIT_MILLIS = PropertyConfig
            .getRequiredIntegerProperty("settings.server.writewait.millis");
    ...
    /**
     * Get version - reserved for future usage in case we need to change the
     * protocol
     */
    ...
    /**
     * Set whether the connection should be kept alive after sending this message
     * (default is true). Use of false is used when returning a message to a client
     * that is invalid or just sending a test message
     */
    ...
    /**
     * Get whether the connection should be kept alive after sending this message
     */
        ...
        while (!bDone) {
            ...
            if (outBuffer.hasRemaining()) {
                ...
                if (nSleep >= WRITE_TIMEOUT_MILLIS) {
                    throw new SocketTimeoutException("P2P write timeout. Left to write: " + outBuffer.remaining()
                            + " of " + nToWrite + " send buffer size: " + channel.socket().getSendBufferSize()
                            + " recv buffer size: " + channel.socket().getReceiveBufferSize() + " nodelay: "
                            + channel.socket().getTcpNoDelay() + " linger: " + channel.socket().getSoLinger()
    ...
    public void read(SocketChannel channel) throws IOException {
        ...
        if (crcRead != crcCalc) {
            // throw invalid message error so socket is shutdown
            throw new ApplicationError(ErrorCodes.ERROR_INVALID_MESSAGE,
                    "Invalid message - CRC mismatch, msg: " + crcRead + " != calculated " + crcCalc, null);
        }
    ...
    /**
     * Read 4 bytes into buffer and return the cooresponding int. Buffer is cleared
     * after reading (so can be reused)
     */
    ...
    /**
     * Read 8 bytes into buffer and return the cooresponding long. Buffer is cleared
     * after reading (so can be reused)
     */
    ...
    /**
     * Read 2 bytes and return corresponding char. Buffer is cleared after reading
     * (so can be reused)
     */
    ...
    private void readBytes(SocketChannel channel, ByteBuffer buffer) throws IOException {
        ...
        LOOP : while ((count = channel.read(buffer)) >= 0) {
            ...
            if (count == 0) {
                ...
                if (DEBUG)
                    logger.debug("Sleeping... position is " + buffer.position() + " capacity is " + buffer.capacity());
