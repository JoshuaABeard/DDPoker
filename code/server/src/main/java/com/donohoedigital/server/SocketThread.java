...
public class SocketThread extends Thread {
    ...
    protected static final int READ_TIMEOUT_MILLIS = PropertyConfig
            .getRequiredIntegerProperty("settings.server.readtimeout.millis");
    protected static final int READ_WAIT_MILLIS = PropertyConfig
            .getRequiredIntegerProperty("settings.server.readwait.millis");
    ...
    public synchronized void run() {
        ...
        while (!bDone_) {
            ...
            if (GameServer.DEBUG_ONLINE)
                logger.debug(getName() + " AWAKE for " + Utils.getIPAddress(channel_));
            bShutDown = false;
            ...
            try {
                ...
                readData(channel_);
                if (GameServer.DEBUG_ONLINE)
                    logger.debug(getName() + " after read");

                process();
                if (GameServer.DEBUG_ONLINE)
                    logger.debug(getName() + " after process");
            }
            // handle exceptions
            catch (Throwable t) {
                ...
                try {
                    ...
                    if (GameServer.DEBUG_ONLINE)
                        logger.debug(getName() + " DONE - returning to pool");
    ...
    /**
     * Is this a keep alive thread? Defaults to false; for overriding. Controls
     * whether the socket is closed after processing, or added back to server's
     * selector for futher reading
     */
    ...
    protected boolean handleException(Throwable t) {
        try {
            ...
            // logger.error("Exception in handleException: " +
            // Utils.formatExceptionText(ignored) +
    ...
    protected void readData(SocketChannel channel) throws IOException {
        ...
        while ((count = channel.read(buffer_)) >= 0) {
            ...
            if (count != 0) {
                if (GameServer.DEBUG_ONLINE)
                    logger.debug(getName() + " read " + count);
                // logger.debug("Read " + count + ": <" + Utils.decode(buffer_.array(), 0,
                // buffer_.position())+">");
                ...
                if (request_ == null) {
                    processHeaders();
                    if (GameServer.DEBUG_ONLINE)
                        logger.debug(getName() + " after process headers");
                }
            ...
            if (count == 0) {
                ...
                if (nSleep >= READ_TIMEOUT_MILLIS) {
                    throw new ApplicationError(ErrorCodes.ERROR_SERVER_IO, "Read timeout", getBufferAsString(), null);
                }
                if (buffer_.position() == buffer_.capacity()) {
                    throw new ApplicationError(ErrorCodes.ERROR_SERVER_IO, "Buffer full, but data still left to read",
                ...
                if (GameServer.DEBUG_ONLINE)
                    logger.debug(
                            "Sleeping... position is " + buffer_.position() + " capacity is " + buffer_.capacity());
        ...
        if (GameServer.DEBUG_ONLINE)
            logger.debug(getName() + " after shutdowninput");

        // make sure we have a request
        if (request_ == null)
            throw new ApplicationError(ErrorCodes.ERROR_SERVER_FORBIDDEN, "Forbidden (x3)", getBufferAsString(), null);
        ...
        if (nContentExpected > 0 && buffer_.remaining() != nContentExpected) {
            throw new ApplicationError(ErrorCodes.ERROR_SERVER_IO, "Content length mismatch; Content-Length ("
                    + nContentExpected + ") does not match data read: " + buffer_.remaining(), getBufferAsString(),
                    null);
        }
        ...
        if (DEBUG)
            logger.debug("Data: <" + Utils.decode(buffer_.array(), buffer_.position(), buffer_.remaining()) + '>');
    }
    ...
    private void createRequest() {
        ...
        if (sHTTPVersion == null)
            sHTTPVersion = "HTTP/1.0";
        ...
        while (true) {
            ...
            if (DEBUG)
                logger.debug("HEADER name: " + sName + ", value: <" + sValue + '>');
        ...
        if (servlet_.isDDMessageHandler()) {
            ...
            if (!sMethod.equalsIgnoreCase("POST") || !sURI.startsWith(DD_URI_STARTS_WITH)
                    || !sUserAgent.equalsIgnoreCase(DDMessenger.getUSERAGENT())) {
                throw new ApplicationError(ErrorCodes.ERROR_SERVER_FORBIDDEN, "Forbidden (x2)", getBufferAsString(),
                        null);
            }
        }
    }
    ...
    /**
     * Return contents of buffer as a string for error messages, limit to 3000 bytes
     */
     ...
     *
     *
     * HTTP/1.1 200 OK Date: Thu, 14 Aug 2003 03:48:48 GMT Jetty/4.1.3 (Mac OS X
     * 10.2.6 ppc) Servlet-Engine: Jetty/4.1.3 (Servlet 2.3; JSP 1.2; java 1.4.1_01)
     * Content-Type: application/x-eaglegames-war-msg
     *
     * HTTP/1.1 403 Permission+denied+dude%2E Date: Thu, 14 Aug 2003 22:10:40 GMT
     * Server: Jetty/4.1.3 (Mac OS X 10.2.6 ppc) Servlet-Engine: Jetty/4.1.3
     * (Servlet 2.3; JSP 1.2; java 1.4.1_01) Content-Type: text/html Content-Length:
     * 1185
     *
     */
}
