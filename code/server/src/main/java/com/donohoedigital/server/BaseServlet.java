...
public abstract class BaseServlet extends HttpServlet {
    ...
    /**
     * used for any subclass init, called from GameServer after config files have
     * been loaded.
     */
     ...
     *             if server already set (one
     */
    void setServer(GameServer server) {
        ApplicationError.assertNull(this.server, "Server already set - instance of BaseServlet cannot be shared",
                this.server);
        this.server = server;
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (sUserAgent == null || !sUserAgent.equals(DDMessenger.getUSERAGENT())) {
            ...
            // ret = new DDMessage(DDMessage.CAT_TESTING, "Test message returning back at
            // you.");
     ...
     */
    protected void returnMessage(HttpServletResponse response, DDByteArrayOutputStream data) throws IOException {
        response.setContentType(DDMessage.CONTENT_TYPE);
        OutputStream out;
        if (response instanceof GameServletResponse) {
            out = ((GameServletResponse) response).getOutputStream2();
    ...
    /**
     * Subclasses implement this to process message received. If desired, can call
     * returnMessage() to send the message back over the stream. If this is called,
     * you must return null (or the message will get returned twice). This is
     * provided if a message needs to be returned within a lock, for example. If
     * returnMessage() is not called, then the caller of this will invoke it
     * automatically
     */
    public abstract DDMessage processMessage(HttpServletRequest request, HttpServletResponse response,
            DDMessage ddreceived) throws IOException;

    /**
     * Return new DDMessage for use with received messages. Can be overriden to
     * return a subclass of DDMessage
     */
    ...
    /**
     * Does this servlet expect dd messages in and out? Default is true.
     */
