 ...
 * session. A Session is a collecion of URLs in a single url file * Each thread
 * establishes a new Weblogic session and requests each URL * in the collection.
 * The collection of URLs is requested -loop times. * A new session is
 * established at the beginning of the loop. *
 * <P>
 ...
 * </PRE>
 *
 * * Where Tn is the thread number for the session, filename is the name * of
 * the file that contained the URLs, [xxx.yy] is the loop that thread is on *
 * (xxx is the loop, yy is the index of the url: e.g., 050.02 means the * 50th
 * loop, 2nd URL in the url list), * N.NNN is the number of seconds it took to
 * fetch the page. After the URL * that is fetched, (zzzzz) is the number of
 * bytes returned in the page and * [n] is number of zeros received at the end
 * of the page (this is used to * look for a bug where extra bytes were being
 * sent down -- a valid * value is 0 or 1). You will see a summary at the end
 * like below (this * is what is put in the local log file): *
 *
 * <PRE>
 ...
 * <P>
 * * You can log summary information and errors to a log file with the -log
 * option. * * @author Doug Donohoe *
 */
...
public class LoadGen {
    ...
    static boolean bDoSessions = true; // if false, each request generates a new session; if true, each thread=1
                                        // session
    ...
    private static void setupCommandLineOptions() {
        ...
        CommandLine.setParamDescription("[url list]", "[url list 1] ... [url list N]",
        ...
        CommandLine.setDescription("forcenewsession",
                "Force a new session to be created every request (special testing)");
    ...
    public static void main(String[] args) {
        ...
        double response = (double) totalResponseTime / 1000.0 / (double) totalRequests; // average response time in
                                                                                        // seconds
        ...
        if (bCalcStats) {
            ...
            for (int j = 0; j < nSessions; j++) {
                ...
                for (int i = 0; i < urlspecs.length; i++) {
                    ...
                    variance = (1 / (count - 1) * urlspec.dRunningTotalSquares)
                            - (count / (count - 1) * urlavg * urlavg);

                    sURL = urlspec.sURL;
                    if (sURL.length() > MAXLEN)
                        sURL = sURL.substring(0, MAXLEN);
                    log("    URL #" + (i + 1) + " - " + fsum5Floating.form(urlavg) + "  "
                            + fsum4Floating.form(urlspec.dMin) + "  " + fsum5Floating.form(urlspec.dMax) + "  "
                            + fsum5Floating.form(variance) + "  " + fsum6Floating.form(urlspec.dRunningTotal) + " "
                            + fsumCount.form(urlspec.nCount) + "  " + sURL);
    ...
    /**
     * Read the given file, it assumes one URL per line. Return URL Specs:
     * <p/>
    ...
    /**
     * * This method logs a message to the logfile if logging is on as * well as
     * printing out the message to the console
     */
                ...
                for (int i = 0; i < urlspecs.length; i++) {
                    ...
                    if (bCalcStats) {
                        synchronized (LoadGen.class) {
                            urlspec.nCount++;
                            if (time > urlspec.dMax)
                                urlspec.dMax = time;
                            if (time < urlspec.dMin)
                                urlspec.dMin = time;
                    ...
                    if (bPageStats) {
                        ...
                        synchronized (LoadGen.class) {
                            ...
                            System.out.println(" " + sThreadName + " [" + fLoop.form((long) loop) + "." + (i + 1) + "]"
                                    + " " + fTime.form(time) + " " + urlout + " (" + fBytes.form(bytesRead) + ")" + " ["
                                    + zeroBytesRead + "]");
                        }
                    }
                }
    ...
    /**
     * This private class lists an URL and the amount of time to sleep before
     * requesting the URL (if the -sleep option is passed)
     */
