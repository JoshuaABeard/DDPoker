...
/**
 * This class is for testing registrations through a proxy. We configure the
 * client (e.g. DD Poker) to register to the address/port that this program is
 * running on. Then, that request is intercepted and passed to the proxy we
 * configure and the results are passed back. This is actually simulating a
 * chained proxy also.
 *
 * We support two types of proxies: Normal proxies where you pass-thru the
 * request, changing GET foo HTTP/1.0 to GET http://server/foo HTTP/1.0.
 *
 * The other is the CONNECT method, where we pass CONNECT server HTTP/1.0 and
 * then the original headers/post data from the client through after that.
 *
 * @author donohoe
 */
public class ProxySocketThread extends SocketThread implements PostWriter, DDMessageListener {
    ...
    protected void process() throws IOException, ServletException {
        ...
        logger.debug("DATA: " + data.getOut().size() + " bytes ->" + "\n========= OUTPUT =========\n"
                + Utils.decodeBasic(data.getOut().getBuffer(), 0, data.getOut().size()) + "=========  END   =========");
