...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 4, 2008 Time: 3:01:53 PM To
 * change this template use File | Settings | File Templates.
 */
public class JoinGameResource {
    public static ByteArrayResource create(OnlineGame game, boolean observe) {
        String name = "game" + game.getId() + (observe ? "obs" : "") + '.' + PokerConstants.JOIN_FILE_EXT;
        StringBuilder sb = new StringBuilder(game.getUrl());
        if (observe)
            sb.append(PokerConstants.JOIN_OBSERVER_QUERY);
        sb.append('\n');
