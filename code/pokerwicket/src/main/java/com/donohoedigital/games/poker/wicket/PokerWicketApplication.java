...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Apr 17, 2008 Time: 1:31:07 PM
...
public class PokerWicketApplication extends BaseWicketApplication {
    ...
    public static final Date START_OF_TIME = new GregorianCalendar(2005, Calendar.AUGUST, 1).getTime(); // August 1st
                                                                                                        // 2005 (1st
                                                                                                        // month of DD
                                                                                                        // Poker online)
    ...
    protected void init() {
        ...
        // PokerJetty, but if it is already running, changes made are not copied, which
        // makes
        ...
        if (getConfigurationType().equals(RuntimeConfigurationType.DEVELOPMENT)) {
            ...
            getResourceSettings().getResourceFinders().add(0,
                    new Path(new File(repoRoot, "code/wicket/src/main/java").getAbsolutePath()));
            getResourceSettings().getResourceFinders().add(0,
                    new Path(new File(repoRoot, "code/pokerwicket/src/main/java").getAbsolutePath()));
        }
    }
    ...
    /**
     * Production if running on a donohoe digital machine FIX: do this smarter (when
     * we fix the JDBC url too!)
     */
    @Override
