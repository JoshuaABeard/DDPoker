...
public class GameDetail extends OnlinePokerPage {
    ...
    public GameDetail(PageParameters params) {
        ...
        add(DateLabel.forDatePattern("date", new Model<>(getDate(game)),
                PropertyConfig.getMessage("msg.format.datetime")));

        // game url if reg/play mode
        add(new GameUrl("gameUrl", game, PokerSession.get().isLoggedIn(), this)
                .setVisible(mode == OnlineGame.MODE_REG || mode == OnlineGame.MODE_PLAY));
        ...
        add(new StringLabel("tournamentProfile", new TournamentProfileHtml(tournament).toHTML(null))
        ...
        protected void populateItem(Item<TournamentHistory> row) {
            ...
            if (history.isEnded()) {
                ...
                } else // busted
                {
                    row.add(new PokerCurrencyLabel("prize"));
                    row.add(new HiddenComponent("numChips"));
                    row.add(new StringLabel("bustedOut", "Busted out&nbsp;").setEscapeModelStrings(false)); // FIX: use
                                                                                                            // properties
                }
    ...
    /**
     * Get OnlineGame identified from page parameters. Returns null and sets error
     * response page if none is found.
     */
    private OnlineGame getOnlineGame(PageParameters params) {
        ...
        if (historyId != -1L) {
            ...
            if (game == null) {
                setResponsePage(new ErrorPage("Unable to find game using tournament history id " + historyId + '.')); // FIX:
                                                                                                                        // property?
                return null;
            }
        }
