...
/**
 * Ascending sorted hand. Starts sorted and remains sorted when cards added
...
public class HandSorted extends Hand {
    ...
    /**
     * Add card sorted starting at given index, returning index at which card was
     * added
     */
    ...
    /**
     * Returns whether this hand is equivalent to the given hand. 1st, the cards in
     * both hands must have the same rank. 2nd, if the cards in this hand are of the
     * same suit, so must the other hand. This is used in evaluating and ranking the
     * starting 2 cards in holdem. It assumes there are the same number of cards and
     * they have been sorted (the same way) prior to calling this method.
     */
    public boolean isEquivalent(HandSorted cards) {
        if (size() != cards.size())
            return false;
        if (isSuited() != cards.isSuited())
            return false;

        for (int i = 0; i < size(); i++) {
            if (!getCard(i).isSameRank(cards.getCard(i)))
                return false;
        }
    ...
    /**
     * Returns whether this hand (customary use is the board) contains any straight
     * draws (including gut-shot draws like 8c Qs 2s, where a 9x Tx would be
     * completed with a J)
     */
    ...
    /**
     * Return whether a connector is in this hand (specifying gap - i.e., a gap of 0
     * means something like 10 9 where a gap of 1 means something like 10 8).
     */
     ...
     * Return whether a connector is in this hand (specifying gap - i.e., a gap of 0
     * means something like 10 9 where a gap of 1 means something like 10 8).
     * nMinBottom is the miminum bottom card for the connector (inclusive) - pass 1
     * for ace)
     */
    public boolean hasConnector(int nGap, int nMinBottom) {
        ...
        for (int i = ace ? -1 : 0; i < (nSize - 1); i++) {
            c1 = (i == -1) ? Card.SPADES_LOW_A : getCard(i);
            c2 = getCard(i + 1);
            if (c1.getRank() < nMinBottom)
                continue;
            if (c1.getRank() == c2.getRank())
                continue;
            if (c1.getRank() + (nGap + 1) >= c2.getRank())
                return true;
        }
        return false;
    }
