...
public class TournamentHistoryList extends DMArrayList<TournamentHistory> {
    ...
    /**
     * calculate rank for each player in this tournament history Assumes items in
     * list are sorted properly. This is okay since we send them down in ordr
     * (WanManager) or pull them from the DB ordered.
     *
     * @param bEnded
     * @param bDebug
     */
    public void calculateInfo(boolean bEnded, boolean bDebug) {
        ...
        for (int i = size() - 1; i >= 0; i--) {
            ...
            if (bDebug) {
                logger.debug("  ==> " + hist.getPlayerName() + (hist.isComputer() ? " (ai)" : "") + " finished "
                        + hist.getPlace() + " and won " + hist.getPrize() + " num ai: " + nAI + "  rank: "
                        + hist.getRank1() + " ended: " + hist.isEnded());
            }
        }
    }
}
