...
public class Hand extends DMArrayList<Card> {
    ...
    public boolean isConnectors(int minRank, int maxRank) {
        ...
        return (rank1 >= minRank) && (rank1 <= maxRank) && (rank2 >= minRank) && (rank2 <= maxRank)
    ...
    public boolean isInHand(int rank) {
        int nStart = size() - 1;
        for (; nStart >= 0; nStart--) {
            if (getCard(nStart).getRank() == rank)
                return true;
        }
    ...
    public boolean isInHand(int rank, int suit) {
        ...
        for (; nStart >= 0; nStart--) {
            c = getCard(nStart);
            if (c.getRank() == rank && c.getCardSuit().getRank() == suit)
                return true;
        }
    ...
    public boolean containsRank(int rank) {
        return (((1L << Card.getCard(CardSuit.CLUBS_RANK, rank).getIndex())
                ...
                | (1L << Card.getCard(CardSuit.SPADES_RANK, rank).getIndex())) & fingerprint()) > 0L;
    }
    ...
    public boolean hasPossibleFlush() {
        switch (size()) {
            ...
            case 1 :
                return false;
            case 2 :
                return getCard(0).getSuit() == getCard(1).getSuit() && getCard(0).getSuit() == getCard(2).getSuit();
            default :
    ...
    public boolean hasQuads() {
        switch (size()) {
            ...
            case 4 : {
                int rank = getCard(0).getRank();
                return getCard(1).getRank() == rank && getCard(2).getRank() == rank && getCard(3).getRank() == rank;
            }
            default : {
                int rank[] = new int[Card.ACE + 1];
                for (int i = size() - 1; i >= 0; --i) {
    ...
    public int getNumSuits() {
        return (containsSuit(Card.CLUBS) ? 1 : 0) + (containsSuit(Card.DIAMONDS) ? 1 : 0)
                + (containsSuit(Card.HEARTS) ? 1 : 0) + (containsSuit(Card.SPADES) ? 1 : 0);
    }
