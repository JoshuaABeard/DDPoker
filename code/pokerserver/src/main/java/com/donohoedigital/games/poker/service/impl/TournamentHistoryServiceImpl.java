...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 25, 2008 Time: 5:13:18 PM
...
public class TournamentHistoryServiceImpl implements TournamentHistoryService {
    ...
    @Transactional(readOnly = true)
    public TournamentHistoryList getAllTournamentHistoriesForGame(Integer count, int offset, int pagesize,
            Long gameId) {
    ...
    public void upgradeAllTournamentHistoriesForGame(OnlineGame game, Logger logger) {
        ...
        for (TournamentHistory hist : list) {
            if (hist.getNumPlayers() != num || !hist.getTournamentName().equals(name)) {
                if (logger != null)
                    logger.info("  UPDATING history " + hist.getId() + ": " + name + " (" + num + " players)");
    ...
    @Transactional(readOnly = true)
    public LeaderboardSummaryList getLeaderboard(Integer count, int offset, int pagesize, LeaderboardType type,
            int games_limit, String nameSearch, Date begin, Date end) {
        return dao.getLeaderboard(count, offset, pagesize, type == LeaderboardType.roi, games_limit, nameSearch, begin,
                end);
    }
}
