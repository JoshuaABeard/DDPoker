...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 23, 2008 Time: 3:22:57 PM
 * <p/>
 * Online game service operates on OnlineGames that come from the client, which
 * doesn't pass down the database id. Hence, most of the writable modifications
 * first check for existence using the unique key of url and license key.
 */
...
public class OnlineGameServiceImpl implements OnlineGameService {
    ...
    @Transactional(readOnly = true)
    public OnlineGameList getOnlineGames(Integer count, int offset, int pagesize, Integer[] modes, String nameSearch,
            Date begin, Date end, OrderByType orderByType) {
        return gameDao.getByMode(count, offset, pagesize, modes, nameSearch, begin, end,
                orderByType == OrderByType.mode);
    }
        ...
        for (TournamentHistory history : histories) {
            ...
            if (history.getPlayerType() == PLAYER_TYPE_ONLINE) {
                ...
                if (profile == null) {
                    ...
                    logger.warn(
                            "Missing profile in history: " + history.getPlayerName() + " for game id=" + game.getId());
