...
public class OnlineProfilePurger extends BaseCommandLineApp {
    ...
    private void doPurge() {
        ...
        ApplicationError.assertTrue(processed == list.getTotalSize(),
                "Processed " + processed + " of " + list.getTotalSize() + " rows");
        ...
        if (sum.getHistoryCount() == 0) {
            ...
            if (lastSameKey(sum)) {
                if (p.getModifyDate().getTime() < days_14.getTime()) {
                    ...
                    logger.debug(".... Keeping " + p.getName()
                            + " because created in past 14 days (even though there are other profiles)");
                }
