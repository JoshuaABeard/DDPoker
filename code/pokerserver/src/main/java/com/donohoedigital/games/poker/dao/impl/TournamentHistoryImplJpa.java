...
public class TournamentHistoryImplJpa extends JpaBaseDao<TournamentHistory, Long> implements TournamentHistoryDao {
    ...
    public int deleteAllForGame(OnlineGame game) {
        Query query = entityManager.createQuery("delete from TournamentHistory t " + "where t.onlineGame = :game");
    ...
    public int getAllForGameCount(Long gameId) {
        Query countQuery = entityManager
                .createQuery("select count(t) from TournamentHistory t " + "where t.onlineGame.id = :id");
    ...
    public TournamentHistoryList getAllForGame(Integer count, int offset, int pagesize, Long gameId) {
        Query query = entityManager.createQuery(
                "select t from TournamentHistory t " + "where t.onlineGame.id = :id " + "order by t.place");
    ...
    public int getAllForProfileCount(Long id, String nameSearch, Date begin, Date end) {
        Query countQuery = entityManager.createQuery("select count(t) from TournamentHistory t "
                + "where t.profile.id = :id " + "and t.tournamentName like :name "
    ...
    @SuppressWarnings({"unchecked"})
    public TournamentHistoryList getAllForProfile(Integer count, int offset, int pagesize, Long id, String nameSearch,
            Date begin, Date end) {
        Query query = entityManager.createQuery(
                "select t from TournamentHistory t " + "where t.profile.id = :id " + "and t.tournamentName like :name "
                        + ((begin != null && end != null) ? "and t.endDate >= :begin and t.endDate <= :end " : "")
    ...
    public int getLeaderboardCount(int games_limit, String nameSearch, Date begin, Date end) {
        ...
        Query countQuery = entityManager.createNativeQuery("select count(*) from " + "  (select count(1) "
                ...
                + (!empty ? " AND wpr_name like :name " : "") + "   GROUP BY wpr_name "
    ...
    @SuppressWarnings({"unchecked"})
    public LeaderboardSummaryList getLeaderboard(Integer count, int offset, int pagesize, boolean sortByROI,
        ...
        // note: need to join with wan_profile to get correct profile name (e.g., for AI
        // players)
        Query query = entityManager.createNativeQuery("SELECT count(whi_profile_id) AS gamesplayed, "
                + "       whi_profile_id, " + "       wpr_name, \n" + "       avg(whi_rank_1) AS rank1, \n"
