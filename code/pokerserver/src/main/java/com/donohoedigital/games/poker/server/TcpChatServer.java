...
/**
 * TCP-based chat server for online lobby. Replaces UDP-based ChatServer.
 * Uses GameServer framework with Peer2PeerMessage protocol.
 */
public class TcpChatServer extends GameServer
{
    ...
    private void sendLeaveNotification(ChatConnection leaving)
    {
        ...
        leaveMsg.setChat(PropertyConfig.getMessage("msg.chat.goodbye",
                Utils.encodeHTML(leaving.playerInfo.getName())));
    ...
    private void sendJoinNotification(ChatConnection joining, SocketChannel skip)
    {
        ...
        joinMsg.setChat(PropertyConfig.getMessage("msg.chat.hello",
                Utils.encodeHTML(joining.playerInfo.getName())));
    ...
    private String getStatusHTML()
    {
        ...
        synchronized (connections)
        {
            for (ChatConnection conn : connections)
            {
                sb.append("<li>").append(Utils.encodeHTML(conn.playerInfo.getName()))
                        .append(" (")
                        .append(conn.channel.socket().getInetAddress().getHostAddress())
                        .append(":").append(conn.channel.socket().getPort())
                        .append(")</li>");
            }
        }
        sb.append("</ul>");
    ...
    {
        @Override
        public com.donohoedigital.comms.DDMessage processMessage(
                jakarta.servlet.http.HttpServletRequest request,
                jakarta.servlet.http.HttpServletResponse response,
                com.donohoedigital.comms.DDMessage ddreceived) throws IOException
        {
    ...
    public static class ChatSocketThread extends SocketThread
    {
        ...
        private void handleHello(SocketChannel channel, OnlineMessage msg)
        {
            ...
            EngineMessage validate = PokerServlet.validateKeyAndVersion(
                    validateThis, remoteIP, chatServer_.bannedKeyService,
            ...
            // Get aliases
            List<OnlineProfile> aliases = chatServer_.onlineProfileService.getAllOnlineProfilesForEmail(
            ...
            synchronized (chatServer_.connections)
            {
                for (ChatConnection existing : chatServer_.connections)
                {
                    ...
                    if (existing.licenseKey.equals(licenseKey) && existing.channel != channel)
                    {
                        ...
                        sendError(channel, PropertyConfig.getMessage("msg.chat.dupkey",
                                Utils.encodeHTML(profileName), licenseKey));
                        return;
                    }
                ...
                logger.info(playerInfo.getName() + " HELLO (" +
                        channel.socket().getInetAddress().getHostAddress() + ":" +
                        channel.socket().getPort() + ")");

                // Send WELCOME with player list
                sendWelcome(channel, PropertyConfig.getMessage("msg.chat.welcome",
                        Utils.encodeHTML(playerInfo.getName())));
