...
public class OnlineProfileServiceImpl implements OnlineProfileService {
    ...
    @Transactional(readOnly = true)
    public int getMatchingOnlineProfilesCount(String nameSearch, String emailSearch, String keySearch,
            boolean includeRetired) {
        return dao.getMatchingCount(nameSearch, emailSearch, keySearch, includeRetired);
    }

    @Transactional(readOnly = true)
    public List<OnlineProfile> getMatchingOnlineProfiles(Integer count, int offset, int pagesize, String nameSearch,
    ...
    public OnlineProfile authenticateOnlineProfile(OnlineProfile profile) {
        ...
        // Use bcrypt password checking: profile.getPassword() is plaintext from login
        // attempt
    ...
    private PasswordGenerator passwordGenerator_ = new PasswordGenerator((PasswordGenerator.OPTION_INCLUDE_LETTERS
            | PasswordGenerator.OPTION_INCLUDE_NUMBERS | PasswordGenerator.OPTION_EXCLUDE_SIMILAR), null);

    /**
     * Get dummys ... lazily create if null. Synchronized so thread safe.
     */
