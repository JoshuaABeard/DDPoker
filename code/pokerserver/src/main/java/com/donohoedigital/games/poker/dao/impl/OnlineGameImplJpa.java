...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 14, 2008 Time: 10:56:58 PM
 * To change this template use File | Settings | File Templates.
 */
@Repository
public class OnlineGameImplJpa extends JpaBaseDao<OnlineGame, Long> implements OnlineGameDao {
    // private static final Logger logger =
    // LogManager.getLogger(OnlineGameImplJpa.class);
    ...
    public OnlineGame getByTournamentHistoryId(Long id) {
        Query query = entityManager.createQuery(
                "select o from OnlineGame o, TournamentHistory h " + "where h.id = :tid and h.onlineGame = o");
    ...
    public int getByModeCount(Integer[] modes, String nameSearch, Date begin, Date end) {
        ...
        Query countQuery = entityManager.createQuery("select count(o) from OnlineGame o " + "where o.mode in (" + sModes
                + ") " + ((nameSearch != null) ? "and o.hostPlayer like :name " : "") + "and o." + dateColumn
    ...
    public OnlineGameList getByMode(Integer count, int offset, int pagesize, Integer[] modes, String nameSearch,
            Date begin, Date end, boolean bOrderByModeName) {
        // always use dates to take advantage of index on dates
        if (begin == null)
            begin = BEGINNING_OF_TIME;
        if (end == null)
        ...
        Query query = entityManager.createQuery("select o from OnlineGame o " + "where o.mode in (" + sModes + ") "
                + ((nameSearch != null) ? "and o.hostPlayer like :name " : "") + "and o." + dateColumn
                + " between :begin and :end " + sOrderBy);
    ...
    public int getHostSummaryCount(String nameSearch, Date begin, Date end) {
        Query countQuery = entityManager.createQuery("select count(distinct o.hostPlayer) from OnlineGame o "
     ...
     */
    @SuppressWarnings({"unchecked"})
    public PagedList<HostSummary> getHostSummary(Integer count, int offset, int pagesize, String nameSearch, Date begin,
            Date end) {
        if (count == null) {
        ...
        Query query = entityManager.createNativeQuery("select wgm_host_player, count(wgm_id) AS cnt, wpr_is_retired "
                + "from wan_game, wan_profile " + "where wgm_host_player = wpr_name "
                + "and wgm_host_player like :name "
                + ((begin != null && end != null) ? "and wgm_modify_date >= :begin and wgm_modify_date <= :end " : "")
                + "group by wgm_host_player " + "order by cnt desc, wgm_host_player");
        if (begin != null && end != null) {
            query.setParameter("begin", begin);
            query.setParameter("end", end);
        }
    ...
    public int purge(Date beforeThisDate, Integer mode) {
        ...
        Query query = entityManager.createQuery(
                "delete from TournamentHistory h " + "where h.onlineGame in " + "    (select o from OnlineGame o "
                        + "     where o.createDate < :date " + "     and o.mode = :mode " + "    )");
        ...
        // delete all games
        query = entityManager
                .createQuery("delete from OnlineGame o " + "where o.createDate < :date " + "and o.mode = :mode ");
        query.setParameter("date", beforeThisDate);
        query.setParameter("mode", mode);
