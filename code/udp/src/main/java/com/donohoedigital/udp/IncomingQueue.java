...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 8, 2006 Time: 7:43:20 PM To
...
public class IncomingQueue {
    ...
    /**
     * Debug purposes: Is the queue waiting for a message prior to the first message
     * on the queue? If queue is empty, true is returned
     */
    ...
    /**
     * Dispatch messages. Basically the messages in the queue are sorted by message
     * id. We start at the top of the queue and look to see if the message at he top
     * is the next message we are expecting. If it is, we make sure all parts are
     * there (if it is a multi-part). If so, we remove from the queue then,
     * reassemble (if multi-part), then dispatch to the handler. The logic while
     * sync'ing on the queue is short and should be quick, so as not to hold up the
     * UDPManager thread that is putting items on the queue. Number of messages
     * dispatched is limited to nNum - used to prevent DispatchQueue hogging by any
     * one link. If more messages to dispatch after nNum, return true (so this is
     * added back to DispatchQueue)
     */
