...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 8, 2006 Time: 12:15:54 PM
...
public class AckList {
    ...
    /**
     * set whether or not we should meter the acks going out based on modcount
     * changes
     */
    ...
    void queueAcks(UDPLink link, UDPData.Type type) {
        ...
        if (bMeter_ && modCount_ == modCountLastAck_ && diff < UDPLink.ACK_METER_MILLIS && nAcksAtThisCnt_ > 5)
            return;
        ...
        while (nNum > 0) {
            ...
            // insert at front of queue (in order we send) so it gets sent quickly (to
            // minimize resends/timeouts)
            link.queue(new UDPData(type, ++nAckMessageID_, (short) 1, (short) 1, buffer.array(), buffer.arrayOffset(),
                    buffer.position(), UDPData.USER_TYPE_UNSPECIFIED), nIndex++);
        }
    ...
    public void ackTest(boolean bShortTest, int size, float iters) {
        if (false) {
            int acks[] = {200, 201, 202, 205, 204, 203, 197, 195, 196, 198, 199, 180, 185, 190, 192, 193, 195, 194, 183,
                    196, 197, 201, 184, 182, 189, 186, 188, 187, 181, 191, 182};
