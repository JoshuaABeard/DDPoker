...
public class UDPMessage {
    ...
    /**
     * Get session id - used to track if messages coming in are from a new session
     * or an old session
     */
    ...
    public void write(UDPServer server) throws IOException {
        ...
        if (outBuffer.position() != HEADER_SIZE)
            ApplicationError.assertTrue(false,
                    "Mismatched header size " + outBuffer.position() + " != HEADER_SIZE of " + HEADER_SIZE);
        ...
        if (outBuffer.capacity() != outBuffer.position())
            ApplicationError.assertTrue(false,
                    "Capacity " + outBuffer.capacity() + " != Position " + outBuffer.position());
        ...
        while (!bDone) {
            ...
            // UDP will either send all or nothing - wait and try again. This is very
            // unlikely to happen
            if (outBuffer.hasRemaining()) {
                ...
                if (nSleep >= WRITE_TIMEOUT_MILLIS) {
                    throw new SocketTimeoutException("UDP send timeout. Packet size: " + outBuffer.remaining()
                            + " send buffer size: " + channel.socket().getSendBufferSize() + " recv buffer size: "
    ...
    //
    // NOTE: reading is done single-threaded via UDPServer, so we can share objects
    // for perf
    //
    ...
    UDPMessage(UDPServer server, ByteBuffer inBuffer, InetSocketAddress to, InetSocketAddress from) {
        ...
        if (inBuffer.limit() < HEADER_SIZE) {
            throw new ApplicationError(ErrorCodes.ERROR_INVALID_MESSAGE,
                    "Invalid message from " + Utils.getAddressPort(from) + " - too small.  Expected at least "
                            + HEADER_SIZE + " bytes, received " + inBuffer.limit(),
                    null);
        }

        // calculate checksum
        _crc32.reset();
        _crc32.update(inBuffer.array(), inBuffer.arrayOffset(), HEADER_SIZE - CRC_SIZE); // CRC is on header less CRC at
                                                                                            // end
        ...
        // validate checksum - used to verify this is a DD message and not some random
        // message
        if (crcRead != crcCalc) {
            // throw invalid message error so socket is shutdown
            throw new ApplicationError(ErrorCodes.ERROR_INVALID_MESSAGE, "Invalid message from "
                    + Utils.getAddressPort(from) + " - CRC mismatch. crcRead " + crcRead + " != crcCalc " + crcCalc,
                    null);
        }
    ...
    public String toString() {
        ...
        return "From: " + srcID_ + " (" + Utils.getAddressPort(srcAddrActual_) + "/"
                + Utils.getAddressPort(srcAddrApparent_) + ")" + " to: " + dstID_ + " ("
                + Utils.getAddressPort(dstAddr_) + ") [" + nSize + " " + (nSize == 1 ? "part" : "parts") + "]";
    }
    ...
    boolean DEBUG_isSinglePingAck() {
        return getNumData() == 1
                && (getData(0).getType() == UDPData.Type.PING_ACK || getData(0).getType() == UDPData.Type.MTU_ACK);
    }
    ...
    public void DEBUG_log(UDPLink link) {
        for (UDPData data : data_) {
            if (data.getType() == UDPData.Type.MTU_TEST)
                continue;
