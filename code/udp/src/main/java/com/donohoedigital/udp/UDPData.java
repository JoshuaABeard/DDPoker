...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 2, 2006 Time: 10:43:58 AM
 * To change this template use File | Settings | File Templates.
 */
public class UDPData {
    // data types enum
    public enum Type {
        PING_ACK("ping-ack"), MESSAGE("message"), HELLO("hello"), GOODBYE("good-bye"), MTU_TEST("mtu-test"), MTU_ACK(
                "mtu-ack");
     ...
     */
    public UDPData(Type nType, int nID, short nPartID, short nParts, byte[] data, int nOffset, int nLength,
            byte nUserType) {
        nType_ = nType;
        nID_ = nID;
        nPartID_ = nPartID;
    ...
    public void put(ByteBuffer buffer) {
        ...
        if (nLength_ > 0)
            buffer.put(data_, nOffset_, nLength_);
    }
    ...
    void combine(ArrayList<UDPData> array) {
        ...
        nLength_ = nuLength;
        data_ = nu;
        // nNumParts_ is kept the same so we know how many parts it took to send (again,
