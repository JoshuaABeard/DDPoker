...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 4, 2006 Time: 11:04:19 AM
...
public class UDPManager extends Thread implements Comparator<UDPLink> {
    ...
    public void run() {
        ...
        // we use a blocking queue as our work "to do" list. It blocks until another
        // thread
        // puts something on the queue. We put several types of Objects. The QUIT and
        // SENDALL instances are
        // used to respectively quit (cleanup) and send all queued messages on each
        // link. A UDPLink on the
        // work queue means to send messages to that queue. A UDPMessage on the queue
        // means it is an incoming
        ...
        while (!bDone_) {
            try {
                msg = queue_.take();
                if (msg == QUIT || bDone_) {
                    /* continue */ } else if (msg == SENDALL)
                    processSendAll();
                else if (msg == SENDACK)
                    processSendAcksPing();
                else if (msg instanceof UDPMessage)
                    processMessage((UDPMessage) msg);
                else if (msg instanceof UDPLink)
                    processSend((UDPLink) msg);
                else if (msg instanceof CloseLink)
                    processRemove((CloseLink) msg);
    ...
    public void finish() {
        ...
        // set done flag and send quit message to wakeup queue (if it is waiting on
        // something)
        bDone_ = true;
        timer_.cancel();
    ...
    private void processMessage(UDPMessage msg) {
        // Get link for this message. This will create a new link if this is a new
        // message unless the
        // message contains a GOODBYE UDPData or is only acks, in which case we ignore
        // it (it may be a GOODBYE/acks arriving
        // after the link was closed)
        UDPLink link = getLink(msg.getSourceID(), msg.getDestinationIP(), msg.getSourceIPApparent(),
                !msg.requiresExistingLink());
    ...
    private UDPLink getLink(UDPID id, InetSocketAddress local, InetSocketAddress remote, boolean bCreateNew) {
        ...
        synchronized (links_) {
            ...
            for (UDPLink link : links_) {
                // if ID matches, local must be null (ID only lookup) or if local is specified,
                // local must match
                // to ensure we get the right link
                if (!id.isUnknown() && link.getID().equals(id) && (local == null || link.getLocalIP().equals(local)))
                    return link;

                // if we find a match for the remote, same check on local
                if (link.getRemoteIP().equals(remote) && (local == null || link.getLocalIP().equals(local))) {
                    // if the ID was previously unknown, this is the first response back ... so set
                    // the ID
                    if (link.getID().isUnknown()) {
                        if (!id.isUnknown()) {
                            link.setID(id);
                            if (UDPServer.DEBUG_CREATE_DESTROY)
                                logger.debug("ID updated " + link);
                        }
                    }
                    // if the id is not unknown, it changed for some reason, so update (shouldn't
                    // really occur)
                    else if (!id.isUnknown()) {
                        // TODO: what to do if ID changes? Will this actually occur?
                        logger.warn("ID changed at addr " + Utils.getAddressPort(remote) + " from " + link.getID()
                                + " to " + id);
                        link.setID(id);
                    }
                    return link;
                ...
                //
                // Note: if there happend to be a match on ID and/or remote and the local didn't
                // match, then that
    ...
    public String getStatusHTML(Comparator<UDPLink> comparator) {
        ...
        int num = links.size();
        table.addColumn(new TableColumn(num + (num == 1 ? " Player" : " Players"), TableColumn.VALIGN.TOP,
                TableColumn.HALIGN.LEFT, sHeaderColor));
        ...
        sb.append("In: ").append(Utils.formatSizeBytes(in.getAverageLong())).append("/sec (")
                .append(Utils.formatSizeBytes(in.getHigh())).append(" high - ")
                .append(Utils.formatSizeBytes(in.getPeak())).append(" peak)").append(",&nbsp;&nbsp;").append("Out: ")
                ...
                .append(getMessageOnOutGoingQueue()).append(" msgs - ").append(getPeakMessageOnOutGoingQueue())
                .append(" peak)");
