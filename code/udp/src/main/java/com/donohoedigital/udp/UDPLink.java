...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 3, 2006 Time: 9:48:43 AM To
...
public class UDPLink {
    ...
    void finish(boolean bNotifyManager) {
        if (bDone_)
            return; // don't run finish multiple times
    ...
    /**
     * Queue a message of the given type with given data at specifyied offset and
     * length. Need synchronized due to calls to nextID()
     */
    public synchronized void queue(UDPData.Type type, byte[] data, int offset, int length, byte nUserType) {
        UDPData part;

        // no more messages if good-bye in progress (check here also to avoid alloc new
        // objects)
        if (bGoodbyeInProgress_)
            return;
        ...
        if (length == 0) {
            ...
            if (length % MAX_DATA_SIZE != 0)
                nParts++;

            // unlikely to happen, but always safe to check. This allows
            // max size of message (UDPMessage.DATA_SIZE - UDPData.HEADER_SIZE) *
            // Short.MAX_VALUE,
            // or approx. 44,169,916 bytes.
            if (nParts > UDPData.MAX_PARTS) {
                ApplicationError.assertTrue(false,
                        "Data too big to send: " + length + " bytes (max: " + (MAX_DATA_SIZE * UDPData.MAX_PARTS)
                                + " bytes at " + MAX_DATA_SIZE + " per chunk ... MTU=" + getMTU());
            }
            ...
            if (nPartID != nParts) {
                ApplicationError.assertTrue(false, "# parts mismatch - expected " + nParts + " but got " + nPartID
                        + " for length " + length + " and MAX_DATA_SIZE " + MAX_DATA_SIZE);
            }
        }
    }
    ...
    void queue(UDPData part, int nIndex) {
        ...
        // stats
        if (part.getType() != UDPData.Type.PING_ACK && part.getType() != UDPData.Type.MTU_ACK)
            stats_.recordDataOut();
    }
     ...
     *
     * 1) Adds acks to the queue 2) check for resends - any message not ack'd in
     * reasonable time is marked for resending 3) sends all queued messages (acks,
     * resends, new messages)
     */
    void sendAll() {
        if (bDone_)
            return;
        ...
        synchronized (sendQueue_) {
            ...
            LOOP : while (iter.hasNext() && resendCNT < maxResends) {
                ...
                if (data.elapsed() > minAck) {
                    switch (type) {
                        case MTU_TEST :
                            if (data.getSendCount() == mtuMaxAttempts) {
                                if (UDPServer.DEBUG_MTU) {
                                    logger.debug("  MTU failed - " + data.getID() + " bytes (attempted "
                                            + mtuMaxAttempts + " times) " + toStringNameIP());
                                }
                    ...
                    if (UDPServer.DEBUG_RESEND) {
                        logger.debug("  **** RESEND " + data.elapsed() + " ms (min: " + minAck + "): " + data + " "
                                + toStringNameIP());
                    }
    ...
    private void sendAcks(boolean bSendImmediate) {
        if (acks_ == null || bGoodbyeInProgress_ || bDone_)
            return;

        // send out acks (unless it is empty)
        if (UDPServer.DEBUG_ACKS_OUT)
            logger.debug("ACKS out: " + acks_);
    ...
    void send(UDPMessage msg) {
        try {
            ...
            if (UDPServer.DEBUG_OUTGOING && !msg.DEBUG_isSinglePingAck()) {
                // logger.debug("Sending to " + Utils.getAddressPort(getRemoteIP()) + " ["+
                // msg.getNumData() +"]:");
                msg.DEBUG_log(this);
            }
            ...
            if (nLastErrorCnt_ > 0) {
                logger.info(Utils.getAddressPort(local_) + " send successful.  Last send error (" + sLastError_
                        + ") occurred a total of " + (nLastErrorCnt_ + 1) + " times");
            }
            nLastErrorCnt_ = 0;
            sLastError_ = null;
        } catch (Throwable ioe) {
            ...
            if (sError.equals(sLastError_)) {
                ...
                if (nLastErrorCnt_ % 10 == 0) {
                    logger.error(Utils.getAddressPort(local_) + " last send error (" + sError + ") repeated "
                            + nLastErrorCnt_ + " times");
                }
                return;
            }
    ...
    private boolean aliveCheck() {
        ...
        if ((nMessageID_ == 0 && acks_ == null) || bDone_)
            return false;

        // first time through, set to current time
        long now = System.currentTimeMillis();
        if (lastMessageReceived_ == 0)
        ...
        if (elapsed > TIMEOUT_MILLIS) {
            if (UDPServer.DEBUG_TIMEOUT)
                logger.debug("Timeout after " + elapsed + " millis on " + Utils.getAddressPort(remote_)
                        + " (new session " + localSessionID_ + ")");
    ...
    void processMessage(UDPMessage msg) {
        ...
        // debug
        // if (UDPServer.DEBUG_INCOMING && !msg.DEBUG_isSinglePingAck())
        // logger.debug("Incoming on " + this + " ["+ msg.getNumData() +"] ");
            ...
            switch (type) {
                ...
                case MESSAGE :
                    ...
                    if (!acks_.contains(data) && incomingQueue_.addMessage(data)) {
                        if (UDPServer.DEBUG_INCOMING) {
                            logger.debug(
                                    "  IN  " + data + " (" + incomingQueue_.size() + " queue) " + toStringNameIP());
                ...
                case MTU_TEST :
                    ...
                    if (!mtuAcks_.contains(data)) {
                        if (UDPServer.DEBUG_MTU) {
                            // logger.debug(" TST-"+tstid+" " + data + " (" + msg.getPacketLength() +" pkt
                            // size) " + toStringIPs());
                        }
    ...
    /**
     * New session - need to clear all queues since message ids are now different
     */
    private void newRemoteSession(long newSessionID) {
        ...
        if (!bFirstMessageFromRemote) {
            // logger.info("Remote session changed from " + remoteSessionID_ + " to " +
            // newSessionID + " on " + this);
    ...
    private void processAcks(UDPData data, boolean bMTUAcks) {
        ...
        if (!bMTUAcks && acks.getSessionID() != localSessionID_) {
            ...
            // this is a no-op
            if (hello()) {
                if (UDPServer.DEBUG_ACKS_IGNORED)
                    logger.debug("ACKS ignored, HELLO sent: " + acks);
            }
            return;
        }
        ...
        synchronized (sendQueue_) {
            ...
            while (iter.hasNext()) {
                ...
                if (type == UDPData.Type.MTU_TEST && !bMTUAcks)
                    continue;
                if (type != UDPData.Type.MTU_TEST && bMTUAcks)
                    continue;
                ...
                if (acks.contains(qData)) {
                    ...
                    if (UDPServer.DEBUG_ACKS_IN_DETAIL && !bMTUAcks)
                        logger.debug("  " + ACK + " " + qData + " average round trip is " + stats_.getAverage() + " "
                                + toStringNameIP());
            ...
            if (UDPServer.DEBUG_ACKS_IN && nOldSize != sendQueue_.size() && !bMTUAcks) {
                logger.debug(ACK + "S in " + acks + " from " + getName() + ", queue now has " + sendQueue_.size()
                        + " messages (" + (nOldSize - sendQueue_.size()) + " removed)");
            }
        }
    }
    ...
    /**
     * Class to track average time for ack to come back after being received. Tracks
     * last 100
     */
    public static class UDPStats {
        ...
        public String toString() {
            ...
            sb.append("AVG: " + getAverage() + ", OUT: " + dataOut + ", RE: " + dataresend + ", IN: " + dataIn
                    + ", DUP: " + dataDups + ", BIN: " + Utils.formatSizeBytes(bytesIn) + ", BOUT: "
