...
public class DDTabbedPaneUI extends MetalTabbedPaneUI {
    ...
    public DDTabbedPaneUI(DDTabbedPane tab, String sBevelStyle) {
        tab_ = tab;
        if (sBevelStyle != null)
            bevel_ = new DDBevelBorder(sBevelStyle, DDBevelBorder.RAISED); // used for colors
    }
    ...
    // can't find the logic for the gap, so hack it here (to paint bg, border all
    // the way to content)
    protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h,
            boolean isSelected) {
        if (tabPlacement == LEFT)
            w += 2;
        if (tabPlacement == LEFT && tabIndex == tab_.getTabCount() - 1)
            h += 2;
    ...
    protected void paintText(Graphics g1, int tabPlacement, Font font, FontMetrics metrics, int tabIndex, String title,
     ...
     */
    protected void paintFocusIndicator(Graphics g1, int tabPlacement, Rectangle[] rects, int tabIndex,
        ...
        if (tab_.hasFocus() && isSelected) {
            ...
            g.drawRect((focusRect.x - xgap), (focusRect.y - ygap), focusRect.width + (xgap * 2) - wadj,
