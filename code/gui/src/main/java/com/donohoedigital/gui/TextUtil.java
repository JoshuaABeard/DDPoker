...
public class TextUtil {
    ...
    public TextUtil(Graphics2D g, Font fFont, String sText, float fLineSpacing) {
        ...
        for (int i = 0; i < sText.length(); i++) {
            if (sText.charAt(i) == SEP)
                numLines++;
        }
        metrics = g.getFont().getLineMetrics(A2Z, frc);
        lineHeight = (float) metrics.getDescent() + (metrics.getAscent() * .75f); // adjust for space above chars for
                                                                                    // accents and stuff
        lineSpacing = lineHeight * fLineSpacing;

        totalHeight = (lineHeight * numLines) + ((numLines - 1) * lineSpacing);
        // float starty = (float) (y - nTotalHeight/2) + nHeight; // add height once to
        // adjust
    ...
    /**
     * Prepare to draw string associated with TextMetrics - sets font angle/size
     * transforms and anti alias flag
     */
    ...
    public void drawString(Color cForeground, Color cShadow, int nAlign) {
        ...
        while (!bDone) {
            ...
            if (nAlign == SwingConstants.CENTER)
                xx -= (stringbounds.getWidth() / 2);
            else if (nAlign == SwingConstants.RIGHT)
                xx -= (stringbounds.getWidth());
            double yy = starty;
