...
public class BaseFrame extends JFrame implements DDWindow {
    ...
    public void display(boolean bFull) {
        ...
        if (bFull) {
            SwingUtilities.invokeLater(this::setFullScreenOn);
        }
    }
    ...
    public void setDisplayMode(DisplayMode dm) {
        if (!isFullScreen() || !isDisplayChangeSupported())
            return;
        if (dm == null)
            return;
        dmLastSet_ = dm;
    ...
    /**
     * Turn full screen mode on. Returns true if succeeded. False if failed (i.e.,
    ...
    public boolean setFullScreenOn() {
        ...
        if (!isFullScreen()) {
            if (bActive_)
                bIgnoreNextDeactive_ = true;

            // order of following calls is important
            dispose();
            setUndecorated(true);
            setFullScreenWindow(this);
            // setIgnoreRepaint(true); // seems this is only good for animated apps, not
            // swing
            setResizable(false);
            return true;
        }
    ...
    /**
     * Turn full screen mode off. Returns true if succeeded. False if failed (i.e.,
     * not currently in full screen mode).
     */
    ...
    private class BaseFrameWindowAdapter extends WindowAdapter {
        /**
         * If re-iconifying after alt-tabbing from fullscreen mode, we need to validate
         * to make sure window is relayed out properly
         */
        @Override
        ...
        /**
         * If window deactivated in full screen mode, need to reset values to work
         * around alt-tab bug
         */
        @Override
    ...
    public class Modal {
        ...
        /**
         * Sets up DELETE / BACKSPACE / SPACE as keys which end modal and then calls
         * beginModal()
         */
        ...
        void beginModal(boolean bLog) {
            ...
            try {
                ApplicationError.assertTrue(SwingUtilities.isEventDispatchThread(), "Not in swing thread",
                ...
                while (bModal_) {
                    ...
                    if (!bModal_)
                        logger.warn("***** GUI dispatching when not modal: " + event);
