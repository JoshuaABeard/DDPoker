...
public class DDTable extends JTable implements DDTextVisibleComponent, MouseListener {
    ...
    public DDTable(String sName, String sStyle, String[] columnnames, int[] columnwidths) {
        ...
        for (int i = 0; i < columnwidths.length; i++) {
            ...
            if (headerFG_ != null && headerBG_ != null)
                col.setHeaderRenderer(new DDHeaderRenderer());
            this.addColumn(col);
        }
    }
    ...
    public void mouseReleased(MouseEvent e) {
        if (exporter_ == null && menuitems_ == null)
            return;
        ...
        if (exporter_ != null)
            nMenuCnt++;
        if (menuitems_ != null && menuitems_.isItemsToBeAdded(this))
            nMenuCnt++;
    ...
    /**
     * Get contents of table as a csv string, using platforms' default line end
     * character
     */
    public String toCSV(boolean bIncludeHeader) {
        ...
        if (bIncludeHeader && getTableHeader() != null) {
            ...
            for (int i = 0; i < nColCnt; i++) {
                ...
                if (oValue != null)
                    sb.append(Utils.encodeCSV(oValue.toString()));
            }
            sb.append(sSep);
        }
    ...
    private class DDHeaderRenderer extends DefaultTableCellRenderer {
        public DDHeaderRenderer() {
            super();
            if (headerFG_ != null)
                setForeground(headerFG_);
            if (headerBG_ != null)
                setBackground(headerBG_);
        }
        //
        ...
        // headerFG_.darker(), headerBG_.brighter()));
        // }
        // return c;
        // }
    }
