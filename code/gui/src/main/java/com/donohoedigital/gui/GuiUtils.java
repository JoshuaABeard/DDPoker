...
public class GuiUtils {
    ...
    static final JTextComponent.KeyBinding[] MAC_CUT_COPY_PASTE = {
            new JTextComponent.KeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.META_DOWN_MASK),
                    DefaultEditorKit.copyAction),
            new JTextComponent.KeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_V, InputEvent.META_DOWN_MASK),
                    DefaultEditorKit.pasteAction),
            new JTextComponent.KeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.META_DOWN_MASK),
    ...
    public static DDComponent getDDComponent(Object o) {
        if (o instanceof DDComponent)
            return (DDComponent) o;

        if (o instanceof Component) {
            Component c = (Component) o;
            c = c.getParent();
            while (c != null) {
                if (c instanceof DDComponent)
                    return (DDComponent) c;
                c = c.getParent();
            }
        }
    ...
    /**
     * Return component (or its nearest ancestor) that is opaque and has a
     * completely opaque background color
     */
    ...
    /**
     * Sets font of all children of container to font (container itself is not set
     * to prevent infinite loops if called from setFont itself)
     */
     ...
     * not set to prevent infinite loops if called from setForeground itself). If
     * the child is a JTextComponent (or subclass), the caret color is also set
     */
    ...
    public static void setOpaqueChildren(Container container, boolean b) {
        Component[] children = container.getComponents();
        for (Component aChildren : children) {
            if (aChildren instanceof JPanel || aChildren instanceof JLabel || aChildren instanceof Box
            ...
            if (token.length() == 1 && (first < '0' || first > '9')) {
                ...
                if (needed == 0) {
                    ...
                    if (type == 'M') {
                        ...
                        path.curveTo(xstart + nums[0], ystart + nums[1], xstart + nums[2], ystart + nums[3],
    ...
    /**
     * Add standard escape key actions to the component - action is invoked when one
     * of DELETE / BACKSPACE / SPACE are pressed.
     */
    public static void addEscapeKeyActions(JComponent comp, AbstractAction action) {
        addKeyAction(comp, JComponent.WHEN_IN_FOCUSED_WINDOW, "endmodaldelete", action, KeyEvent.VK_DELETE, 0);
        addKeyAction(comp, JComponent.WHEN_IN_FOCUSED_WINDOW, "endmodalbackspace", action, KeyEvent.VK_BACK_SPACE, 0);
        addKeyAction(comp, JComponent.WHEN_IN_FOCUSED_WINDOW, "endmodalspace", action, KeyEvent.VK_SPACE, 0);
    }
    ...
    public static void removeEscapeKeyActions(JComponent comp) {
        removeKeyAction(comp, JComponent.WHEN_IN_FOCUSED_WINDOW, "endmodaldelete", KeyEvent.VK_DELETE, 0);
        removeKeyAction(comp, JComponent.WHEN_IN_FOCUSED_WINDOW, "endmodalbackspace", KeyEvent.VK_BACK_SPACE, 0);
        removeKeyAction(comp, JComponent.WHEN_IN_FOCUSED_WINDOW, "endmodalspace", KeyEvent.VK_SPACE, 0);
    }

    /**
     * Method to add a key action to a component using the ActionMap/InputMap way.
     */
    public static void addKeyAction(JComponent comp, int nWhen, String sActionName, Action action, int key,
            int key_mods) {
     ...
     * Remove a key action
     */
    public static void removeKeyAction(JComponent comp, int nWhen, String sActionName, int key, int key_mods) {
    ...
    /**
     * get first DDWindow parent of this component (will find InternalWindow before
     * the parent Frame)
     */
    ...
    /**
     * do we draw this component anti-aliased? (looks bad when antialiased in
     * smaller fonts)
     */
    ...
    /**
     * do we draw this font anti-aliased at given scale? (looks bad when antialiased
     * in smaller fonts)
     */
    ...
    /**
     * Internal frame border - copy to adjust how border is drawn to avoid white
     * dots at corner
     */
    @SuppressWarnings({"PublicInnerClass"})
    ...
    /**
     * print to jpg image, scaling image into width x height, return BufferedImage
     * for use by printImageToFile
     */
