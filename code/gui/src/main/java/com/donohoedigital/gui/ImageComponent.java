...
public class ImageComponent extends JComponent implements Icon {
    ...
    /**
     * New ImageComponent from image name (lookup in ImageDef) and given scale
     */
    public ImageComponent(String sName, double dScaleFactor) {
        ...
        if (def.isComposite()) {
            ...
            for (int i = 0; i < nNum; i++) {
                ...
                if (composite == null) {
                    throw new ApplicationError(ErrorCodes.ERROR_FILE_NOT_FOUND,
                            "Composite component not found: " + names[i], (String) null, null);
                }
    ...
    public void changeName(String sName) {
        bimage_ = ImageConfig.getBufferedImage(sName);
        dInitialSize_ = new Dimension((int) (getImageWidth() * dScaleFactor_),
                (int) (getImageHeight() * dScaleFactor_));
        setPreferredSize(dInitialSize_);
        imagebounds_ = new Rectangle(0, 0, getImageWidth(), getImageHeight());
    }

    /**
     * if the image has transparent bits, it may be drawn over a parent that is
     * tiled. That image is specified here, and must be used with setBuffer(true),
     * as the parent's image isn't painted in non-bufferred mode. See PokerGameboard
     * for an example.
     */
    ...
    /**
     * set whether to buffer the image. if the image is large, painted often and
     * scaled, it can be a big performance improvment to buffer the scaled image.
     */
    ...
    /**
     * Return true if given x,y in image is non-transparent, meaning there is some
     * color, even if not completely opaque
     */
    public static boolean isNonTransparent(BufferedImage bimage, int imagex, int imagey) {
        ...
        if (imagex < 0 || imagex >= bimage.getWidth() || imagey < 0 || imagey >= bimage.getHeight())
            return false;
        ...
        // catch this just in case
            ...
            logger.warn("Caught the out-of-bounds exception in findPieceInTerritory: " + " imagex=" + imagex
                    + " imagey=" + imagey + " imagewidth=" + bimage.getWidth() + " imageheight=" + bimage.getHeight());
            alpha = 0;
        }
    ...
    public BufferedImage getImage() {
        if (bComposite_)
            throw new UnsupportedOperationException("getImage() not supported with composite images");
    ...
    /**
     * Set whether the image should be tiled if display area is larger than image
     * (this method sets setScaleToFit to !bTile)
     */
    ...
    /**
     * Set the bounds of the image we should use (in the coordinate space of the
     * original image). Used to clip areas of image. Only used if isScaleToFit() is
     * true.
     */
    ...
    protected void paintComponent(Graphics g1) {
        ...
        if (scaleToFit_) {
            if (custom_ != null && bUseCustom_) {
                ...
                // logger.debug("IMAGECOMPONENT " + sName_ + " size: " + getWidth() + "," +
                // getHeight()
                // + " - bounds: " + bounds_ + " imagebounds: " + imagebounds_);
                if (bBuffer_) {
                    // logger.debug("IMAGECOMPONENT " + sName_ + " size: " + getWidth() + "x" +
                    // getHeight()
                    // + (buffer_ == null ? " [null buffer" : " [buffer: " + buffer_.getWidth() +
                    // "x" + buffer_.getHeight()) +
                    // "] refresh: " + bRefreshBuffer_);
                    boolean bSizeChanged = buffer_ == null
                            || (buffer_.getWidth() != getWidth() || buffer_.getHeight() != getHeight());
    ...
    protected void renderImage(Graphics2D g, Image drawThis) {
        if (!bComposite_) {
            g.drawImage(drawThis, 0, 0, getWidth(), getHeight(), imagebounds_.x, imagebounds_.y,
                    (imagebounds_.x + imagebounds_.width), (imagebounds_.y + imagebounds_.height), this);
        } else {
            renderComposite(g);
        }
    }
    ...
    private void renderComposite(Graphics2D g) {
        ...
        // logger.debug("Orig size: " + origWidth +"x"+origHeight + " drawing to " +
        ...
        for (int i = 0; i < composites_.length; i++) {
            ...
            g.drawImage(img, (int) Math.round(newX), (int) Math.round(newY), (int) Math.ceil(newW),
                    (int) Math.ceil(newH), this);
        }
    }
    ...
    protected void paintParentTile(Graphics2D g) {
        ...
        Rectangle bounds = new Rectangle(0, 0, getWidth() + pRepaint.x, getHeight() + pRepaint.y);
    ...
    /**
     * Draw this image at the given coordinates, scaled to the width/height. Used to
     * draw the component when it is not part of a hierarchy
     */
    ...
    private void drawImageAt(Graphics2D g, Image image, int x, int y, int width, int height) {
        g.drawImage(image, x, y, (x + width), (y + height), imagebounds_.x, imagebounds_.y,
                (imagebounds_.x + imagebounds_.width), (imagebounds_.y + imagebounds_.height), this);
    }
    ...
    private Image whatToDraw() {
        ...
        return (!isEnabled()) ? getDisabledImage() : (bHighlighted_) ? hiliteimage_ : bimage_;
    }

    /**
     * Set highlighted. If setting to true, the highlighted image is created on
     * demand (if not previously created)
     */
    ...
    /**
     * Get image for given name/scale. Images are cached, so this should only be
     * used for items that are reused frequently.
     */
    ...
    public double getIconScaleFactor(int width, int height) {
        ...
        // getting max scales image as big as it can go (so one dimension equals
        // width/height)
        return Math.min((double) width / (double) dInitialSize_.width, (double) height / (double) dInitialSize_.height);
    }

    /**
     * This ImageComponent can be used as an icon (e.g., in a button) at the same
     * time it is used elsewhere. Use this to set the size that the icon is drawn,
     * which is seperate from the size this image is drawn. The default scale is
     * 1.0. The iconScale is multiplied by the width and height of the
     * ImageComponent, which is defined by the original image size and the scale
     * passed in at construction time. Clears icon height/width if previously set.
     */
    ...
    /**
     * Override default icon height (doens't change icon size, but space available
     * to draw icon)
     */
    ...
    /**
     * Override default icon width (doens't change icon size, but space available to
     * draw icon)
     */
    ...
    public int getIconHeight() {
        if (dIconHeight_ != null)
            return dIconHeight_.intValue();
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        paintIcon(g, x, y, width, height, iconwidth, iconheight, nVerticalAlignment_, nHorizontalAlignment_, null);
    }

    /**
     * Actually paint icon
     */
    private void paintIcon(Graphics g, int x, int y, int width, int height, int iconwidth, int iconheight,
            int nVertAlign, int nHorizAlign, Image image) {
    ...
    /**
     * Returns a lightweight object that implements Icon, using this and the current
     * icon settings and image from this as its settings. Useful when same image
     * needs to represent two different sized icons at same time.
     */
    ...
    public Icon getUniqueIcon(boolean bDisabled) {
        ...
        return new UniqueIcon(this, getIconWidth(), getIconHeight(), getActualIconWidth(), getActualIconHeight(),
                nVerticalAlignment_, nHorizontalAlignment_, image);
    }
    ...
    private class UniqueIcon implements Icon {
        ...
        UniqueIcon(ImageComponent ic, int width, int height, int iconwidth, int iconheight, int nVertAlign,
                int nHorizAlign, Image image) {
            this.ic = ic;
            this.width = width;
            this.height = height;
        ...
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ic.paintIcon(g, x, y, width, height, iconwidth, iconheight, nVertAlign, nHorizAlign, image);
        }
    }
