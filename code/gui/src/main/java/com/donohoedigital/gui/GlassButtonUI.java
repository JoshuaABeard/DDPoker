...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 22, 2005 Time: 1:34:16 PM
...
public class GlassButtonUI extends DDButtonUI {
    ...
    protected void paintBackground(Graphics2D g, GlassButton gb) {
        ...
        GradientPaint p = new GradientPaint(0, (float) buttonbounds_.getMaxY(), Color.white, 0,
                (float) buttonbounds_.getY(), GuiUtils.TRANSPARENT);
        g.setPaint(p);
        g.fillRect(0, 0, (int) width, (int) height);
        g.fillRect(0, 0, (int) width, (int) height);
    ...
    protected void paintReflections(Graphics2D g, GlassButton gb) {
        ...
        Shape buttonReflection = new Rectangle2D.Double(buttonbounds_.getX() - nLess, buttonbounds_.getY() - nLess,
                buttonbounds_.getWidth(), buttonbounds_.getHeight());
        ...
        GradientPaint p = new GradientPaint(-5, quarterHeight, BRIGHT_REFLECTION, width + 5, height - quarterHeight,
                GuiUtils.TRANSPARENT);
        g.setPaint(p);
        g.fill(topHighlite);

        // bottom hilite
        p = new GradientPaint(-5, quarterHeight, GuiUtils.TRANSPARENT, width + 5, height - quarterHeight,
                DARK_REFLECTION);
