...
public class GuiManager implements MouseListener {
    ...
    public static void init(DDComponent dd, String sName, String sStyleName) {
        ...
        if (!(dd instanceof DDImageButton) && !(dd instanceof DDScrollPane)) {
        ...
        if (dd instanceof DDTextField) {
            ...
            text.setErrorBackground(
                    StylesConfig.getColor(sStyleName + "." + sType + ".error.bg", text.getErrorBackground()));
        }
        ...
        if (dd instanceof DDNumberSpinner) {
            ...
            spin.setErrorBackground(
                    StylesConfig.getColor(sStyleName + "." + sType + ".error.bg", spin.getErrorBackground()));
        }
        ...
        if (dd instanceof DDListComponent) {
            ...
            list.setSelectionForeground(
                    StylesConfig.getColor(sStyleName + "." + sType + ".selection.fg", list.getSelectionForeground()));
            list.setSelectionBackground(
                    StylesConfig.getColor(sStyleName + "." + sType + ".selection.bg", list.getSelectionBackground()));
        }
        ...
        if (dd instanceof GlassButton) {
            ...
            glass.setOverColor(StylesConfig.getColor(sStyleName + "." + sType + ".over.bg", glass.getOverColor()));
            glass.setDownColor(StylesConfig.getColor(sStyleName + "." + sType + ".down.bg", glass.getDownColor()));
            glass.setDisabledColor(
                    StylesConfig.getColor(sStyleName + "." + sType + ".disable.bg", glass.getDisabledColor()));
            glass.setFocusColor(StylesConfig.getColor(sStyleName + "." + sType + ".focus.bg", glass.getFocusColor()));
        }

        if (dd instanceof PillColors) {
            PillColors pp = (PillColors) dd;
            pp.setGradientFrom(StylesConfig.getColor(sStyleName + "." + sType + ".gradfrom", pp.getGradientFrom()));
            pp.setGradientTo(StylesConfig.getColor(sStyleName + "." + sType + ".gradto", pp.getGradientTo()));
        }
    ...
    /**
     * Add mouse listener (used to display help for whatever DDComponent is in the
     * parent hierarchy)
     */
    ...
    /**
     * Reset the label using given params. Assumes label definition has {0} ... {N}
     * components.
     */
    ...
    public static String getDefaultHelp(DDComponent source) {
        ...
        sHelp = PropertyConfig.getStringProperty(sbHelpName.toString(), null, false);
        return sHelp;
    }
