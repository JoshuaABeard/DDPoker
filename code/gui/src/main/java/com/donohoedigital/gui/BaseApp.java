...
public abstract class BaseApp {
    ...
    /**
     * Subclass must implement - called from window closing event. If true return,
     * frame is closed and app exits
     */
    public abstract boolean okayToClose();

    /**
     * Called in some OS when Preferences menu item selected. Should be overridden
     * to do something.
     */
    ...
    /**
     * Called in some OS when About menu item selected. Should be overridden to do
     * something.
     */
    ...
    private void exitAfterWindowClosed(final int nCode) {
        SwingUtilities.invokeLater(() -> _exit(nCode));
    }
    ...
    // {
    // logger.debug("Mode " + sName + ": " + mode.getWidth() + "x" +
    // mode.getHeight() +
