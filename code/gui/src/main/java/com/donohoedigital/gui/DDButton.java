...
public class DDButton extends JButton implements DDHasLabelComponent, DDExtendedComponent, AncestorListener {
    ...
    public DDButton(String sName, String sStyleName) {
        ...
        bgHighlighted_ = new Color((bgNormal_.getRed() + 128) / 2, (bgNormal_.getGreen() + 128) / 2,
                (bgNormal_.getBlue() + 128) / 2);
        bgSelected_ = bgNormal_.brighter().brighter();
        fgNormal_ = getForeground();
        fgHighlighted_ = new Color((fgNormal_.getRed() + 128) / 2, (fgNormal_.getGreen() + 128) / 2,
                (fgNormal_.getBlue() + 128) / 2);
        fgSelected_ = fgNormal_.brighter().brighter();
        GuiUtils.addKeyAction(this, JComponent.WHEN_FOCUSED, "buttonenter", new GuiUtils.InvokeButton(this),
                KeyEvent.VK_ENTER, 0);
    }
    ...
    public void setBorderGap(int top, int left, int bottom, int right) {
        ...
        bNormal_ = BorderFactory.createCompoundBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED), bEmpty);
        bDown_ = BorderFactory.createCompoundBorder(BorderFactory.createBevelBorder(BevelBorder.LOWERED), bEmpty);
        setBorder(bNormal_);
    }
    ...
    public boolean isBorderPainted() {
        return super.isBorderPainted() && (isEnabled() || ((nDisableMode_ & DISABLED_BORDERLESS) == 0));
    }
    ...
    /**
     * Implemented to set rollover to false when this widget is added - swing does't
     * reset when this (or parent) is removed and later readded.
     */
