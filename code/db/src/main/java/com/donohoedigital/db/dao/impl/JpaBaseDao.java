...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Mar 13, 2008 Time: 11:01:56 AM
...
public abstract class JpaBaseDao<T extends BaseModel<ID>, ID extends Serializable> implements BaseDao<T, ID> {
    ...
    /**
     * Construct. Upon creation, determine the actual model class (must do at
     * runtime due to java generic's type erasure).
     */
    @SuppressWarnings("unchecked")
    public JpaBaseDao() {
        persistentModelClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass())
    ...
    /**
     * For calls returning single item using JPQL. Caller must format query properly
    ...
    /**
     * For calls returning single item using native SQL. Caller must format query
     * properly with numbered parameters (?#).
     */
    @SuppressWarnings({"unchecked"})
    ...
    /**
     * For calls returning non-paged lists. Caller must format query properly with
     * numbered parameters (?#).
     */
    ...
    /**
     * For calls returning non-paged lists with native query. Caller must format
     * query properly with numbered parameters (?#).
     */
    ...
    /**
     * For 'select count(x)...' JPQL methods. Caller must format query properly with
     * numbered parameters (?#).
     */
    ...
    /**
     * For 'select count(x)...' native SQL methods. Caller must format query
     * properly with numbered parameters (?#).
     */
    ...
    /**
     * For calls returning paged lists using JPQL. Caller must format query properly
     * with numbered parameters (?#).
     *
     * @param count
     *            Count returned from previous call to getCount or previous call to
     *            this. Passing this in saves an additional count query (used when
     *            paging through a list - only do the count at the start). May be
     *            null if isCountNeeded() is false
     * @param offset
     *            starting row
     * @param pagesize
     *            max rows to return. <= 0 means return all rows. If <= 0, offset
     *            must be 0
     */
    ...
    /**
     * For calls returning paged lists using native SQL. Caller must format query
     * properly with numbered parameters (?#).
     *
     * @param count
     *            Count returned from previous call to getCount or previous call to
    ...
    /**
     * For calls returning paged lists. Caller must format query properly with
     * numbered parameters (?#).
     *
     * @param count
     *            Count returned from previous call to getCount or previous call to
     ...
     *            must be 0
     * @param nativeQuery
     *            if true, indicates the query is native sql. Otherwise it is JPQL
     */
    @SuppressWarnings({"unchecked"})
    private PagedList<T> getPagedList(String query, Integer count, int offset, int pagesize, boolean nativeQuery,
            Object... params) {
        // validate
        if (offset < 0) {
        ...
        // if count should be provided (paging call), throw exception; otherwise get
        // result size (all rows)
    ...
    /**
     * Determine if count needs to be determined (if count is null or <= 0 and
     * pagesize > 0)
     */
    ...
    /**
     * Sets indexed params from varargs. Vararg can contain sub-array(s) of
     * Object[].
     */
