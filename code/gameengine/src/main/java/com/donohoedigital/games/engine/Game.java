...
public class Game extends TypedHashMap implements GameInfo, GamePlayerList, GameObserverList {
    ...
    /**
     * Name used in PropertyChangeEvents when players added/removed from this game.
     * On an add event, the added player is passed as the "new" property. On a
     * remove event, the removed player is passed as the "old" property.
     */
    public static final String PROP_PLAYERS = "_players_";

    /**
     * Name used in PropertyChangeEvents when players list is cleared during a game
     * load. The "old" property is the old num of players and the "new" property is
     * the new num players.
     */
    public static final String PROP_PLAYERS_LIST = "_playerslist_";

    /**
     * Name used in PropertyChangeEvents when observers added/removed from this
     * game. On an add event, the added observer is passed as the "new" property. On
     * a remove event, the removed observer is passed as the "old" property.
     */
    public static final String PROP_OBSERVERS = "_observers_";

    /**
     * Name used in PropertyChangeEvents when observers list is cleared during a
     * game load. The "old" property is the old num of observers and the "new"
     * property is the new num observers.
     */
    public static final String PROP_OBSERVERS_LIST = "_observerslist_";

    /**
     * Name used in PropertyChangeEvents when player order changed. The "old" value
     * is null and the "new" value is the player array.
     */
    public static final String PROP_PLAYER_ORDER = "_playerorder_";

    /**
     * Name used in PropertyChangeEvents when dice roll set/changed. The "old" value
     * is the previous dice roll and the "new" value is the new dice roll.
     */
    public static final String PROP_PLAYER_ORDER_DICE_ROLL = "_diceroll_";

    /**
     * Name used in PropertyChangeEvents when current player changed. The previous
     * player is passed as the "old" property and the new current player as the
     * "new".
     */
    public static final String PROP_CURRENT_PLAYER = "_current_";

    /**
     * Name used in PropertyChangeEvents when game loaded. The GameState used during
     * the load is passed as the "old" property value and the Game itself as the
     * "new".
     */
    public static final String PROP_GAME_LOADED = "_load_";

    /**
     * Name used in PropertyChangeEvents when game over. Both "old" and "new" are
     * null
     */
    ...
    public void setGameOver(boolean b) {
        ...
        if (b)
            firePropertyChange(PROP_GAME_OVER, null, null);
    }
    ...
    /**
     * Get next seq id for online messages. The very first time this is called on a
     * new game, 1 is returned (because getSeqID() returns 0 if none is there and we
     * increment)
     */
    ...
    /**
     * Add player to game. Added player passed as "new" value in PROP_PLAYERS event.
     */
    ...
    /**
     * Remove player from game. Removed player passed as "old" value in PROP_PLAYERS
     * event.
     */
    ...
    /**
     * Add observer to game. Added observer passed as "new" value in PROP_OBSERVERS
     * event. setObserver(true) called on the player.
     */
    ...
    /**
     * Return player with given id (not necessarily same as getPlayerAt, since the
     * order can change. Returns null if no player exists.
     */
     ...
     * @param nID
     *            player id
     * @param bSearchObservers
     *            search observer list if true
    ...
    public boolean isPlayerBefore(GamePlayer A, GamePlayer B) {
        ...
        int nNumPlayers = getNumPlayers();
        for (int i = 0; i < nNumPlayers; i++) {
            if (getPlayerAt(i).getID() == nA)
                return true;
            if (getPlayerAt(i).getID() == nB)
                return false;
        }
        return false;
    }
    ...
    /**
     * Store the dice roll determining player order - also change order of players
     * so winner is at index 0 (first)
     */
    ...
    /**
     * Fire player order change - used in cases where need to simulate what happens
     * when order changes, like changing player type
     */
    ...
    public void setCurrentPlayer(int i) {
        ...
        if (i == NO_CURRENT_PLAYER)
            currentPlayer_ = null;
        else
            currentPlayer_ = getPlayerAt(i);

        if (oldPlayer != null)
            oldPlayer.setCurrentGamePlayer(false);
        if (currentPlayer_ != null)
            currentPlayer_.setCurrentGamePlayer(true);
    ...
    /**
     * Record the state of this game for use in online play (no ext required)
     */
    ...
    public String getBegin() {
        if (isOnlineGame())
            return GameState.ONLINE_GAME_BEGIN;
    ...
    /**
     * Get default version of SaveDetails for use on normal save. Can be overriden
     */
    ...
    /**
     * Write the given GameState out and store it as our last save. All entries are
     * removed after writing (state.resetAfterReadWrite())
     */
    ...
    /**
     * Save & Write last saved game (must have previously called writeGame or
     * setLastGameState() such that getLastGameState() isn't null)
     */
    ...
    private synchronized void _loadGame(GameState state, boolean bProcessPhase) {
        ...
        // PHASE 4 - repeat process above with players, gamestate, territories custom
        // data, etc.
    ...
    /**
     * Process the phase loaded from the last loadGame(). Can only be called once
     * and only if loadGame is passed false.
     */
    public void processStartPhase() {
        if (sBeginPhase_ == null)
            return;
    ...
    /**
     * Supports reporting bound property changes. If <code>oldValue</code> and
     * <code>newValue</code> are not equal and the <code>PropertyChangeEvent</code>
     * listener list isn't empty, then fire a <code>PropertyChange</code> event to
     * each listener. This method has an overloaded method for each primitive type.
     * For example, here's how to write a bound property set method whose value is
     * an integer:
     *
     * <pre>
     ...
     */
    protected synchronized void firePropertyChange(final String propertyName, final Object oldValue,
     ...
     * <p>
     * A <code>PropertyChangeEvent</code> will get fired in response to setting a
     * bound property, such as <code>setFont</code>, <code>setBackground</code>, or
     * <code>setForeground</code>.
     * <p>
     * Note that if the current component is inheriting its foreground, background,
     * or font from its container, then no event will be fired in response to a
    ...
    /**
     * Removes a <code>PropertyChangeListener</code> for a specific property. If
     * listener is <code>null</code>, no exception is thrown and no action is
     * performed.
     *
     * @param propertyName
