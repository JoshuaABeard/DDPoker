 ...
 * @author Doug Donohoe
 */
public class Gameboard extends ImageComponent implements Scrollable, MouseMotionListener, MouseListener {
    ...
    public Gameboard(GameEngine engine, GameContext context, GameboardConfig gameconfig, boolean bGameMode,
        ...
        if (TESTING(EngineConstants.TESTING_AI_DEBUG)) {
            GuiUtils.addKeyAction(this, JComponent.WHEN_IN_FOCUSED_WINDOW, "debugai", new DebugAI(), KeyEvent.VK_X, 0);
            GuiUtils.addKeyAction(this, JComponent.WHEN_IN_FOCUSED_WINDOW, "debugai2", new DebugAI2(), KeyEvent.VK_V,
                    0);
            GuiUtils.addKeyAction(this, JComponent.WHEN_IN_FOCUSED_WINDOW, "debugai3", new DebugAI3(), KeyEvent.VK_S,
                    0);
        }
    }
    ...
    /**
     * Clear mouse listeners - assumes scrollgameboard will propogate them Done
     * because scrollgameboard needs to get them first. This ensures the order.
     */
    ...
    /**
     * override to set preferred size based on parent's size - we scale so that the
     * map is as big as possible w/out having to scroll in two dimensions (we scroll
     * in dimension with least room)
     */
    @Override
    ...
    /**
     * like getPreferredSize, except based on given dimension instead of parent's
     * size
     */
    ...
    /**
     * Set new size for this widget. Sets preferred size, adjusts paths and repaints
     */
    public void setNewSize(Dimension size, boolean bRepaint) {
        ...
        if (bRepaint)
            context_.getFrame().repaint();
    }

    /**
     * Repaint visible portion of gameboard (typically that inside the
     * GameScrollboard viewport
     */
    public void repaintVisible(boolean bImmediate) {
        if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT))
            logger.debug("REPAINT VISIBLE " + (CNT++) + " immediate: " + bImmediate);
        if (scroll_ == null) {
            if (bImmediate) {
    ...
    /**
     * Return scale last used to convert points in gameboard space to current
     * dimensions <BR>
    ...
    /**
     * Return scale factor that one would multiply the getDefaultMapSize() width and
     * height by so that the one of the dimensions is exactly same as dSize and the
     * other is equal to or greater.
     */
    ...
    /**
     * Calculates the resize dimensions based on the starting smallest size. Calcs
     * 150%, 200%, 300% and 400%. Resizes board to smallest size
     */
    ...
    protected void paintComponent(Graphics g1) {
        ...
        if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT)) {
            logger.debug("REPAINT COMPONENT " + (CNT++) + " (" + getDebugColorName() + ") portion " + bounds_.x + ","
                    + bounds_.y + " " + bounds_.width + "x" + bounds_.height);
            // logger.debug(Utils.formatExceptionText(new Throwable()));
        }
        ...
        // draw label for AI
        if (DEBUG_AI)
            drawTerritories(g, PART_LABEL_AI);
        ...
        // JDD 2019
        // disabled in DD Poker 3.0p2 - realized not really used (probably since 1.0
        // actually)
        ...
        if (EngineBasePanel.PAINT_GROW_BOX && Utils.ISMAC && engine_ != null) {
            ...
            // an infinited loop
            if (!p.bPainting_ && origin.x + bounds_.x + bounds_.width > w && origin.y + bounds_.y + bounds_.height > h
                    && bounds_.width != 51 && bounds_.height != 52) {
    ...
    protected void drawTerritory(Graphics2D g, Territory t, int iPart) {
        ...
        if (!bounds_.intersects(territoryBounds))
            return;
     ...
     */
    protected void drawTerritoryPart(Graphics2D g, Territory t, GeneralPath path, Rectangle territoryBounds,
            int iPart) {
        switch (iPart) {
            case PART_FILL :
            ...
            case PART_BORDERS :
                ...
                if (stroke != null) {
                    ...
                    if (lastStroke_ == null || (lastStroke_.getLineWidth() != nNewStrokeWidth
            ...
            case PART_LABEL_AI :
                ...
                if (font == null)
                    return; // don't draw label if we don't know font
                ...
                // PART_LABEL_AI
                    ...
                    if (s != null) {
                        ...
                        tu2.prepareDraw(x, y, tp.getAngle(), dScale_ * aidebug_.getScale(),
                                getTerritoryLabelAntiAliased(t));
    ...
    private BasicStroke borderStroke_ = new BasicStroke((float) 1.0, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND);
    ...
    /**
     * Repaint a game piece container by figuring out what the actual territory is
     */
    ...
    /**
     * repaint given territory by repainting rectangle that contains it. Multiple
     * calls to this in a row tend to be coallesced by AWT.
     */
    ...
    /**
     * Repaint gamepiece, passing bImmediate based on whether the use triggered the
     * mouse movement or not (can enter through scrolling
     */
    ...
    protected void drawMouseItems(Graphics2D g) {
        ...
        if (TESTING(EngineConstants.TESTING_DEBUG_REPAINT_DETAILS)) {
            logger.debug(paintAtMouse_.getName() + " calling drawImage: " + x + "," + y + " " + r.width + "x" + r.height
                    + " scale: " + dScale_);
        }

        int nMoving = paintAtMouse_.getMovingQuantity();
        paintAtMouse_.drawImageAt(g, paintAtMouse_.getImageComponent(), nMoving, 0, nMoving, x, y, r.width, r.height,
                dScale_);
    }
    ...
    /**
     * repaint area around mouse equal to this gamepiece (we repaint the union of
     * the piece at the current and previous mouse positions, so old one is painted
     * over)
     */
    ...
    /**
     * Set board's mouse point to last AWT point (needed for case where last point
     * not known)
     */
    ...
    /**
     * Set new "last mouse point". Used by ScrollGameboard after resize & move.
     * Calls setLastMousePoint()
     */
    ...
    /**
     * Find items at last mouse point stored by gameboard (or set previously sith
     * setLastMousePoint)
     */
    ...
    private void findTerritoryAt(double x, double y, boolean bMouseAction) {
        ...
        if (old != territoryAtMouse_ || !bMouseAction) {
            if (old != null)
                fireMouseExited(old);
            if (territoryAtMouse_ != null)
                fireMouseEntered(territoryAtMouse_);
        }
    }
    ...
    private void findPieceAt(double x, double y, boolean bMouseAction) {
        ...
        if (old != pieceAtMouse_ || !bMouseAction) {
            if (old != null)
                fireMouseExited(old);
            if (pieceAtMouse_ != null)
                fireMouseEntered(pieceAtMouse_);
        }
    }
    ...
    private EngineGamePiece findPieceInTerritory(Territory t, double x, double y) {
        ...
        synchronized (t.getMap()) {
            ...
            while (pieces.hasNext()) {
                ...
                if (!piece.isVisible() || !piece.allowMouseOver())
                    continue;
    ...
    /**
     * Call allowTerritorySelection on all TerritorySelectionListeners. If any
    ...
    /**
     * Call allowGamePieceSelection on all GamePieceSelectionListeners. If any
     * return true, this returns true
     */
    ...
    private void processTerritorySelection(MouseEvent e) {
        if (nSelectionMode_ == SELECTION_MODE_OFF)
            return;
        ...
        if (territoryAtMouse_.isEdge())
            return;
        if (territoryAtMouse_.isDecoration())
            return;
    ...
    private void processGamePieceSelection(MouseEvent e) {
        if (nPieceSelectionMode_ == SELECTION_MODE_OFF)
            return;
    ...
    public void mouseExited(MouseEvent e) {
        ...
        if (e != null && contains(e.getX(), e.getY()))
            return;
        setMouseXY(-1, -1);
        setLastMousePoint();
        territoryAtMouseWhenPressed_ = null;
    ...
    /**
     * finds items under mouse and processes them (if they were under mouse when it
     * was pressed)
     */
    public void mouseReleased(MouseEvent e) {
        ...
        if (e.getButton() != MouseEvent.BUTTON1 && paintAtMouse_ != null)
            return;
    ...
    public void setCursor(Cursor c) {
        cursor_ = c;
        if (overrideCursor_ == null)
            super.setCursor(cursor_);
    }

    public void setOverrideCursor(Cursor c) {
        if (overrideCursor_ == c)
            return;
        overrideCursor_ = c;
        if (overrideCursor_ != null)
            super.setCursor(overrideCursor_);
        else
            super.setCursor(cursor_);
    }
}
