...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: May 18, 2006 Time: 11:12:02 AM
...
public class EngineWindow extends BaseFrame {
     ...
     * Constructor
     */
    public EngineWindow(GameEngine engine, GameContext context, String sName, int nDesiredMinWidth,
     ...
     */
    public void init(GamePhase gamephase, boolean bMain, Dimension size, boolean bFull, String sTitle,
            boolean bResizable) {
        ...
        // adjust
        if (size.width < DESIRED_MIN_WIDTH)
            size.width = DESIRED_MIN_WIDTH;
        if (size.height < DESIRED_MIN_HEIGHT)
            size.height = DESIRED_MIN_HEIGHT;

        // can't be bigger than screen size (and should be screen size in full)
        if (bFull || size.height > mode.getHeight())
            size.height = mode.getHeight();
        if (bFull || size.width > mode.getWidth())
        ...
        if (!Utils.ISMAC) {
            GuiUtils.addKeyAction(base_, JComponent.WHEN_IN_FOCUSED_WINDOW, "gameenginequit", quit, KeyEvent.VK_Q,
        ...
        if (!Utils.ISMAC) {
            GuiUtils.addKeyAction(base_, JComponent.WHEN_IN_FOCUSED_WINDOW, "gameengineclose", bMain ? quit : close,
    ...
    public boolean addDialog(InternalDialog dialog) {
        if (dialog instanceof EngineDialog)
            return true;
        else
    ...
    private class EngineWindowAdapter extends WindowAdapter implements ComponentListener {
        ...
        public void windowStateChanged(WindowEvent e) {
            // logger.debug("Window state changed: "+ e.getNewState() + " is Max: "+
            // isMaximized());
        ...
        private void savePosition() {
            ...
            // at bottom/right
            if (location_.x >= -50 && location_.x < (mode.getWidth() - (DESIRED_MIN_WIDTH / 2)))
