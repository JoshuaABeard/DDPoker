 ...
 * @author Doug Donohoe
 */
public abstract class ProfileList extends DDPanel implements AWTEventListener, FocusListener, AncestorListener {
     ...
     */
    public ProfileList(GameEngine engine, GameContext context, List<BaseProfile> profiles, String sStyle,
     ...
     * remember profile
     */
    private static void rememberProfile(BaseProfile profile, EnginePrefs prefs, String sPrefName) {
        if (profile != null)
     ...
     * init
     */
    private void init(GameEngine engine, GameContext context, List<BaseProfile> profiles, String sStyle,
            String sMsgName, String sPanelName, String sIconName, boolean bUseCopyButton) {
    ...
    private JComponent createProfileList(boolean bUseCopy) {
        ...
        // scroll
        scroll_ = new DDScrollPane(formatProfiles, STYLE, null, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
        ...
        if (bUseCopy)
            buttonCopy_ = new ProfileButton("copyprofile" + sMsgName_);

        buttons_.add(buttonNew_);
        if (bUseCopy)
            buttons_.add(buttonCopy_);
    ...
    /**
     * Set file mode - if true, associated files are modified when
     * edit/new/delete/copy occurs. Otherwise, just the UI and profile ArrayList is
     * updated
     */
    ...
    private void focus() {
        if (selectedPanel_ != null)
            selectedPanel_.name_.setIcon(hasFocus() ? selectedIcon_ : emptyIcon_);
    }
    ...
    /**
     * add a new profile, return profile created. Usable in non-ui mode. Runs phase
     * named NewProfile.<msgname from constructor> - if sPhaseSuffix is non-null,
     * then .<suffix> is appended to the phase name.
     */
    public BaseProfile newProfile(String sPhaseSuffix) {
        BaseProfile profile = createEmptyProfile();

        boolean bAdded = showProfileDialog(
                "NewProfile" + sMsgName_ + (sPhaseSuffix == null ? "" : ("." + sPhaseSuffix)), profile);
        if (bAdded) {
            addProfile(profile);
            return profile;
        }
    ...
    private void deleteProfile() {
        if (EngineUtils.displayConfirmationDialog(context_, PropertyConfig
                .getMessage("msg.confirm.deleteprofile" + sMsgName_, Utils.encodeHTML(selected_.getName())))) {
    ...
    private void checkButtons() {
        ...
        buttonDelete_
                .setEnabled(bSelected && (selected_.canDelete() || TESTING(EngineConstants.TESTING_PROFILE_EDITABLE)));
        buttonEdit_.setEnabled(bSelected && (selected_.canEdit() || TESTING(EngineConstants.TESTING_PROFILE_EDITABLE)));
        if (buttonCopy_ != null)
            buttonCopy_.setEnabled(
                    bSelected && (selected_.canCopy() || TESTING(EngineConstants.TESTING_PROFILE_EDITABLE)));
    }
    ...
    public void eventDispatched(AWTEvent event) {
        ...
        if (event instanceof KeyEvent) {
            ...
            if (!hasFocus() || k.getID() != KeyEvent.KEY_PRESSED
     ...
     * occurs. The source of <code>ChangeEvents</code> delivered to
     * <code>ChangeListeners</code> will be this <code>DDNumberSpinner</code>.
     *
     * @param listener
     ...
     * DDNumberSpinner with addChangeListener().
     *
     ...
     * <code>DDNumberSpinner</code>, to each <code>ChangeListener</code>. When a
     * <code>ChangeListener</code> has been added to the spinner, this method method
     * is called each time a <code>ChangeEvent</code> is received from the model.
     *
     * @see #addChangeListener
     * @see #removeChangeListener
