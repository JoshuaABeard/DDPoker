...
public abstract class GameEngine extends BaseApp {
    ...
    public void init() {
        ...
        // set theme
        if (!bHeadless_)
            GuiUtils.setTheme(new EngineMetalTheme());
        ...
        // int nScreenMode = prefs.getInt(EngineConstants.PREF_WINDOW_MODE,
        // EngineConstants.MODE_ASK);
        //
        //
        // switch (nScreenMode)
        // {
    ...
    protected void preConfigManagerInit() {
        if (!TESTING_SKIP_SPLASH && !bHeadless_) {
            URL file = new MatchingResources("classpath*:config/" + sAppName + "/images/" + getSplashBackgroundFile())
                    .getSingleRequiredResourceURL();
            URL icon = new MatchingResources("classpath*:config/" + sAppName + "/images/" + getSplashIconFile())
                    .getSingleRequiredResourceURL();
    ...
    /**
     * Use by subclass to prevent startup if any pre-reqs not met
     */
    ...
    public EnginePrefs getPrefsNode() {
        if (prefNode_ == null)
            prefNode_ = new EnginePrefs(DDOption.getOptionPrefs(sPrefNode_));
        return prefNode_;
    }
    ...
    public String getGUID() {
        if (guid_ == null)
            guid_ = new RandomGUID(ConfigUtils.getLocalHost(true), true).toString();
        return guid_;
    }

    /**
     * Get player UUID identifier. Replaces legacy license key system with UUID v4
    ...
    /**
     * Set player UUID identifier. Saves to disk for persistence across sessions.
     *
     * @param id
     *            Player UUID
     */
    ...
    /**
     * Clear all preferences. Simplified version without banned key tracking (no
     * longer needed in open source).
     */
    ...
    /**
     * See if display is min size - 800x600 (allow less to accommodate tablet PC
     * 768x1024)
     */
    private boolean checkSize() {
        ...
        if ((mode.getWidth() < MIN_WIDTH || mode.getHeight() < MIN_HEIGHT) && splashscreen_ != null) {
            String sMessage = PropertyConfig.getMessage("msg.wrongsize", mode.getWidth(), mode.getHeight());
    ...
    protected void initMainWindow() {
        ...
        if (loadGameboardConfig())
            gameconfig_ = new GameboardConfig(sMainModule_);

        // ready to go
        bReady_ = true;

        // show main window now if now waiting for splash screen (or skipping splash
        // screen)
    ...
    /**
     * Call when ready to show main window. Removes the splash screen if visible
     */
    public void showMainWindow() {
        ...
        // init main window
        defaultContext_.getFrame().init(null, true, getStartingSize(), bFull_,
                PropertyConfig.getRequiredStringProperty("msg.application.name"), true);
    ...
    /**
     * Very first start phase - calls start() by default, but can be overridden
     * (e.g., for load saved game)
     */
    ...
    /**
     * Called from window closing - calls Exit phase and returns false (meaning that
     * the caller won't close the app - instead, Exit does that)
     */
    @Override
    ...
    private class ContextTracker {
        ...
        boolean remove(GameContext context) {
            ApplicationError.assertTrue(contexts.remove(context), "Window not found in list",
                    context.getWindow().getName());
