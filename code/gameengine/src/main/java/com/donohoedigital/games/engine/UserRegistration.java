...
public class UserRegistration extends BasePhase implements PropertyChangeListener {
    ...
    public void init(GameEngine engine, GameContext context, GamePhase gamephase) {
        ...
        // name
        OptionText otext = new OptionText(NODE, RegistrationMessage.PARAM_NAME, STYLE, sTextBorderStyle_, msg_, 100,
                "^.+$", WIDTH, true);
        regbase.add(otext);
        ...
        // email
        otext = new OptionText(NODE, RegistrationMessage.PARAM_REG_EMAIL, STYLE, sTextBorderStyle_, msg_, 256,
                PropertyConfig.getRequiredStringProperty("regexp.email"), WIDTH, true);
        regbase.add(otext);
        otext.getTextField().addPropertyChangeListener("value", this);
        options_.add(otext);

        // address
        OptionTextArea atext = new OptionTextArea(NODE, RegistrationMessage.PARAM_ADDRESS, STYLE, sTextBorderStyle_,
                msg_, 1024, ".(?:.|\\s)*", 2, WIDTH);
        regbase.add(atext);
        ...
        // city
        otext = new OptionText(NODE, RegistrationMessage.PARAM_CITY, STYLE, sTextBorderStyle_, msg_, 50, "^.+$", WIDTH,
                true);
        regbase.add(otext);
        otext.getTextField().addPropertyChangeListener("value", this);
        options_.add(otext);
        ...
        // state
        otext = new OptionText(NODE, RegistrationMessage.PARAM_STATE, STYLE, sTextBorderStyle_, msg_, 50, null, WIDTH,
                true);
        regbase.add(otext);
        otext.getTextField().addPropertyChangeListener("value", this);
        options_.add(otext);
        ...
        // postal
        otext = new OptionText(NODE, RegistrationMessage.PARAM_POSTAL, STYLE, sTextBorderStyle_, msg_, 50, "^.+$",
                WIDTH, true);
        regbase.add(otext);
        otext.getTextField().addPropertyChangeListener("value", this);
        options_.add(otext);
        ...
        // country
        otext = new OptionText(NODE, RegistrationMessage.PARAM_COUNTRY, STYLE, sTextBorderStyle_, msg_, 120, "^.+$",
                WIDTH, true);
        regbase.add(otext);
        otext.getTextField().addPropertyChangeListener("value", this);
        options_.add(otext);
    ...
    public boolean processButton(GameButton button) {
        if (button.getName().equals(registerButton_.getName())) {
            ...
            UserRegistrationSend dialog = (UserRegistrationSend) context_.processPhaseNow("UserRegistrationSend",
                    params);
