...
public abstract class EngineGameAI extends GameAI {
    ...
    /**
     * Creates a new instance of EngineGameAI. Stores the current engine and game
     * (so this class needs to be created after a game has been created)
     */
    public EngineGameAI() {
        this(true);
    }
    ...
    public EngineGameAI(boolean bUseTerritoryCache) {
        ...
        if (bUseTerritoryCache)
            my_ = new Territory[Territory.getTerritoryArrayCached().length];
    }
    ...
    /**
     * Return array of territories belonging to this player - note that the array
     * only contains getNumMyTerritories() entries, so don't use the length of the
     * array in a loop!
     */
    ...
    /**
     * Return number of entries in myTerritories (not equal to length of array!
     */
