...
public abstract class EngineGamePiece extends GamePiece {
    ...
    /**
     * Is this piece visible on the gameboard. It might not be if in the middle of a
     * move, or set to not drawn.
     */
    ...
    public int getDrawingHiddenQuantity() {
        ...
        // BUG 176 - added isComputer() to prevent drawing hidden quantity when computer
        // player
     ...
     * getTerritory().getScaleImagesAsDouble(). Can be overridden to stop this
     * behavoir
     */
     ...
     */
    public void drawPiece(Gameboard board, Graphics2D g, Territory t, GeneralPath path, Rectangle territoryBounds,
     ...
     */
    public void drawPiece(Gameboard board, Graphics2D g, Territory t, GeneralPath path, Rectangle territoryBounds,
            int iPart, int x_adjust, int y_adjust, TerritoryPoint tp) {
        ...
        if (!bounds_.intersects(boundsLastDrawn_))
            return;
    ...
    /**
     * Default is to position bottom of image at territory point. Override and
     * return false to position at center
     */
    ...
    /**
     * Return bounds of image as it was last drawn. A new Rectangle is not created
     * each time, so consider this read-only.
     */
    ...
    /**
     * Get width/height to draw this at current board dimensions. Returns same
     * object each time, so consider it read-only
     */
    ...
    public void drawImageAt(Graphics2D g, ImageComponent ic, int nNum, int nHiddenNum, int nMovingNum, double x,
        ...
        if (nNum == 0 && nHiddenNum == 0 && nMovingNum == 0)
            return;

        // see if we draw the quantity circle
        if (!isQuantityDrawn(nNum, nHiddenNum))
            return;
    ...
    /**
     * Used by subclass to override quantity text shadow color in specific
     * circumstances. Return null to indicate default behavoir
     */
    ...
    /**
     * Return string drawn for quantity, can be overriden by subclass for special
     * display
     */
    ...
    /**
     * Used to return image component used for small icons - default is to return
     * getImageComponent()
     */
