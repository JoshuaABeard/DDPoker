...
public class ButtonBox extends DDPanel implements AncestorListener {
    ...
    /**
     * Defaults nGap to 5 (gap between buttons) and borderSpace to 8 if vertical
     * arrangement, 5 otherwise.
     */
    public ButtonBox(GameContext context, GamePhase gamephase, Phase phase, String sName, boolean bVertical,
            boolean bSmallButton) throws ApplicationError {
        this(context, gamephase, phase, sName, 8, (bVertical ? 8 : 4), bVertical, bSmallButton);
    }

    /**
     * Creates a new instance of ButtonBox
     */
    public ButtonBox(GameContext context, GamePhase gamephase, Phase phase, String sName, int nGap, int nBorderSpace,
            boolean bVertical, boolean bSmallButton) throws ApplicationError {
        super(sName);
        ...
        if (sEnter != null && sEnter.equals("NONE"))
            sEnter = null;

        buttonPanel_ = new DDPanel();
        setBorder(BorderFactory.createEmptyBorder(bVertical ? nBorderSpace : 1, nBorderSpace, nBorderSpace,
                nBorderSpace));
        ...
        if (buttons != null) {
            ...
            for (int i = 0; i < nSize; i++) {
                ...
                button = new GlassButton(listener.getGameButton().getName(), bSmallButton ? "Glass" : "GlassBig");
    ...
    public void ancestorAdded(AncestorEvent event) {
        if (defaultButton_ != null) {
            ...
            // logger.debug("BUTTONBOX ROOT " + root.getName() +" Setting default button to
            // " + ((defaultButton_ == null) ? "null" : defaultButton_.getName()));
        }
    }
    ...
    public void ancestorRemoved(AncestorEvent event) {
        if (defaultButton_ != null) {
            ...
            // logger.debug("BUTTONBOX ROOT " + root.getName() +" removing default button "
            // + ((defaultButton_ == null) ? "null" : defaultButton_.getName()));
        }
    }
