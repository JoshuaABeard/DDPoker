...
public class FileChooserDialog extends DialogPhase implements PropertyChangeListener, KeyListener {
            ...
            if (selected_.exists()) {
                try {
                    ...
                    EngineUtils.displayInformationDialog(context_,
                            PropertyConfig.getMessage("msg.error.replace", selected_.getName()));
                    return false;
                }
            }
            fResult = selected_;
    ...
    private void getSelected() {
        ...
        if (sName.length() == 0) {
            selected_ = null;
            if (choose_.getSelectedFile() != null)
                choose_.setSelectedFile(null);
        } else {
        ...
        // if (selected_ != null)
        // {
        // logger.debug("Selected: "+ selected_.getAbsolutePath() + " exists: " +
        // selected_.exists());
        // }
        checkButtons();
    }
    ...
    private JLabel getLabel(Container container, String sText) {
        Component[] children = container.getComponents();
        for (Component child : children) {
            if (child instanceof JLabel && ((JLabel) child).getText().equalsIgnoreCase(sText)) {
                return (JLabel) child;
            }
            if (child instanceof Container) {
