...
public abstract class BasePhase implements Phase {
    ...
    /**
     * Reinit phase for use, called before reset(), used to store new gamephase data
     */
    ...
    /**
     * Called by engine - passes the last non-transient phase invoked before this
     * phase (i.e., the phase that launched this phase). BasePhase ignores this
     * (doesn't store what is passed in) to avoid keeping pointers to old phases.
     * Subclasses can override this if they need to know from whence they came.
     */
    ...
    /**
     * Called when a phase is removed as the main component ( when using
     * engine.setMainUIComponent()) or when a DialogPhase's dialog is closed. Other
     * phases that don't use a UI (e.g., ChainPhase or LoopPhases) are finished when
     * their start() method is done, so any cleanup can be done then.
     */
    public void finish() {
    ...
    /**
     * By default, no phase should be added as an entry, unless it specifically
     * allows it by overriding this. It should call the _addGameStateEntry to do the
     * default work
     */
    ...
    /**
     * Take entry and return the phase and params in a TypedHashMap usable by
     * ChainPhase. Returns null if no phase stored.
     */
    public static TypedHashMap getPhaseFromGameStateEntry(GameStateEntry entry) {
        ...
        if (sName == null)
            return null; // empty entry (as added above)
