 ...
 * @author Doug Donohoe
 */
public class ScrollGameboard extends JViewport
        implements
            MouseMotionListener,
            MouseListener,
            MouseWheelListener,
     ...
     */
    public ScrollGameboard(GameEngine engine, GameContext context, GameboardConfig gameconfig, boolean bGameMode,
            Territory tUpperLeft, int nSmallWidth, int nSmallHeight) {
        this(engine, context, gameconfig, bGameMode, tUpperLeft, nSmallWidth, nSmallHeight, nSmallWidth, nSmallHeight,
                true);
    }
     ...
     */
    public ScrollGameboard(GameEngine engine, GameContext context, GameboardConfig gameconfig, boolean bGameMode,
            Territory tUpperLeft, int nSmallWidth, int nSmallHeight, int nStartingWidth, int nStartingHeight,
            boolean bScaleBig) {
        super();
        engine_ = engine;
        getClickToScroll();
    ...
    private void init() {
        // clear board's original listeners and add ours. We propogate down to the
        // board.
    ...
    private void setupSize(int nWidth, int nHeight) {
        ...
        // create resize dimensions based on starting size - also sets board to this
        // initialize size
    ...
    public void changeGameboardSize(int nType, boolean bAllTheWay, boolean bZoomToMouse) {
        ...
        // change size
        if (nType == INCREASE) {
            if (!board_.increaseSize(bAllTheWay))
                return;
        } else {
            if (!board_.decreaseSize(bAllTheWay))
                return;
        }
    ...
    public void componentResized(ComponentEvent e) {
        ...
        if (nWidth < nSmallWidth_)
            nWidth = nSmallWidth_;
        if (nHeight < nSmallHeight_)
            nHeight = nSmallHeight_;
    ...
    private void checkMouseScroll(MouseEvent e) {
        ...
        if (x < minx || x > maxx || y < miny || y > maxy) {
            ...
            if (x < minx) {
                adjustx_ = -1;
                if (y < (miny + AUTO_SCROLL_CORNER_BUFFER))
                    adjusty_ = -1;
                if (y > (maxy - AUTO_SCROLL_CORNER_BUFFER))
                    adjusty_ = +1;
            }
            ...
            if (y < miny) {
                adjusty_ = -1;
                if (x < (minx + AUTO_SCROLL_CORNER_BUFFER))
                    adjustx_ = -1;
                if (x > (maxx - AUTO_SCROLL_CORNER_BUFFER))
                    adjustx_ = +1;
            }
    ...
    private void setScrollCursor() {
        if (adjustx_ < 0 && adjusty_ == 0)
            setOverrideCursor(Cursors.SCROLLLEFT);
        if (adjustx_ < 0 && adjusty_ < 0)
            setOverrideCursor(Cursors.SCROLLUPLEFT);
        if (adjustx_ < 0 && adjusty_ > 0)
            setOverrideCursor(Cursors.SCROLLDOWNLEFT);

        if (adjustx_ == 0 && adjusty_ < 0)
            setOverrideCursor(Cursors.SCROLLUP);
        if (adjustx_ == 0 && adjusty_ > 0)
            setOverrideCursor(Cursors.SCROLLDOWN);

        if (adjustx_ > 0 && adjusty_ == 0)
            setOverrideCursor(Cursors.SCROLLRIGHT);
        if (adjustx_ > 0 && adjusty_ < 0)
            setOverrideCursor(Cursors.SCROLLUPRIGHT);
        if (adjustx_ > 0 && adjusty_ > 0)
            setOverrideCursor(Cursors.SCROLLDOWNRIGHT);
    ...
    private void scrollBoardAndWait() {
        if (bSCROLL) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    if (bSCROLL && (bMOUSEDOWN || !CLICKTOSCROLL))
                        scrollBoard();
                }
            });
        }
    ...
    /**
     * Scrolls board by current adjustx_ and adjusty_ values (* SCROLL_AMOUNT)
     */
    ...
    private void scrollBoard(int myadjustx, int myadjusty) {
        ...
        // skip if same
        if (viewpointx == origx && viewpointy == origy) {
            return;
        }
    ...
    /**
     * If mouse moved within AUTO_SCROLL_BUFFER pixels of edge, start scrolling
     */
    ...
    public void eventDispatched(AWTEvent event) {
        ...
        if (event instanceof MouseEvent) {
            ...
            if (e.getSource() instanceof Component) {
                board_.lastPointFromAWT_ = SwingUtilities.convertPoint((Component) e.getSource(), e.getX(), e.getY(),
                        board_);
            }
        }
    }
    ...
    private void moveMap(MouseEvent e, boolean bScroll) {
        ...
        if (bScroll)
            scrollBoard(lastMouse_.x - e.getX(), lastMouse_.y - e.getY());
        lastMouse_.x = e.getX();
        lastMouse_.y = e.getY();
    }
    ...
    /**
     * Set override cursor on board, unless modal mode, then set on the topmost
     * panel
     */
    private void setOverrideCursor(Cursor c) {
        if (!isModalMode()) {
            ...
            if (c == null)
                c = Cursors.PROHIBITED;
            topPanel_.setCursor(c);
        }
    }

    /**
     * Class to pass mouse events from modal blocker to board so that it can still
     * be scrolled
     */
    public class ModalHandler implements MouseListener, MouseMotionListener, MouseWheelListener {
        ...
        private MouseEvent convertToBoard(MouseEvent e) {
            ...
            if (p.getX() >= 0 && p.getX() < scroll.getWidth() && p.getY() >= 0 && p.getY() < scroll.getHeight()) {
        ...
        public void mouseClicked(MouseEvent e) {
            e = convertToBoard(e);
            if (e != null)
                ScrollGameboard.this.mouseClicked(e);
        }

        public void mouseDragged(MouseEvent e) {
            e = convertToBoard(e);
            if (e != null)
                ScrollGameboard.this.mouseDragged(e);
        }
        ...
        public void mouseMoved(MouseEvent e) {
            e = convertToBoard(e);
            if (e != null)
                ScrollGameboard.this.mouseMoved(e);
        }

        public void mousePressed(MouseEvent e) {
            e = convertToBoard(e);
            if (e != null)
                ScrollGameboard.this.mousePressed(e);
        }

        public void mouseReleased(MouseEvent e) {
            e = convertToBoard(e);
            if (e != null)
                ScrollGameboard.this.mouseReleased(e);
        }

        public void mouseWheelMoved(MouseWheelEvent e) {
            e = (MouseWheelEvent) convertToBoard(e);
            if (e != null)
                ScrollGameboard.this.mouseWheelMoved(e);
        }
