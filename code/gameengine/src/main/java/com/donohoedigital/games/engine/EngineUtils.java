...
public class EngineUtils {
    ...
    private static javax.swing.border.Border standardMenuLowerTextBorder_ = BorderFactory.createEmptyBorder(2, 10, 2,
            10);
    ...
    /**
     * Set component responsible for displaying & scrolling gameboard
     */
    ...
    /**
     * Set cursor using invokeLater - used to do things like scrolling where the
     * painting happens before cursor is set (looks more natural)
     */
    ...
    /**
     * Show message in a confirmation dialog. Return true if 'yes' pressed, false
     * otherwise.
     */
    public static boolean displayConfirmationDialog(GameContext context, String sMsg) {
     ...
     */
    public static boolean displayConfirmationDialog(GameContext context, String sMsg, String sNoShowKey) {
     ...
     */
    public static boolean displayConfirmationDialog(GameContext context, String sMsg, String sTitleKey,
            String sNoShowKey) {
     ...
     */
    public static boolean displayConfirmationDialog(GameContext context, String sMsg, String sTitleKey,
            String sNoShowKey, String sNoShowCheckBoxName) {
        GameButton buttonpressed = displayConfirmationDialogCustom(context, "DisplayConfirmation", sMsg, sTitleKey,
                sNoShowKey, sNoShowCheckBoxName);
     ...
     */
    public static boolean displayCancelableConfirmationDialog(GameContext context, String sMsg, String sTitleKey,
            String sNoShowKey, String sNoShowCheckBoxName, int nTimeout) {
        GameButton buttonpressed = displayConfirmationDialogCustom(context,
                nTimeout == 0 ? "DisplayConfirmationCancelable" : "DisplayTimedMessage", sMsg, sTitleKey, sNoShowKey,
                sNoShowCheckBoxName, nTimeout);
     ...
     */
    public static GameButton displayConfirmationDialogCustom(GameContext context, String sPhase, String sMsg,
            String sTitleKey, String sNoShowKey, String sNoShowCheckBoxName) {
     ...
     * buttons. Should check for null
     */
    private static GameButton displayConfirmationDialogCustom(GameContext context, String sPhase, String sMsg,
            String sTitleKey, String sNoShowKey, String sNoShowCheckBoxName, int nTimeoutSeconds) // only used with
                                                                                                    // cancelable
                                                                                                    // timeout
    {
        ...
        if (sTitleKey != null)
            params.setString(DisplayMessage.PARAM_WINDOW_TITLE_KEY, sTitleKey);
        if (sNoShowKey != null) {
     ...
     */
    public static void displayInformationDialog(GameContext context, String sMsg) {
     ...
     * Show message in a information dialog
     */
    public static void displayInformationDialog(GameContext context, String sMsg, boolean bModal) {
     ...
     * Same as above, except display "don't show" option if sNoShowKey is non-null
     */
    public static void displayInformationDialog(GameContext context, String sMsg, String sNoShowKey) {
        displayInformationDialog(context, sMsg, null, sNoShowKey);
    }

    /**
     * Show message in a information dialog for current player. If sNoShowkey is not
     * null, then a "don't show this dialog" option is shown
     */
    public static void displayInformationDialog(GameContext context, String sMsg, String sTitleKey, String sNoShowKey) {
     ...
     */
    public static void displayInformationDialog(GameContext context, String sMsg, String sTitleKey, String sNoShowKey,
            String sNoShowCheckBoxName, boolean bModal) {
        TypedHashMap params = new TypedHashMap();
        params.setString(DisplayMessage.PARAM_MESSAGE, sMsg);
        if (sTitleKey != null)
            params.setString(DisplayMessage.PARAM_WINDOW_TITLE_KEY, sTitleKey);
    ...
    /**
     * Return true if the given player has a piece of the given type in this
     * territory that is visible (i.e., not currently moving)
     */
    ...
    /**
     * Return true if the given territory has a piece of the given type (owned by
     * the territory owner), with either visible or hidden quantities (meaning it
     * may have just been purchased)
     */
    ...
    /**
     * Is there a territory owned by the given playerOwner adjacent to this
     * territory which contains a piece of the given type (also owner by the
     * playerOwner) that wasn't just placed
     */
    public static boolean isPreExistingAdjacent(Territory t, GamePlayer playerOwner, int nType) {
        ...
        for (int i = 0; i < adjacentTerritories.length; i++) {
            if (adjacentTerritories[i].getGamePlayer() == playerOwner
                    ...
                    && adjacentTerritories[i].getGamePiece(nType, playerOwner).getQuantity() > 0)
                return true;
        }
        return false;
    }
    ...
    /**
     * Return true if the given territory has a piece of the given type (owned by
     * the territory owner) where the visible quantity is greater than 0 (meaning it
     * existed before purchasing took place)
     */
    ...
    /**
     * Return all pieces of a given type in container. Should synchronize on
     * container.getMap() around call to this and use of iterator to avoid
     * concurrent modification exceptions and to make sure all pieces you get are
     * still there while you iterate
     */
    ...
    public static void setLeds(DDLabel online, DDLabel connect) {
        ...
        if (ledonline_ != null)
            ledonline_.setIcon(ledgreen_);
        if (ledconnect_ != null)
            ledconnect_.setIcon(ledyellowoff_);
    }
    ...
    /**
     * background audio from gamephase data - loops for audio-start and audio-loop
     * params.
     */
    ...
    /**
     * music loop from gamephase data - loops for audio-start and audio-loop params.
     */
    ...
    public synchronized static void addCancelable(CancelablePhase phase) {
        if (cancelables_ == null)
            cancelables_ = new ArrayList();

        if (!cancelables_.contains(phase))
            cancelables_.add(phase);
    }
    ...
    public synchronized static void cancelCancelables() {
        if (cancelables_ == null || cancelables_.size() == 0)
            return;
