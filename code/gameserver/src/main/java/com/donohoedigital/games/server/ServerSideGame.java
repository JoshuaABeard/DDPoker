...
public class ServerSideGame extends ServerDataFile implements GameInfo {
    ...
    // FIX: this file is from WarAOI and probably should be deleted. Added this
    // postalservice so it will compile
    @Autowired
    ...
    // PropertyConfig.getRequiredStringProperty("settings.email.from");
    // public static final String EMAIL_REPLYTO =
    // PropertyConfig.getRequiredStringProperty("settings.email.replyto");
    ...
    /**
     * Create a new online game object by loading an existing one given the info in
     * the message
     */
    ...
    /**
     * Create a new online game object by loading an existing one given a game file
     */
        ...
        if (elim != null) {
            ...
            if (elim && bEvict) {
                ...
                for (int i = 0; i < actionList_.size(); i++) {
                    ...
                    if (item.isPlayerActionRequired(id) && !item.hasPlayerActed(id)) {
    ...
    private void checkDone(ActionItem item, boolean bCheckEmpty) {
        ...
        if (item.isDone()) {
            ...
            if (next != null) {
                ...
                if (item != null)
                    checkDone(item, false);
            }
        }
    }

    /**
     * See if data in action update - if so, it is a game update, so send it on to
     * the other players (skip those in player id list)
     */
    private void checkActionData(ActionItem action, EngineMessage message) {
        ...
        // but in case of multiple armies, the other ones controlled by that player
        // already
        ...
        for (int i = 0; i < nNumPlayers_; i++) {
            ...
            if (getPasswordAt(i).equals(sPass))
                continue;
    ...
    /**
     * Return a new player queue for the given index Returns a new object every
     * time, so cache return for perf
     */
    ...
    /**
     * Get last seq-id for given key. If no seq id previously set, -1 is returned
     */
    ...
    private void sendEmail(String sEmailID, String sLocale, int i, String sMsg) {
        ...
        {
            EngineMessage msg = new EngineMessage(gameID_, EngineMessage.PLAYER_SERVER,
                    EngineMessage.CAT_EMAIL_JOIN_GAME);
        ...
        // error info
        EngineMailErrorInfo info = new EngineMailErrorInfo(sAppName_, gameID_, i, sEmailID);

        // get results and send email
        String sFrom = (String) emails_.get(0); // from the host
        String sTo = (String) emails_.get(i);

        // TODO: send from server w/ bounce handling? DDMail.sendMail(sTo, EMAIL_FROM,
        // EMAIL_REPLYTO, email.getSubject(),
        postalService.sendMail(sTo, sFrom, null, email.getSubject(), email.getPlain(), email.getHtml(), attach, info);
    }
    ...
    /**
     * Get next save file number given the list of existing files. Assumes the files
     * are in sorted order, with last one being highest number
     */
