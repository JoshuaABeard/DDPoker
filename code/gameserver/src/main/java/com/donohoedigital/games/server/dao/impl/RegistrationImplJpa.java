...
public class RegistrationImplJpa extends JpaBaseDao<Registration, Long> implements RegistrationDao {
    ...
    private int countWhere(String keyStart, String sWhere, Object param) {
        ...
        if (keyStart == null)
            ...
            keyStart += "%";
        Query query = entityManager.createQuery("select count(distinct t.licenseKey) from Registration t "
                + "where t.licenseKey not in " + "(select b.key from BannedKey b) " + "AND t.duplicate = FALSE "
                + "AND t.licenseKey like :key " + "AND " + sWhere);
    ...
    public List<RegDayOfYearCount> countByDayOfYear(String keyStart) {
        ...
        for (Object[] objs : results) {
            list.add(new RegDayOfYearCount(((Number) objs[0]).intValue(), ((Number) objs[1]).intValue(),
    ...
    public List<RegHourCount> countByHour(String keyStart) {
        ...
        for (Object[] objs : results) {
            list.add(new RegHourCount(((Number) objs[0]).intValue(), ((Number) objs[1]).intValue()));
        }
        return list;
    }
    ...
    public int getMatchingCount(String nameSearch, String emailSearch, String keySearch, String addressSearch) {
        Query countQuery = entityManager
                .createQuery("select count(t) from Registration t " + "where t.licenseKey like :key "
                        + "  AND t.name like :name " + "  AND t.email like :email " + "  AND t.address like :address ");
    ...
    @SuppressWarnings({"unchecked"})
    public PagedList<Registration> getMatching(Integer count, int offset, int pagesize, String nameSearch,
        ...
        Query query = entityManager.createQuery(
                "select t from Registration t " + "where t.licenseKey like :key " + "  AND t.name like :name "
                        + "  AND t.email like :email " + "  AND t.address like :address " + " order by t.licenseKey");
    ...
    public List<Registration> getAllForKey(String sKey) {
        Query query = entityManager.createQuery("select t from Registration t " + "where t.licenseKey = :key");
    ...
    public List<Registration> getAllBanned() {
        Query query = entityManager.createQuery(
                "select t from Registration t " + "where t.licenseKey in " + "(select b.key from BannedKey b)");
        return (List<Registration>) query.getResultList();
    }
    ...
    public List<String> getAllSuspectKeys(int nMin) {
        ...
        Query query = entityManager.createQuery("select t.licenseKey from Registration t "
                + "where t.licenseKey not in " + "(select b.key from BannedKey b) " + "and t.duplicate = false "
                + "group by t.licenseKey " + "having count(t.licenseKey) >= :min" + // IntelliJ wrongly flags an error
                                                                                    // here as of 7.0.3
    ...
    public boolean isDuplicate(Registration reg) {
        ...
        if (reg.isRegistration()) {
            query = entityManager.createQuery("select count(t.id) from Registration t "
                    + "where t.licenseKey = :key and t.duplicate = false " + "and t.type = :type "
                    + "and (    t.email = :email " + "or t.ip = :ip " + "or t.hostNameModified = :host " + ')');
            ...
            query = entityManager.createQuery(
                    "select count(t.id) from Registration t " + "where t.licenseKey = :key and t.duplicate = false "
                            + "and (    t.ip = :ip " + "      or t.hostNameModified = :host " + ')');
            query.setParameter("key", reg.getLicenseKey());
            query.setParameter("ip", reg.getIp());
            query.setParameter("host", reg.getHostNameModified());
        }
    ...
    public int markDuplicates(Registration reg) {
        if (!reg.isRegistration() || reg.isDuplicate())
            return 0;

        Query query = entityManager.createQuery("update Registration t set t.duplicate = true "
                + "where t.licenseKey = :key and t.duplicate = false " + "and t.id <> :id "
                + "and (    t.email = :email " + "or t.ip = :ip " + "or t.hostNameModified = :host " + ')');
        Long id = reg.getId();
        if (id == null)
            id = -1L; // need to specify id since (id <> null) is always false
