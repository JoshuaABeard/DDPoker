...
public class RegInfo implements Comparable<RegInfo> {
    ...
    public boolean isDownload() {
        return (sKey_.compareTo("2100-4299-7560-5704") < 0 || sKey_.compareTo("2200-8599-8704-9538") < 0);
    }
    ...
    public void addRegistration(Registration reg, boolean bFilterDD) {
        ...
        if (bFilterDD) {
            if (contains(email, "donohoe.info"))
                return;
            if (contains(email, "gregking"))
                return;
            if (contains(sKey_, "2100-0000-8239-2505"))
                return; // greg
        }
    ...
    public String toHTML(List<Registration> msgs, int nMaxLines) {
        ...
        StringBuilder sb = new StringBuilder();
        Registration msg;
        if (nMaxLines <= 0 || nMaxLines > msgs.size())
            nMaxLines = msgs.size();
        int j = 0;
        boolean bRecent;
        ...
        for (; j < nMaxLines; j++) {
            ...
            if (msg.isActivation() || msg.isPatch()) {
                ...
                sb.append("<BR>");
                sb.append(msg.getName()).append(" at ").append(msg.getAddress()).append("<BR>").append(msg.getCity())
                        .append(", ").append(msg.getState()).append("  ").append(msg.getPostal()).append(" ")
                        .append(msg.getCountry());
            }
            sb.append(FNTD);
