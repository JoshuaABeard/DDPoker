 ...
 * suitable. The \ character is used as an escape character, allowing delimiters
 * to be escaped and \xxx decimal value to be used. Quoted strings (delimited by
 * double quotes) are treated as one token. Return (\n) can be escaped using the
 * ESCAPED_RETURN value.
 */
...
public class EscapeStringTokenizer implements Enumeration {
     ...
     * replace each '\n' character with escape-ESCAPED_RETURN. ESCAPED_RETURN equals
     * capital-N.
     */
     ...
     * @param delim
     *            A string containing all delimiters
     * @param returnTokens
     *            If true, return delimiters as tokens. This differs from
     *            StringTokenizer in that adjacent delimiters are returned in the
     *            same token.
     */
    ...
    /**
     * Are there any more delimiters in the string? This should only be called if
     * hasMoreTokens is false, to determine if the string contains trailing
     * delimiters.
     */
    ...
    /**
     * Specifies a string to be added to the MyStringTokenizer object. The next call
     * to nextToken() will return this string.
     */
