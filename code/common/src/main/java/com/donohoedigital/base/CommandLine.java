...
/**
 * This class is used to parse command line arguments Features: Auto usage
 * message, auto -? and -help options
 */
...
public class CommandLine {
    ...
    /**
     * Return Option for given name; create new if non-existant and bCreate passed
     */
    ...
    /**
     * Set usage for this command line app. Typically this is a description of the
     * application w/out explaining each parameter (those descriptions are set
     * individually)
     */
    ...
    /**
     * Specify mininum number of non-optioned command line arguments are required.
     * Typically used for commands which take one-to-many items like file names (for
     * example, foo <file1> ... <filen>)
     */
    ...
    /**
     * Specify maxinum number of non-optioned command line arguments are required.
     * Typically used for commands which take a fixed number of items like diff (for
    ...
    /**
     * Sets a description for parameters passed at end of command line. Used in
     * usage and error message. The sName is what appears in the usage parameter
     * list (e.g., "file"). The sUsage string is what appears after the usage
     * summary (e.g., "[file 1] ... [file N]"). The sDesc string is the description
     * of the parameter.
     */
    public static void setParamDescription(String sName, String sUsage, String sDesc) {
        sParamName_ = sName;
        sParamUsage_ = sUsage;
        sParamDesc_ = sDesc;
    }
    ...
    /**
     * Parse arguments from main(). Results can be obtained with getOptions() and
     * getRemainingArgs() methods
     */
    ...
    public static void parseArgs(String[] args) {
        ...
        if (sMacFileArg_ != null)
            vArgs.add(sMacFileArg_);

        // check size of remaining args
        int nNum = vArgs.size();
        if (nMinRequiredParams_ != null && nNum < nMinRequiredParams_) {
            exitWithError("Requires at least " + nMinRequiredParams_ + " (" + sParamName_ + ')');
        } else if (nMaxRequiredParams_ != null && nNum > nMaxRequiredParams_) {
            exitWithError("Requires no more than " + nMaxRequiredParams_ + " (" + sParamName_ + ')');
        }
    ...
    /**
     * Return parsed options in TypedHashMap, mapping option name to Object
     * containing value (String, Integer or Boolean)
     */
    ...
    private static void printUsage() {
        ...
        if (nMinRequiredParams_ != null || nMaxRequiredParams_ != null) {
            ...
            // min and max
                ...
                System.out.print(" (between " + nMinRequiredParams_ + " and " + nMaxRequiredParams_ + ')');
            }
        ...
        while (iter.hasNext()) {
            ...
            System.out.println((o.bRequired ? "*" : " ") + "  -" + o.sName
                    + (o.sExample == null ? "" : " [" + o.sExample + ']') + " - " + (o.sDesc == null ? "" : o.sDesc));
        }
        if (bRequired)
