...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Jan 10, 2009 Time: 3:15:11 PM
...
public abstract class ManagedQueue<T> {
     ...
     *
     * @param capacity
     *            max amount queue can hold
     * @param waitLimitWarningMillis
     *            amount to wait attempting to add to queue before printing warning
     * @param waitLimitErrorMillis
     *            after warning, try and add to queue again, throwing exception
     *            after this amount of time
     * @param quitItem
     *            an instance of T that is used to indicate last item on the queue
     */
     ...
     *            an instance of T that is used to indicate last item on the queue
     * @param threadName
     *            name for thread (if null, defaults to class' simple name)
     */
    public ManagedQueue(int capacity, long waitLimitWarningMillis, long waitLimitErrorMillis, T quitItem,
            String threadName) {
    ...
    /**
     * Waits for thread to processes all items currently on the queue then returns.
     * Synchronized so only one call to stop at a time.
     */
    ...
    protected void add(T item) {
        try {
            if (!queue.offer(item, waitLimitWarningMillis, TimeUnit.MILLISECONDS)) {
                logger.warn("Exceeded " + waitLimitWarningMillis + " milliseconds waiting to add item: " + item);
                ...
                if (waitLimitErrorMillis <= 0 || !queue.offer(item, waitLimitErrorMillis, TimeUnit.MILLISECONDS)) {
                    throw new ApplicationError(
                            "Exceeded " + waitLimitErrorMillis + " milliseconds waiting to add item: " + item);
                }
            }
