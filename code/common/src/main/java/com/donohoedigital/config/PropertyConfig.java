...
public class PropertyConfig extends Properties {
    ...
    public PropertyConfig(String appName, String[] modules, ApplicationType type, String sLocale,
    ...
    /**
     * Get a string property that should be there, throw runtime error if not
     */
    ...
    /**
     * Get a string property Checks system properties first (for Docker/environment
     * variable overrides), then falls back to properties file values.
     */
    private static String getStringProperty(String sKey, boolean bReportMissing) {
        ...
        // Check system properties first (allows -D overrides and environment variable
        // passthrough)
        ...
        if (sValue != null)
            sValue = sValue.trim(); // need to remove trailing spaces

        if (sValue == null && testing)
            return "TESTING-MISSING-" + sKey;
    ...
    /**
     * Get a message and insert the params into it (params replaced where strings of
     * the form {#} are found).
     */
    ...
    /**
     * return number as a place "e.g., 1=1st, 2=2nd". This version takes the
     * pre-formatted display value of value
     *
    ...
    /**
     * Get amount for display - use K for large numbers if bDoK is true, use bDoM
     * for 1,000,000+ if true
     */
