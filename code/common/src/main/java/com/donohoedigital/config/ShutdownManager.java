...
/**
 * Created by IntelliJ IDEA. User: donohoe Date: Sep 22, 2008 Time: 4:30:42 PM
...
public class ShutdownManager implements Thread.UncaughtExceptionHandler {
    ...
    /**
     * Set verbose message flag (default is true). If verbose, prints messages on
     * abnormal shutdown or any shutdown w/listeners defined during: start of
     * shutdown, calling of listeners and finish. If set to false, no info messages
     * are displayed.
     *
     * @param b
     */
    ...
    /**
     * Add a shutdown listener. Shutdown listeners are called in the reverse order
     * they were instantiated
     *
     * @param shutdownListener
     *            listener which will be called on shutdown
     */
    ...
    /**
     * Construct and register it to handle shutdowns, signals, uncaught exceptions,
     * and log what's going on
     */
    ...
    private synchronized void shuttingDown() {
        ...
        for (ShutdownListener listener : listeners) {
            try {
                if (log)
                    logger.info("Calling shutdown listener: " + listener);
