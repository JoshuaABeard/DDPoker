...
public class AudioPlayer implements Runnable, LineListener {
    ...
    public String toString() {
        return "[start: " + getStartName() + " loop: " + getLoopName() + " isPaused: " + isPaused() + " isPlaying: "
                + isPlaying() + "]";
    }
        ...
        private DataLine getDataLine() {
            ...
            try {
                ...
                if (nSize > 250000) {
                    ...
                    info = new DataLine.Info(Clip.class, stream.getFormat(), (int) nSize);
                }
    ...
    private void playSound() {
        ...
        if (dataline_ instanceof Clip) {
            ...
            try {
                ...
                logger.error(
                        stream_.def.getName() + ": Caught exception playing sound: " + Utils.formatExceptionText(lue));
                return;
            } catch (IOException ioe) {
                logger.error(
                        stream_.def.getName() + ": Caught exception playing sound: " + Utils.formatExceptionText(ioe));
                return;
            }
        } else {
            ...
            try {
                ...
                if (loopstream_ != null)
                    loopstream_.stream.mark(Integer.MAX_VALUE);
                ...
                while (bLooping && !bStopPlayback_) {
                    ...
                    while (!bStopPlayback_ && (bPaused_ || (cnt = localstream.stream.read(tempBuffer, 0,
                        ...
                        if (!bStopPlayback_ && (bPaused_ || line.available() == 0))
            ...
            try {
                ...
                logger.warn(stream_.def.getName() + ": Exception caught draining/stopping: "
    ...
    synchronized void setPan(float value) {
        ...
        if (dataline_ == null)
            return;
        if (!dataline_.isControlSupported(FloatControl.Type.PAN))
            return;

        try {

            FloatControl panControl = (FloatControl) dataline_.getControl(FloatControl.Type.PAN);
