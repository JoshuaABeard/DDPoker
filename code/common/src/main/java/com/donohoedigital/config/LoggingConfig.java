...
public class LoggingConfig {
    ...
    public void init() {
        if (loggingConfig != null) {
            if (!loggingConfig.appName.equals(appName) || !loggingConfig.type.equals(type)) {
                ApplicationError.warnNotNull(loggingConfig,
                        "LoggingConfig already initialized with different app/type (" + loggingConfig.appName + "/"
                                + loggingConfig.type + "), but now attempting to init with " + appName + "/" + type);
            }
            return;
        }
        loggingConfig = this;
        ...
        if (url == null) {
            ...
            String sFileName;
            if (type == CLIENT || type == HEADLESS_CLIENT || type == COMMAND_LINE)
                sFileName = appName + ".log";
            else if (type == WEBAPP)
                sFileName = appName + "-web.log";
            else
                sFileName = appName + "-server.log";
            ...
            if (!useDefault) {
                ...
                if (appoverride != null)
                    logger.info("Log4j app override file used: {}", appoverride);
                if (useroverride != null)
                    logger.info("Log4j user override file used: {}", useroverride);
                if (useroverride2 != null)
                    logger.info("Log4j user override file 2 used: {}", useroverride2);
            }
        }
    }
    ...
    private LoggerContext configure(URL... urls) {
        ...
        // helpful to see what loggers exist, that will become disabled when we reload
        // logging
        if (debugLoggers) {
            LoggerContext context = (LoggerContext) LogManager.getContext(false);
            Collection<org.apache.logging.log4j.core.Logger> loggers = context.getLoggers();
            loggers.forEach(logger -> System.err.println(logger.getName() + ": " + logger.getLevel()));
        }
        ...
        // we have a new context, recreate the few loggers that are creating before this
        // is run
