...
public class ConfigUtils {
    ...
    // allow resetting logger (needed since this is created before LoggingConfig
    // runs)
    ...
    /**
     * Get a class given a class name. Throw an exception if bThrowExceptionOnError
     * true and class not found. Otherwise return null and log a warning message.
     */
    ...
    public static void verifyNewDirectory(File dir) {
        if (!dir.exists()) {
            ...
            if (!dir.mkdirs()) {
                ...
                throw new ApplicationError(ErrorCodes.ERROR_CREATE, "Unable to create directory", dir.getAbsolutePath(),
        ...
        if (!dir.isDirectory()) {
            ...
            throw new ApplicationError(ErrorCodes.ERROR_CREATE, "Path should be a directory", dir.getAbsolutePath(),
    ...
    public static void verifyNewFile(File file) {
        if (file.exists()) {
            ...
            throw new ApplicationError(ErrorCodes.ERROR_CREATE, "File should not exist", file.getAbsolutePath(),
        ...
        try {
            if (!file.createNewFile()) {
                ...
                throw new ApplicationError(ErrorCodes.ERROR_CREATE, "Unable to create file", file.getAbsolutePath(),
                        "Check permissions");
            }
    ...
    public static void verifyDirectory(File dir) {
        if (!dir.exists() || !dir.canRead() || !dir.isDirectory()) {
            // logger.error("Directory doesn't exist or can't be read: " +
            // dir.getAbsolutePath());
    ...
    /**
     * Read given file and return a String. File is read by lines and each line is
     * appended with a newline (including the last line).
     */
    ...
    /**
     * Read given url and return a String. File is read by lines and each line is
     * appended with a newline (including the last line).
     */
    ...
    public static void copyFile(File from, File to) {
        try (FileInputStream fis = new FileInputStream(from);
            ...
            logger.error("Unable to copy {} to {}{}", from.getAbsolutePath(), to.getAbsolutePath(),
    ...
    /**
     * Copy contents of directory if dest files are missing or older than source
     * files
     */
    public static void copyDir(File dFrom, File dTo, FilenameFilter filter) {
        ...
        for (int i = 0; files != null && i < files.length; i++) {
            src = files[i];
            if (src.getName().toLowerCase().contains("cvs"))
                continue;
    ...
    public static void copyURLs(String sResourceBase, String pattern, File dTo) {
        ...
        for (URL src : matches) {
            ...
            if (srcString.toLowerCase().contains("cvs"))
                continue;
    ...
    public static String getLocalHost(boolean bLogError) {
        ...
        try {
            ...
            if (st.hasMoreTokens()) {
                ...
                if (bLogError)
                    logger.warn("Unable to determine local host name, guessing it is: {}", sLocalHost);
            } else {
                if (bLogError)
