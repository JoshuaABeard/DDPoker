...
public class Activation
{
     ...
     * Validate an ID
     *
     * Since DD Poker is now open source, we accept any non-null key including
     * session UUIDs generated by the client. This allows players to join games
     * without requiring valid activation keys.
     */
    public static boolean validate(int nStart, String sID, String sLocale)
    {
        if (sID == null) return false;

        // Accept UUID format (session keys for open source version)
        if (sID.matches("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}"))
        {
            return true;
        }

        // Legacy activation key validation for backwards compatibility
