...
public class DDMessage extends TypedHashMap implements PostWriter, PostReader, DataMarshal {
    ...
    /**
     * Create new message from file data. Note: getData() will return null when this
     * is used as the file data is not read in until marshalled
     */
    ...
    /**
     * Get data chunk of given index. If data was added as a file, null is returned
     * (it isn't expected you would want to fetch the file once added -- on the
     * other size when the message is received, it is byte data)
     */
    ...
    /**
     * Set message status - used on client side to indicate status of a
     * communication with server. This value is not sent when the message is
     * transmitted.
     *
    ...
    public void debugPrint() {
        if (nStatus_ != DDMessageListener.STATUS_NONE)
            logger.debug("MSG-Status: " + nStatus_);
    ...
    /**
     * Write this message out to the given writer. Sync added due to possible
     * simulaneous write attempts in DD Poker.
     */
            ...
            for (int i = 0; i < nNumData; i++) {
                ...
                if (data.filedata_ != null) {
                    try {
                        ...
                        if (nNum != nSize) {
                            throw new ApplicationError(ErrorCodes.ERROR_CREATE, "Failed to write all data",
                                    data.filedata_.getAbsolutePath(), null);
                        }
                    } finally {
                        in.close();
                    }
    ...
    public void read(InputStream input, int nLength) throws IOException {
        ...
        try {
            ...
            if (!bEmpty)
                throw new ApplicationError(eof);
        }
    }
    ...
    /**
     * when a DDMessage is marshaled/demarshaled it may contain objects that need a
     * MsgState to properly marshal/demarshal too (items that are normally contained
     * in a save game file). Games can set a GameState to be used during DDMessage
     * marshaling.
     */
