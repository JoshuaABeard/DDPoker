...
/**
 * Simple client to use in messaging architecture, handles DNS timeout and other
 * things
 *
 * @author donohoe
 */
...
public class DDHttpClient {
    ...
    private static final Map<String, InetSocketAddress> DNSCACHE = Collections
            .synchronizedMap(new LinkedHashMap<String, InetSocketAddress>(256, 0.75f, true) {
                @Override
    ...
    public DDHttpClient(URL url, DDMessageListener listener, HttpOptions options) throws IOException {
        ...
        if (DEBUG_DETAILS)
            logger.debug(sHost + " is " + addr_.getAddress().getHostAddress() + (bFromCache ? " (from cache)" : ""));
    ...
    public void connect() throws IOException {
        ...
        try {
            ...
            throw new ApplicationError(ErrorCodes.ERROR_UNEXPECTED_EXCEPTION,
                    "Unexpected exception connect() to socket " + addr_.getAddress().getHostAddress() + ':'
                            + addr_.getPort(),
                    e, null);
        }

        if (DEBUG_LISTENER && listener_ != null)
            listener_.debugStep(DDMessageListener.DEBUG_STEP_BEGUN_CONNECTING,
        ...
        while (!sc_.finishConnect()) {
            ...
            if (DEBUG_LISTENER && listener_ != null)
                listener_.debugStep(DDMessageListener.DEBUG_STEP_SLEEP_CONNECTING, "sleeping " + WAITMILLIS
                        + " millis, total slept = " + nWait + " (" + CONNECTTIMEOUT + " timeout)");
    ...
    /**
     * Do the write - this calls the appropriate write method based on the HTTP
     * Options provided and the existence of the writer
     */
    public void write(PostWriter writer, String sPostContentType, String sUserAgent) throws IOException {
        ...
        if (listener_ != null)
            listener_.updateStep(DDMessageListener.STEP_SENDING);
    ...
    /**
     * Write request to server (header, user agent, content type, post data (nLength
     * bytes of post[])
     */
    public void writeHeaders(OutputStream os, String sMethod, String sContentType, String sUserAgent, int nLength)
            throws IOException {
        ...
        if (sUserAgent != null)
            sb.append("user-agent: ").append(sUserAgent).append(CRLF);
        // sb.append("Accept: text/html, image/gif, image/jpeg, image/png, *; q=.2, */*;
        // q=.2").append(CRLF);
        if (sContentType != null)
            sb.append("Content-Type: ").append(sContentType).append(CRLF);
        if (nLength > 0)
            sb.append("Content-Length: ").append(nLength).append(CRLF);
        if (options_ != null) {
            ...
            if ((options_.nBeginRange > 0) || (options_.nEndRange > 0)) {
                ...
                if (options_.nBeginRange > 0)
                    sb.append(options_.nBeginRange);
                sb.append('-');
                if (options_.nEndRange > 0)
                    sb.append(options_.nBeginRange);
                sb.append(CRLF);
            }
        }
    ...
    /**
     * Write request to proxy server - CONNECT line plus data to send to proxied
     * host
     */
    ...
    /**
     * Begin reading the response (up to headers), unless bProxyPassThru is true in
     * HttpOptions, in which case we simply initialize the input stream
     */
    public void startRead() throws IOException {
        ...
        if (options_ != null && options_.bProxyPassThru)
            return;
        ...
        while (!bDone) {
            ...
            if (listener_ != null && bFirstChar) {
                listener_.updateStep(DDMessageListener.STEP_RECEIVING);
                bFirstChar = false;
            }
    ...
    /**
     * Class to look up a host in a thread so we can timeout if takes too long
     */
    private static class LookupHost implements Runnable {
        ...
        /*
         * Returns true if address defined or exception occurred Not used, but keep it
         * in case I remember why here in first place
         */
        ...
        private InetAddress getAddress() throws UnknownHostException {
            ...
            if (addr == null)
                throw new UnknownHostException("Lookup returned no result for " + host);
    ...
    public static class HttpOptions {
        ...
        /**
         * If non-null, set to a host or IP, means the URL given is the proxy and this
         * host is the ultimate destination, via the CONNECT method
         */
        public String sConnectDestViaProxy;

        /**
         * If true, the URL given is the proxy and the "post" data is the original
         * get/post with headers, with a modified URI/host (caller is responsible for
         * modifying the URI/host) Note: this and sConnectDestViaProxy are independent;
         * and sConnectDestViaProxy takes precedence. This is basically used to simulate
         * the transparent use of a 3rd party proxy
         */
