...
- [thin-client-cleanup] `HandInfo`, `HandInfoFast`, `HandStrength`, `HandFutures`, `HandPotential`, `HoldemSimulator` in the `poker` module cannot be deleted yet. `ServerAIContext` in `pokerserver` uses all of them for the V1 server AI. `HoldemHand`, `Showdown`, and `WebSocketTournamentDirector` use `HandInfo.getBest()` for display — `HandInfoFaster` has no `getBest()` equivalent. `PocketMatrixByte`/`PocketMatrixString` kept for `AdvisorGridPanel`. See plan Task 12 status note. (2026-03-01)
