...
- [api] `api` module must declare `spring-boot-starter-data-jpa` directly in its POM. Previously arrived accidentally via `api → pokerserver → poker`. After Phase A (pokerserver no longer depends on poker), this transitive chain was cut and api failed to compile in clean builds. (2026-03-02)
...
- [deps] Deleting gameserver module removed transitive hibernate-core + h2 from pokerserver — must declare them directly in pokerserver/pom.xml. (2026-03-04)
- [deps] app-context-gameserver.xml was deleted with gameserver module then restored to pokerserver/src/main/resources/ using DriverManagerDataSource instead of c3p0 ComboPooledDataSource. (2026-03-04)
