...
- [architecture] poker module has zero dependency on pokergamecore AND pokerengine — Maven Enforcer bans both. All game logic and hand evaluation is server-side. (2026-03-06)
- [architecture] `pokergameprotocol` module contains all client-server DTOs (WebSocket messages, REST DTOs, HandEvaluationData). Zero engine dependencies. (2026-03-06)
- [architecture] Client display types (ClientCard, ClientHand, ClientBettingRound, ClientHandScore) live in poker.display package. Replace pokerengine Card/Hand/BettingRound. (2026-03-06)
- [architecture] Client-side constants split: ProtocolConstants (pokergameprotocol) for protocol contract, PokerClientConstants (poker) for display/option keys. (2026-03-06)
- [architecture] Server sends HandEvaluationData (score, handType, bestFiveCards, rank components) in AdvisorData, ShowdownPlayerData, WinnerData messages. Client no longer evaluates hands locally. (2026-03-06)
- [architecture] Client-side player state uses ClientPlayer view model (poker.online package). PokerPlayer, HoldemHand, PokerTable, Pot engine classes deleted. (2026-03-05)
- [architecture] Shared types (BettingRound, ActionType, TableState, PlayerAction, GamePlayerInfo, HandInfoFast, GameEvent system, PureTournamentClock) live in pokerengine (engine.state, engine.event packages) — used by server only. (2026-03-06)
- [poker] PokerSaveGame and local save/load removed — server manages game state. (2026-03-05)
- [poker] PokerStats removed from client — Monte Carlo simulation is server-side only. (2026-03-05)
- [poker] Showdown.doAllInPercentages() removed — server sends pre-calculated all-in percentages. (2026-03-05)
- [architecture] `pokerserver` no longer depends on `poker` (desktop client). Action constants moved to `PokerActionConstants` in `pokerengine`. (2026-03-02)
- [api] `api` module deleted (2026-03-05). All REST endpoints consolidated into `pokergameserver` controllers under `/api/v1/` prefix. Single Spring Boot process serves both game hosting and REST API.
- [poker] RemotePokerTable and RemoteHoldemHand are the ONLY table/hand classes in the poker module (standalone view models, no extends). PokerTable/HoldemHand engine classes deleted. Use ClientPokerTable/ClientHoldemHand interfaces in all new UI code. (2026-03-05)
- [cleanup] Pre-release cleanup complete on main (2026-03-04): HSQLDB→H2, gameserver module deleted, 6 dead ChainPhase files deleted, JUnit 4→5 (pokerengine + 14 JUnit 3 tests in common/gui/pokerserver), StringBuffer→StringBuilder, System.out→logger, TODOs→Future:. All 3 branches merged. (2026-03-04)
- [pokerserver] Spring Boot migration complete (2026-03-05): `PokerServerMain` is now `@SpringBootApplication`, all XML configs deleted (app-context-*.xml, persistence-pokerserver.xml, pokerserver.properties). Datasource configured via `spring.datasource.*` in `application.properties`. `PokerServer` init runs as `@PostConstruct`. (2026-03-05)
- [docker] Single-process deployment (2026-03-05): Docker runs only `PokerServerMain` (Spring Boot). No dual-process api+pokerserver. Health check at `/api/v1/health`. (2026-03-05)
- [server] GameServletRequest.getRequestURL() must return StringBuffer (HttpServletRequest interface contract) — never change this to StringBuilder. (2026-03-04)
