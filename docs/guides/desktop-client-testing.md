...
Ready-to-run scenario scripts in `tests/scenarios/`. All scripts share
`tests/scenarios/lib.sh` for common launch/cleanup/helper logic.
...
bash tests/scenarios/test-chip-conservation.sh --hands 10

# Advisor test (build already done)
bash tests/scenarios/test-advisor-do-it.sh --skip-build

# Side-pot regression (reuse a running game)
bash tests/scenarios/test-allin-side-pot.sh --skip-build --skip-launch

# Game-over test with 4 players
bash tests/scenarios/test-gameover-ranks.sh --players 4
```
...
for script in tests/scenarios/test-*.sh; do
    echo "=== $script ==="
...
bash tests/scenarios/test-app-launch.sh --skip-build
