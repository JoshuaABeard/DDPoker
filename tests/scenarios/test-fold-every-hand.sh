...
#
# Usage:
#   bash tests/scenarios/test-fold-every-hand.sh [options]
...
while true; do
    ...
    close_visible_dialog_if_any "fold-every-loop" > /dev/null 2>&1 || true
    ...
    case "$mode" in
        ...
        REBUY_CHECK)
            ...
            ;;
        QUITSAVE|NONE)
            close_visible_dialog_if_any "fold-every-idle" > /dev/null 2>&1 || true
            sleep 0.2
            ;;
    esac
    sleep 0.15
