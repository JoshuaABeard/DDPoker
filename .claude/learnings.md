...
- [dev-server] `ShowTournamentTable.setInputMode(MODE_REBUY_CHECK)` returns early before calling `super.setInputMode()`, so `nInputMode_` is NEVER set to MODE_REBUY_CHECK. The API can never report "REBUY_CHECK" as inputMode. Don't write tests that wait for REBUY_CHECK mode (2026-02-23)
...
- [dev-server] `GameStartHandler.buildProfile()` must call `profile.setLastRebuyLevel(profile.getLastLevel())` AFTER `profile.fixAll()` when `rebuys=true`. Without it, `isRebuyAllowed()` returns false (default lastRebuyLevel=0, check is `level<=0` which fails at level 1) (2026-02-23)
